...
class TestBranchDirect:
    ...
    def test_should_direct_load_branch(self):
        setup = _branch_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-branch", "live" if setup["live"] else "unit")
        if _skip:
