...
class TestHovercardDirect:

    def test_should_direct_list_hovercard(self):
        setup = _hovercard_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-hovercard", "live" if setup["live"] else "unit")
        if _skip:
...
def _hovercard_direct_setup(mockres):
