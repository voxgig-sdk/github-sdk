...
class TestSshSigningKeyDirect:

    def test_should_direct_list_ssh_signing_key(self):
        setup = _ssh_signing_key_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ssh_signing_key", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_ssh_signing_key(self):
        setup = _ssh_signing_key_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ssh_signing_key", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _ssh_signing_key_direct_setup(mockres):
