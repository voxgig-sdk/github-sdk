...
class TestCheckAutomatedSecurityFixDirect:

    def test_should_direct_load_check_automated_security_fix(self):
        setup = _check_automated_security_fix_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-check_automated_security_fix", "live" if setup["live"] else "unit")
        if _skip:
...
def _check_automated_security_fix_direct_setup(mockres):
