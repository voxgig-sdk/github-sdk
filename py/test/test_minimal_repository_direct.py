...
class TestMinimalRepositoryDirect:

    def test_should_direct_list_minimal_repository(self):
        setup = _minimal_repository_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-minimal_repository", "live" if setup["live"] else "unit")
        if _skip:
...
def _minimal_repository_direct_setup(mockres):
