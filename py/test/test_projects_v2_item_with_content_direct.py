...
class TestProjectsV2ItemWithContentDirect:

    def test_should_direct_list_projects_v2_item_with_content(self):
        setup = _projects_v2_item_with_content_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-projects_v2_item_with_content", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["org01", "project_number01"]:
    ...
    def test_should_direct_load_projects_v2_item_with_content(self):
        setup = _projects_v2_item_with_content_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-projects_v2_item_with_content", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _projects_v2_item_with_content_direct_setup(mockres):
