...
class TestStatusEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Status(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Status(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_STATUS_ENTID JSON to run live")
        client = setup["client"]
        ...
        status_ref01_data = helpers.to_map(runner.entity_data(status_ref01_ent.create(status_ref01_data, None)))
        assert status_ref01_data is not None
        assert status_ref01_data["id"] is not None
        ...
        status_ref01_list_result = status_ref01_ent.list(status_ref01_match, None)
        assert isinstance(status_ref01_list_result, list)
...
def _status_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_STATUS_ENTID"))
