...
class TestActionsCacheUsageByRepositoryDirect:

    def test_should_direct_load_actions_cache_usage_by_repository(self):
        setup = _actions_cache_usage_by_repository_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-actions_cache_usage_by_repository", "live" if setup["live"] else "unit")
        if _skip:
...
def _actions_cache_usage_by_repository_direct_setup(mockres):
