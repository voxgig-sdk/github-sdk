...
class TestFollowingDirect:

    def test_should_direct_list_following(self):
        setup = _following_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-following", "live" if setup["live"] else "unit")
        if _skip:
...
def _following_direct_setup(mockres):
