...
class TestOrganizationInvitationDirect:

    def test_should_direct_list_organization_invitation(self):
        setup = _organization_invitation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_invitation", "live" if setup["live"] else "unit")
        if _skip:
...
def _organization_invitation_direct_setup(mockres):
