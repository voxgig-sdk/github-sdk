...
class TestHookDeliveryItemDirect:

    def test_should_direct_list_hook_delivery_item(self):
        setup = _hook_delivery_item_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-hook_delivery_item", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["hook_delivery_item01", "owner01", "repo01"]:
...
def _hook_delivery_item_direct_setup(mockres):
