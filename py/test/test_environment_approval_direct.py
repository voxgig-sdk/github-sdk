...
class TestEnvironmentApprovalDirect:

    def test_should_direct_list_environment_approval(self):
        setup = _environment_approval_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-environment_approval", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["owner01", "repo01", "run01"]:
...
def _environment_approval_direct_setup(mockres):
