...
class TestReleaseAssetDirect:

    def test_should_direct_list_release_asset(self):
        setup = _release_asset_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-release_asset", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["release_asset01", "owner01", "repo01"]:
    ...
    def test_should_direct_load_release_asset(self):
        setup = _release_asset_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-release_asset", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _release_asset_direct_setup(mockres):
