...
class TestReferrerDirect:

    def test_should_direct_list_referrer(self):
        setup = _referrer_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-referrer", "live" if setup["live"] else "unit")
        if _skip:
...
def _referrer_direct_setup(mockres):
