...
class TestDeploymentStatusDirect:

    def test_should_direct_list_deployment_status(self):
        setup = _deployment_status_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-deployment_status", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["deployment_status01", "owner01", "repo01"]:
    ...
    def test_should_direct_load_deployment_status(self):
        setup = _deployment_status_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-deployment_status", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _deployment_status_direct_setup(mockres):
