...
class TestActionsForkPrContributorApprovalDirect:

    def test_should_direct_load_actions_fork_pr_contributor_approval(self):
        setup = _actions_fork_pr_contributor_approval_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-actions_fork_pr_contributor_approval", "live" if setup["live"] else "unit")
        if _skip:
...
def _actions_fork_pr_contributor_approval_direct_setup(mockres):
