...
class TestIssueTypeDirect:

    def test_should_direct_list_issue_type(self):
        setup = _issue_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-issue_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _issue_type_direct_setup(mockres):
