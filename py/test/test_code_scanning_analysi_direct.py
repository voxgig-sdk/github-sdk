...
class TestCodeScanningAnalysiDirect:

    def test_should_direct_list_code_scanning_analysi(self):
        setup = _code_scanning_analysi_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-code_scanning_analysi", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_code_scanning_analysi(self):
        setup = _code_scanning_analysi_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-code_scanning_analysi", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _code_scanning_analysi_direct_setup(mockres):
