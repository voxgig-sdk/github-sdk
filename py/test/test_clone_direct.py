...
class TestCloneDirect:

    def test_should_direct_list_clone(self):
        setup = _clone_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-clone", "live" if setup["live"] else "unit")
        if _skip:
...
def _clone_direct_setup(mockres):
