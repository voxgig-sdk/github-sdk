...
class TestCodeFrequencyStatDirect:

    def test_should_direct_list_code_frequency_stat(self):
        setup = _code_frequency_stat_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-code_frequency_stat", "live" if setup["live"] else "unit")
        if _skip:
...
def _code_frequency_stat_direct_setup(mockres):
