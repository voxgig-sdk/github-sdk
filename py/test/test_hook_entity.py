...
class TestHookEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Hook(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Hook(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_HOOK_ENTID JSON to run live")
        client = setup["client"]
        ...
        hook_ref01_list_result = hook_ref01_ent.list(hook_ref01_match, None)
        assert isinstance(hook_ref01_list_result, list)
        ...
        hook_ref01_markdef_up0_value = "Mark01-hook_ref01_" + str(setup["now"])
        ...
        assert hook_ref01_resdata_up0[hook_ref01_markdef_up0_name] == hook_ref01_markdef_up0_value

        # LOAD
        hook_ref01_match_dt0 = {
            "id": hook_ref01_data["id"],
        }
        hook_ref01_data_dt0_loaded = hook_ref01_ent.load(hook_ref01_match_dt0, None)
        hook_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(hook_ref01_data_dt0_loaded))
        assert hook_ref01_data_dt0_load_result is not None
        assert hook_ref01_data_dt0_load_result["id"] == hook_ref01_data["id"]
...
def _hook_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_HOOK_ENTID"))
