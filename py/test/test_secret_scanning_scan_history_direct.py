...
class TestSecretScanningScanHistoryDirect:

    def test_should_direct_list_secret_scanning_scan_history(self):
        setup = _secret_scanning_scan_history_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-secret_scanning_scan_history", "live" if setup["live"] else "unit")
        if _skip:
...
def _secret_scanning_scan_history_direct_setup(mockres):
