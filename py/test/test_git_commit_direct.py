...
class TestGitCommitDirect:

    def test_should_direct_load_git_commit(self):
        setup = _git_commit_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-git_commit", "live" if setup["live"] else "unit")
        if _skip:
...
def _git_commit_direct_setup(mockres):
