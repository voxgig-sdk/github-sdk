...
class TestMetaEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Meta(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Meta(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_META_ENTID JSON to run live")
        client = setup["client"]
        ...
        meta_ref01_list_result = meta_ref01_ent.list(meta_ref01_match, None)
        assert isinstance(meta_ref01_list_result, list)
...
def _meta_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_META_ENTID"))
