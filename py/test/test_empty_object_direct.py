...
class TestEmptyObjectDirect:

    def test_should_direct_load_empty_object(self):
        setup = _empty_object_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-empty_object", "live" if setup["live"] else "unit")
        if _skip:
...
def _empty_object_direct_setup(mockres):
