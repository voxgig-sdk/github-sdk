...
class GithubTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GithubTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GithubTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GithubTestRunner.getenv("GITHUB_TEST_LIVE")
        override = GithubTestRunner.getenv("GITHUB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GithubTestRunner.getenv(key)
        ...
        explain = GithubTestRunner.getenv("GITHUB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GITHUB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GithubTestRunner._test_control is not None:
            return GithubTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GithubTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GithubTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GithubTestRunner.load_test_control()
...
def load_env_local():
    GithubTestRunner.load_env_local()


def env_override(m):
    return GithubTestRunner.env_override(m)


def entity_data(v):
    return GithubTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GithubTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GithubTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GithubTestRunner.load_test_control()


def live_client_options():
    return GithubTestRunner.live_client_options()


def live_delay_ms():
    return GithubTestRunner.live_delay_ms()
