...
class TestPorterLargeFileDirect:

    def test_should_direct_list_porter_large_file(self):
        setup = _porter_large_file_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-porter_large_file", "live" if setup["live"] else "unit")
        if _skip:
...
def _porter_large_file_direct_setup(mockres):
