...
class TestBillingUsageReportDirect:

    def test_should_direct_list_billing_usage_report(self):
        setup = _billing_usage_report_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-billing_usage_report", "live" if setup["live"] else "unit")
        if _skip:
...
def _billing_usage_report_direct_setup(mockres):
