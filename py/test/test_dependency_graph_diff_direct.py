...
class TestDependencyGraphDiffDirect:

    def test_should_direct_load_dependency_graph_diff(self):
        setup = _dependency_graph_diff_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dependency_graph_diff", "live" if setup["live"] else "unit")
        if _skip:
...
def _dependency_graph_diff_direct_setup(mockres):
