...
class TestCheckRunDirect:

    def test_should_direct_load_check_run(self):
        setup = _check_run_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-check_run", "live" if setup["live"] else "unit")
        if _skip:
...
def _check_run_direct_setup(mockres):
