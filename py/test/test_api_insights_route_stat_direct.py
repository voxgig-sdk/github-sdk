...
class TestApiInsightsRouteStatDirect:

    def test_should_direct_list_api_insights_route_stat(self):
        setup = _api_insights_route_stat_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api_insights_route_stat", "live" if setup["live"] else "unit")
        if _skip:
...
def _api_insights_route_stat_direct_setup(mockres):
