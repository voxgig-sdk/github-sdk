...
class TestArtifactDirect:

    def test_should_direct_load_artifact(self):
        setup = _artifact_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-artifact", "live" if setup["live"] else "unit")
        if _skip:
...
def _artifact_direct_setup(mockres):
