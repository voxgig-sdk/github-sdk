...
class TestCheckAnnotationDirect:

    def test_should_direct_list_check_annotation(self):
        setup = _check_annotation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-check_annotation", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["check_run01", "owner01", "repo01"]:
...
def _check_annotation_direct_setup(mockres):
