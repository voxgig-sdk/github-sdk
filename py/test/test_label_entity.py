    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_LABEL_ENTID JSON to run live")
        client = setup["client"]
        ...
        label_ref01_markdef_up0_value = "Mark01-label_ref01_" + str(setup["now"])
        ...
        assert label_ref01_resdata_up0[label_ref01_markdef_up0_name] == label_ref01_markdef_up0_value
...
def _label_basic_setup(extra):
    ...
    idmap = vs.transform(
        ["label01", "label02", "label03", "repo01", "repo02", "repo03", "milestone01", "milestone02", "milestone03", "milestone_number01", "owner01"],
        {
            "`$PACK`": ["", {
                "`$KEY`": "`$COPY`",
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_LABEL_ENTID"))
