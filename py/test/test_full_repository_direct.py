...
class TestFullRepositoryDirect:

    def test_should_direct_load_full_repository(self):
        setup = _full_repository_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-full_repository", "live" if setup["live"] else "unit")
        if _skip:
...
def _full_repository_direct_setup(mockres):
