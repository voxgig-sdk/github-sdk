...
class TestCollaboratorDirect:
    ...
    def test_should_direct_list_collaborator(self):
        setup = _collaborator_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-collaborator", "live" if setup["live"] else "unit")
        if _skip:
...
def _collaborator_direct_setup(mockres):
