...
class TestCodeScanningVariantAnalysiDirect:

    def test_should_direct_load_code_scanning_variant_analysi(self):
        setup = _code_scanning_variant_analysi_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-code_scanning_variant_analysi", "live" if setup["live"] else "unit")
        if _skip:
...
def _code_scanning_variant_analysi_direct_setup(mockres):
