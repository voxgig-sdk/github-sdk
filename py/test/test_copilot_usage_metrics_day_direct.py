...
class TestCopilotUsageMetricsDayDirect:

    def test_should_direct_list_copilot_usage_metrics_day(self):
        setup = _copilot_usage_metrics_day_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-copilot_usage_metrics_day", "live" if setup["live"] else "unit")
        if _skip:
...
def _copilot_usage_metrics_day_direct_setup(mockres):
