...
class TestOrganizationProgrammaticAccessGrantDirect:

    def test_should_direct_list_organization_programmatic_access_grant(self):
        setup = _organization_programmatic_access_grant_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_programmatic_access_grant", "live" if setup["live"] else "unit")
        if _skip:
...
def _organization_programmatic_access_grant_direct_setup(mockres):
