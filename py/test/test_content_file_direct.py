...
class TestContentFileDirect:

    def test_should_direct_load_content_file(self):
        setup = _content_file_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-content_file", "live" if setup["live"] else "unit")
        if _skip:
...
def _content_file_direct_setup(mockres):
