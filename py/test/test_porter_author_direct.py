...
class TestPorterAuthorDirect:

    def test_should_direct_list_porter_author(self):
        setup = _porter_author_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-porter_author", "live" if setup["live"] else "unit")
        if _skip:
...
def _porter_author_direct_setup(mockres):
