...
class TestDependabotAlertWithRepositoryDirect:

    def test_should_direct_list_dependabot_alert_with_repository(self):
        setup = _dependabot_alert_with_repository_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-dependabot_alert_with_repository", "live" if setup["live"] else "unit")
        if _skip:
...
def _dependabot_alert_with_repository_direct_setup(mockres):
