...
class TestBranchShortDirect:

    def test_should_direct_list_branch_short(self):
        setup = _branch_short_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-branch_short", "live" if setup["live"] else "unit")
        if _skip:
...
def _branch_short_direct_setup(mockres):
