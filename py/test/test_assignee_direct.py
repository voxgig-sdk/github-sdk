...
class TestAssigneeDirect:

    def test_should_direct_list_assignee(self):
        setup = _assignee_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-assignee", "live" if setup["live"] else "unit")
        if _skip:
...
def _assignee_direct_setup(mockres):
