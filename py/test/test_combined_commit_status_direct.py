...
class TestCombinedCommitStatusDirect:

    def test_should_direct_list_combined_commit_status(self):
        setup = _combined_commit_status_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-combined_commit_status", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["owner01", "ref01", "repo01"]:
...
def _combined_commit_status_direct_setup(mockres):
