...
class TestCollaboratorEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Collaborator(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Collaborator(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_COLLABORATOR_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        collaborator_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.collaborator")))
        collaborator_ref01_data = None
        if len(collaborator_ref01_data_raw) > 0:
        ...
        collaborator_ref01_list_result = collaborator_ref01_ent.list(collaborator_ref01_match, None)
        assert isinstance(collaborator_ref01_list_result, list)
...
def _collaborator_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "GITHUB_TEST_COLLABORATOR_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_COLLABORATOR_ENTID"))
