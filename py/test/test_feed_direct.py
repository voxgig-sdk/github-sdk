...
class TestFeedDirect:

    def test_should_direct_list_feed(self):
        setup = _feed_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-feed", "live" if setup["live"] else "unit")
        if _skip:
...
def _feed_direct_setup(mockres):
