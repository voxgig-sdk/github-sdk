...
class TestGitignoreDirect:

    def test_should_direct_list_gitignore(self):
        setup = _gitignore_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-gitignore", "live" if setup["live"] else "unit")
        if _skip:
...
def _gitignore_direct_setup(mockres):
