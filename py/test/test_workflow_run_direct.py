...
class TestWorkflowRunDirect:

    def test_should_direct_load_workflow_run(self):
        setup = _workflow_run_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-workflow_run", "live" if setup["live"] else "unit")
        if _skip:
...
def _workflow_run_direct_setup(mockres):
