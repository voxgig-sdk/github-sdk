...
class TestCodeScanningAutofixDirect:

    def test_should_direct_load_code_scanning_autofix(self):
        setup = _code_scanning_autofix_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-code_scanning_autofix", "live" if setup["live"] else "unit")
        if _skip:
...
def _code_scanning_autofix_direct_setup(mockres):
