...
class TestRulesetVersionDirect:

    def test_should_direct_list_ruleset_version(self):
        setup = _ruleset_version_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ruleset_version", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["ruleset_version01", "owner01", "repo01"]:
...
def _ruleset_version_direct_setup(mockres):
