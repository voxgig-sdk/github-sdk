...
class TestStatusDirect:
    ...
    def test_should_direct_list_status(self):
        setup = _status_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-status", "live" if setup["live"] else "unit")
        if _skip:
