...
class TestBaseGistDirect:

    def test_should_direct_list_base_gist(self):
        setup = _base_gist_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-base_gist", "live" if setup["live"] else "unit")
        if _skip:
...
def _base_gist_direct_setup(mockres):
