...
class TestPullRequestSimpleEntity:

    def test_should_create_instance(self):
        testsdk = GithubSDK.test(None, None)
        ent = testsdk.PullRequestSimple(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _pull_request_simple_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "pull_request_simple." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_PULL_REQUEST_SIMPLE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        pull_request_simple_ref01_ent = client.PullRequestSimple(None)
        ...
        pull_request_simple_ref01_data = helpers.to_map(runner.entity_data(pull_request_simple_ref01_ent.create(pull_request_simple_ref01_data, None)))
        assert pull_request_simple_ref01_data is not None




def _pull_request_simple_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/pull_request_simple/PullRequestSimpleTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITHUB_TEST_PULL_REQUEST_SIMPLE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_PULL_REQUEST_SIMPLE_ENTID"))
