...
class TestCommitActivityDirect:

    def test_should_direct_list_commit_activity(self):
        setup = _commit_activity_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-commit_activity", "live" if setup["live"] else "unit")
        if _skip:
...
def _commit_activity_direct_setup(mockres):
