...
class TestParticipationDirect:

    def test_should_direct_list_participation(self):
        setup = _participation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-participation", "live" if setup["live"] else "unit")
        if _skip:
...
def _participation_direct_setup(mockres):
