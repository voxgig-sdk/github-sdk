...
class TestOidcCustomSubDirect:

    def test_should_direct_list_oidc_custom_sub(self):
        setup = _oidc_custom_sub_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-oidc_custom_sub", "live" if setup["live"] else "unit")
        if _skip:
...
def _oidc_custom_sub_direct_setup(mockres):
