...
class TestSecretScanningLocationDirect:

    def test_should_direct_list_secret_scanning_location(self):
        setup = _secret_scanning_location_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-secret_scanning_location", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["alert_number01", "owner01", "repo01"]:
...
def _secret_scanning_location_direct_setup(mockres):
