...
class TestDiffEntryDirect:

    def test_should_direct_list_diff_entry(self):
        setup = _diff_entry_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-diff_entry", "live" if setup["live"] else "unit")
        if _skip:
...
def _diff_entry_direct_setup(mockres):
