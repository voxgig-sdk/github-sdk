...
class TestRunnerDirect:
    ...
    def test_should_direct_load_runner(self):
        setup = _runner_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-runner", "live" if setup["live"] else "unit")
        if _skip:
...
def _runner_direct_setup(mockres):
