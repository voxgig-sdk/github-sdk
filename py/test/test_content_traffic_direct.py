...
class TestContentTrafficDirect:

    def test_should_direct_list_content_traffic(self):
        setup = _content_traffic_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-content_traffic", "live" if setup["live"] else "unit")
        if _skip:
...
def _content_traffic_direct_setup(mockres):
