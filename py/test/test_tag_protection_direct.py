...
class TestTagProtectionDirect:

    def test_should_direct_list_tag_protection(self):
        setup = _tag_protection_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tag_protection", "live" if setup["live"] else "unit")
        if _skip:
...
def _tag_protection_direct_setup(mockres):
