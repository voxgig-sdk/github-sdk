...
class TestGitTreeDirect:

    def test_should_direct_load_git_tree(self):
        setup = _git_tree_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-git_tree", "live" if setup["live"] else "unit")
        if _skip:
...
def _git_tree_direct_setup(mockres):
