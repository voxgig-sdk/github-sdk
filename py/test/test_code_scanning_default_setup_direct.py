...
class TestCodeScanningDefaultSetupDirect:

    def test_should_direct_list_code_scanning_default_setup(self):
        setup = _code_scanning_default_setup_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-code_scanning_default_setup", "live" if setup["live"] else "unit")
        if _skip:
...
def _code_scanning_default_setup_direct_setup(mockres):
