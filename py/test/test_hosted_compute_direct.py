...
class TestHostedComputeDirect:

    def test_should_direct_list_hosted_compute(self):
        setup = _hosted_compute_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-hosted_compute", "live" if setup["live"] else "unit")
        if _skip:
...
def _hosted_compute_direct_setup(mockres):
