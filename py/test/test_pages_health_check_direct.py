...
class TestPagesHealthCheckDirect:

    def test_should_direct_load_pages_health_check(self):
        setup = _pages_health_check_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-pages_health_check", "live" if setup["live"] else "unit")
        if _skip:
...
def _pages_health_check_direct_setup(mockres):
