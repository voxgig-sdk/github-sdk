...
class TestActionsPublicKeyDirect:

    def test_should_direct_load_actions_public_key(self):
        setup = _actions_public_key_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-actions_public_key", "live" if setup["live"] else "unit")
        if _skip:
...
def _actions_public_key_direct_setup(mockres):
