...
class TestCheckDirect:
    ...
    def test_should_direct_list_check(self):
        setup = _check_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-check", "live" if setup["live"] else "unit")
        if _skip:
