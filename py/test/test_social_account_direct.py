...
class TestSocialAccountDirect:

    def test_should_direct_list_social_account(self):
        setup = _social_account_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-social_account", "live" if setup["live"] else "unit")
        if _skip:
...
def _social_account_direct_setup(mockres):
