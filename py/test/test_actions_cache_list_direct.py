...
class TestActionsCacheListDirect:

    def test_should_direct_list_actions_cache_list(self):
        setup = _actions_cache_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-actions_cache_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _actions_cache_list_direct_setup(mockres):
