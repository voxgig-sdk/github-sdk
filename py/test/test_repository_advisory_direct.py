...
class TestRepositoryAdvisoryDirect:

    def test_should_direct_list_repository_advisory(self):
        setup = _repository_advisory_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_advisory", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_repository_advisory(self):
        setup = _repository_advisory_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-repository_advisory", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _repository_advisory_direct_setup(mockres):
