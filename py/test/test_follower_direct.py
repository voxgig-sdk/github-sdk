...
class TestFollowerDirect:

    def test_should_direct_list_follower(self):
        setup = _follower_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-follower", "live" if setup["live"] else "unit")
        if _skip:
...
def _follower_direct_setup(mockres):
