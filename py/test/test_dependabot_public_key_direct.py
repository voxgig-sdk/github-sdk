...
class TestDependabotPublicKeyDirect:

    def test_should_direct_load_dependabot_public_key(self):
        setup = _dependabot_public_key_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dependabot_public_key", "live" if setup["live"] else "unit")
        if _skip:
...
def _dependabot_public_key_direct_setup(mockres):
