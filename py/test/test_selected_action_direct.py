...
class TestSelectedActionDirect:

    def test_should_direct_list_selected_action(self):
        setup = _selected_action_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-selected_action", "live" if setup["live"] else "unit")
        if _skip:
...
def _selected_action_direct_setup(mockres):
