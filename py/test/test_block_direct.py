...
class TestBlockDirect:

    def test_should_direct_list_block(self):
        setup = _block_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-block", "live" if setup["live"] else "unit")
        if _skip:
...
def _block_direct_setup(mockres):
