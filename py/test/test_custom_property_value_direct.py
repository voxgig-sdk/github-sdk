...
class TestCustomPropertyValueDirect:

    def test_should_direct_list_custom_property_value(self):
        setup = _custom_property_value_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-custom_property_value", "live" if setup["live"] else "unit")
        if _skip:
...
def _custom_property_value_direct_setup(mockres):
