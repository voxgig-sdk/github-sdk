...
class TestWebhookConfigDirect:

    def test_should_direct_load_webhook_config(self):
        setup = _webhook_config_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-webhook_config", "live" if setup["live"] else "unit")
        if _skip:
...
def _webhook_config_direct_setup(mockres):
