    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_PULL_ENTID JSON to run live")
        client = setup["client"]
        ...
        pull_ref01_markdef_up0_value = "Mark01-pull_ref01_" + str(setup["now"])
        pull_ref01_data_up0_up[pull_ref01_markdef_up0_name] = pull_ref01_markdef_up0_value

        pull_ref01_resdata_up0 = helpers.to_map(runner.entity_data(pull_ref01_ent.update(pull_ref01_data_up0_up, None)))
        assert pull_ref01_resdata_up0 is not None
        assert pull_ref01_resdata_up0["id"] == pull_ref01_data_up0_up["id"]
        assert pull_ref01_resdata_up0[pull_ref01_markdef_up0_name] == pull_ref01_markdef_up0_value
...
def _pull_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_PULL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)
    if idmap_resolved.get("repo_id") is None:
        idmap_resolved["repo_id"] = idmap_resolved.get("repo01")
