...
class TestRunnerApplicationDirect:

    def test_should_direct_list_runner_application(self):
        setup = _runner_application_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-runner_application", "live" if setup["live"] else "unit")
        if _skip:
...
def _runner_application_direct_setup(mockres):
