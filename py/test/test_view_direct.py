...
class TestViewDirect:
    ...
    def test_should_direct_list_view(self):
        setup = _view_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-view", "live" if setup["live"] else "unit")
        if _skip:
