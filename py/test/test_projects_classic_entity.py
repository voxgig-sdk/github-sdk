...
class TestProjectsClassicEntity:

    def test_should_create_instance(self):
        testsdk = GithubSDK.test(None, None)
        ent = testsdk.ProjectsClassic(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _projects_classic_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "update", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "projects_classic." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITHUB_TEST_PROJECTS_CLASSIC_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        projects_classic_ref01_ent = client.ProjectsClassic(None)
        ...
        projects_classic_ref01_data = helpers.to_map(runner.entity_data(projects_classic_ref01_ent.create(projects_classic_ref01_data, None)))
        assert projects_classic_ref01_data is not None
        ...
        projects_classic_ref01_markdef_up0_value = "Mark01-projects_classic_ref01_" + str(setup["now"])
        projects_classic_ref01_data_up0_up[projects_classic_ref01_markdef_up0_name] = projects_classic_ref01_markdef_up0_value

        projects_classic_ref01_resdata_up0 = helpers.to_map(runner.entity_data(projects_classic_ref01_ent.update(projects_classic_ref01_data_up0_up, None)))
        assert projects_classic_ref01_resdata_up0 is not None
        assert projects_classic_ref01_resdata_up0[projects_classic_ref01_markdef_up0_name] == projects_classic_ref01_markdef_up0_value




def _projects_classic_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/projects_classic/ProjectsClassicTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITHUB_TEST_PROJECTS_CLASSIC_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITHUB_TEST_PROJECTS_CLASSIC_ENTID"))
