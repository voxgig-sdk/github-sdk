...
class TestShortBranchDirect:

    def test_should_direct_list_short_branch(self):
        setup = _short_branch_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-short_branch", "live" if setup["live"] else "unit")
        if _skip:
...
def _short_branch_direct_setup(mockres):
