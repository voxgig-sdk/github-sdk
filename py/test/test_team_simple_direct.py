...
class TestTeamSimpleDirect:

    def test_should_direct_list_team_simple(self):
        setup = _team_simple_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-team_simple", "live" if setup["live"] else "unit")
        if _skip:
...
def _team_simple_direct_setup(mockres):
