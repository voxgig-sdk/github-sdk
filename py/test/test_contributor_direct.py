...
class TestContributorDirect:

    def test_should_direct_list_contributor(self):
        setup = _contributor_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-contributor", "live" if setup["live"] else "unit")
        if _skip:
...
def _contributor_direct_setup(mockres):
