...
class TestCodeownersErrorDirect:

    def test_should_direct_list_codeowners_error(self):
        setup = _codeowners_error_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-codeowners_error", "live" if setup["live"] else "unit")
        if _skip:
...
def _codeowners_error_direct_setup(mockres):
