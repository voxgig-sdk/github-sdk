...
class TestBlobDirect:

    def test_should_direct_load_blob(self):
        setup = _blob_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-blob", "live" if setup["live"] else "unit")
        if _skip:
...
def _blob_direct_setup(mockres):
