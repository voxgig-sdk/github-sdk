...
class TestApiInsightsTimeStatDirect:

    def test_should_direct_list_api_insights_time_stat(self):
        setup = _api_insights_time_stat_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api_insights_time_stat", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["actor01", "actor_type01", "org01"]:
    ...
    def test_should_direct_load_api_insights_time_stat(self):
        setup = _api_insights_time_stat_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-api_insights_time_stat", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _api_insights_time_stat_direct_setup(mockres):
