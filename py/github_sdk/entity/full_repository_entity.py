...
class FullRepositoryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FullRepositoryEntity(self._client, opts)
    ...
    def data_get(self) -> FullRepository:
    ...
    def match_get(self) -> FullRepository:
    ...
    def load(self, reqmatch=None, ctrl=None) -> FullRepository:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.FullRepository().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: FullRepositoryCreateData, ctrl=None) -> FullRepository:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: FullRepositoryUpdateData, ctrl=None) -> FullRepository:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
