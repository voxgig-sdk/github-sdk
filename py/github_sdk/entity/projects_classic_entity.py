...
class ProjectsClassicEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProjectsClassicEntity(self._client, opts)
    ...
    def data_get(self) -> ProjectsClassic:
    ...
    def match_get(self) -> ProjectsClassic:
    ...
    def create(self, reqdata: ProjectsClassicCreateData, ctrl=None) -> ProjectsClassic:
        utility = self._utility
    ...
    def update(self, reqdata: ProjectsClassicUpdateData, ctrl=None) -> ProjectsClassic:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ProjectsClassic:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ProjectsClassic().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
