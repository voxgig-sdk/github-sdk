...
class ContentFileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ContentFileEntity(self._client, opts)
    ...
    def match_get(self) -> ContentFile:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ContentFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ContentFile().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
