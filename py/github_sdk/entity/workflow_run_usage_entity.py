...
class WorkflowRunUsageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WorkflowRunUsageEntity(self._client, opts)
    ...
    def data_get(self) -> WorkflowRunUsage:
    ...
    def match_get(self) -> WorkflowRunUsage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> WorkflowRunUsage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.WorkflowRunUsage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
