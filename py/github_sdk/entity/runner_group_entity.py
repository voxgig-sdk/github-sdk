...
class RunnerGroupEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RunnerGroupEntity(self._client, opts)
    ...
    def match_get(self) -> RunnerGroup:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RunnerGroup:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RunnerGroup().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RunnerGroupCreateData, ctrl=None) -> RunnerGroup:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: RunnerGroupUpdateData, ctrl=None) -> RunnerGroup:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
