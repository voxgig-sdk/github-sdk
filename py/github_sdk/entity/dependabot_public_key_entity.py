...
class DependabotPublicKeyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DependabotPublicKeyEntity(self._client, opts)
    ...
    def data_get(self) -> DependabotPublicKey:
    ...
    def match_get(self) -> DependabotPublicKey:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DependabotPublicKey:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DependabotPublicKey().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
