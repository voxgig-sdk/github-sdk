...
class DependabotSecretEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DependabotSecretEntity(self._client, opts)
    ...
    def data_get(self) -> DependabotSecret:
    ...
    def match_get(self) -> DependabotSecret:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DependabotSecret:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DependabotSecret().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
