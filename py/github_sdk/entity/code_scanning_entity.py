...
class CodeScanningEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CodeScanningEntity(self._client, opts)
    ...
    def data_get(self) -> CodeScanning:
    ...
    def match_get(self) -> CodeScanning:
    ...
    def create(self, reqdata: CodeScanningCreateData, ctrl=None) -> CodeScanning:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CodeScanning:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CodeScanning().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
