...
class ThreadSubscriptionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ThreadSubscriptionEntity(self._client, opts)
    ...
    def data_get(self) -> ThreadSubscription:
    ...
    def match_get(self) -> ThreadSubscription:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ThreadSubscription:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ThreadSubscription().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: ThreadSubscriptionUpdateData, ctrl=None) -> ThreadSubscription:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
