...
from github_sdk.github_types import (
    Interaction,
    InteractionLoadMatch,
    InteractionRemoveMatch,
)
...
class InteractionEntity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Interaction:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Interaction().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Interaction:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Interaction().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
