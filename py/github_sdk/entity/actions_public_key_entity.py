...
class ActionsPublicKeyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActionsPublicKeyEntity(self._client, opts)
    ...
    def data_get(self) -> ActionsPublicKey:
    ...
    def match_get(self) -> ActionsPublicKey:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ActionsPublicKey:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ActionsPublicKey().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
