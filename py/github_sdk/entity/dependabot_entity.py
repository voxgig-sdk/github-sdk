...
class DependabotEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DependabotEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Dependabot]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Dependabot().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: DependabotUpdateData, ctrl=None) -> Dependabot:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Dependabot:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Dependabot().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
