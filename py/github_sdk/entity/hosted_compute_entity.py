...
class HostedComputeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HostedComputeEntity(self._client, opts)
    ...
    def data_get(self) -> HostedCompute:
    ...
    def match_get(self) -> HostedCompute:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[HostedCompute]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.HostedCompute().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> HostedCompute:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.HostedCompute().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
