...
class ActionsCacheListEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActionsCacheListEntity(self._client, opts)
    ...
    def data_get(self) -> ActionsCacheList:
    ...
    def match_get(self) -> ActionsCacheList:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ActionsCacheList]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ActionsCacheList().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ActionsCacheList:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ActionsCacheList().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
