...
class SelfHostedRunnerEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SelfHostedRunnerEntity(self._client, opts)
    ...
    def data_get(self) -> SelfHostedRunner:
    ...
    def match_get(self) -> SelfHostedRunner:
    ...
    def load(self, reqmatch=None, ctrl=None) -> SelfHostedRunner:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.SelfHostedRunner().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
