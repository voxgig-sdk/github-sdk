...
class PullRequestSimpleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PullRequestSimpleEntity(self._client, opts)
    ...
    def data_get(self) -> PullRequestSimple:
    ...
    def match_get(self) -> PullRequestSimple:
    ...
    def create(self, reqdata: PullRequestSimpleCreateData, ctrl=None) -> PullRequestSimple:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PullRequestSimple:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PullRequestSimple().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
