...
class DependabotAlertEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DependabotAlertEntity(self._client, opts)
    ...
    def data_get(self) -> DependabotAlert:
    ...
    def match_get(self) -> DependabotAlert:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DependabotAlert:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DependabotAlert().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DependabotAlert]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DependabotAlert().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: DependabotAlertUpdateData, ctrl=None) -> DependabotAlert:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
