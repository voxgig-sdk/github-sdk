...
class CodeSecurityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CodeSecurityEntity(self._client, opts)
    ...
    def data_get(self) -> CodeSecurity:
    ...
    def match_get(self) -> CodeSecurity:
    ...
    def update(self, reqdata: CodeSecurityUpdateData, ctrl=None) -> CodeSecurity:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CodeSecurity:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CodeSecurity().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
