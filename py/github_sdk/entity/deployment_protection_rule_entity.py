...
class DeploymentProtectionRuleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeploymentProtectionRuleEntity(self._client, opts)
    ...
    def data_get(self) -> DeploymentProtectionRule:
    ...
    def match_get(self) -> DeploymentProtectionRule:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DeploymentProtectionRule:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DeploymentProtectionRule().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DeploymentProtectionRuleCreateData, ctrl=None) -> DeploymentProtectionRule:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
