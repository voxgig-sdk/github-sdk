...
class NetworkConfigurationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NetworkConfigurationEntity(self._client, opts)
    ...
    def data_get(self) -> NetworkConfiguration:
    ...
    def match_get(self) -> NetworkConfiguration:
    ...
    def load(self, reqmatch=None, ctrl=None) -> NetworkConfiguration:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.NetworkConfiguration().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: NetworkConfigurationCreateData, ctrl=None) -> NetworkConfiguration:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: NetworkConfigurationUpdateData, ctrl=None) -> NetworkConfiguration:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
