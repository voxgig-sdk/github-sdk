...
class ProjectsV2FieldEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProjectsV2FieldEntity(self._client, opts)
    ...
    def data_get(self) -> ProjectsV2Field:
    ...
    def match_get(self) -> ProjectsV2Field:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ProjectsV2Field:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ProjectsV2Field().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ProjectsV2Field]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ProjectsV2Field().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
