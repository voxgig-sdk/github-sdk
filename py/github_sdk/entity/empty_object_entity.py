...
class EmptyObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EmptyObjectEntity(self._client, opts)
    ...
    def match_get(self) -> EmptyObject:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EmptyObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EmptyObject().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: EmptyObjectCreateData, ctrl=None) -> EmptyObject:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: EmptyObjectUpdateData, ctrl=None) -> EmptyObject:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
