...
class ContributorEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ContributorEntity(self._client, opts)
    ...
    def match_get(self) -> Contributor:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Contributor]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Contributor().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
