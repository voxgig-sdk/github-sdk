...
class EnterpriseTeamMembershipEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EnterpriseTeamMembershipEntity(self._client, opts)
    ...
    def data_get(self) -> EnterpriseTeamMembership:
    ...
    def match_get(self) -> EnterpriseTeamMembership:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> EnterpriseTeamMembership:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.EnterpriseTeamMembership().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
