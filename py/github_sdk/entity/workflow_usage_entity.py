...
class WorkflowUsageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WorkflowUsageEntity(self._client, opts)
    ...
    def data_get(self) -> WorkflowUsage:
    ...
    def match_get(self) -> WorkflowUsage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> WorkflowUsage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.WorkflowUsage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
