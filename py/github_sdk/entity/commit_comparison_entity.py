...
class CommitComparisonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CommitComparisonEntity(self._client, opts)
    ...
    def data_get(self) -> CommitComparison:
    ...
    def match_get(self) -> CommitComparison:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CommitComparison:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CommitComparison().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
