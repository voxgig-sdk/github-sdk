...
class HookDeliveryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HookDeliveryEntity(self._client, opts)
    ...
    def data_get(self) -> HookDelivery:
    ...
    def match_get(self) -> HookDelivery:
    ...
    def load(self, reqmatch=None, ctrl=None) -> HookDelivery:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.HookDelivery().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
