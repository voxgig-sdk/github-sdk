...
class WorkflowRunEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WorkflowRunEntity(self._client, opts)
    ...
    def match_get(self) -> WorkflowRun:
    ...
    def load(self, reqmatch=None, ctrl=None) -> WorkflowRun:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.WorkflowRun().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: WorkflowRunCreateData, ctrl=None) -> WorkflowRun:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
