...
class ProjectsV2ItemWithContentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProjectsV2ItemWithContentEntity(self._client, opts)
    ...
    def data_get(self) -> ProjectsV2ItemWithContent:
    ...
    def match_get(self) -> ProjectsV2ItemWithContent:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ProjectsV2ItemWithContent:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ProjectsV2ItemWithContent().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ProjectsV2ItemWithContent]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ProjectsV2ItemWithContent().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: ProjectsV2ItemWithContentUpdateData, ctrl=None) -> ProjectsV2ItemWithContent:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
