...
class ActionsHostedRunnerEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActionsHostedRunnerEntity(self._client, opts)
    ...
    def data_get(self) -> ActionsHostedRunner:
    ...
    def match_get(self) -> ActionsHostedRunner:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ActionsHostedRunner:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ActionsHostedRunner().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ActionsHostedRunnerCreateData, ctrl=None) -> ActionsHostedRunner:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: ActionsHostedRunnerUpdateData, ctrl=None) -> ActionsHostedRunner:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
