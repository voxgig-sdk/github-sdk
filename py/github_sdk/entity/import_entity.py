...
class ImportEntity:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Import]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Import().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: ImportUpdateData, ctrl=None) -> Import:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
