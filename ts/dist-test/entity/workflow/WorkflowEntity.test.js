...
(0, node_test_1.describe)('WorkflowEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['update', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'workflow.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_TEST_WORKFLOW_ENTID JSON to run live');
            return;
        }
        ...
        (0, node_assert_1.default)(workflow_ref01_resdata_up0.id === workflow_ref01_data_up0.id);
        (0, node_assert_1.default)(workflow_ref01_resdata_up0[workflow_ref01_markdef_up0.name] === workflow_ref01_markdef_up0.value);
        // LOAD
...
function basicSetup(extra) {
    ...
    let idmap = transform(['workflow01', 'workflow02', 'workflow03', 'repo01', 'repo02', 'repo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_WORKFLOW_ENTID'];
