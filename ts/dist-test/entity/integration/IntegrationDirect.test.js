...
(0, node_test_1.describe)('IntegrationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-integration', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-integration', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["app_slug01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'repos/{owner}/{repo}/branches/{branch_id}/protection/restrictions/apps',
                method: 'GET',
                params: {
            ...
            const candidateId = listArr[0]?.app_slug ?? listArr[0]?.id;
            if (null == candidateId) {
                return; // skip: list response shape does not expose load identifier
            }
            params.app_slug = candidateId;
            params.app_slug = setup.idmap['app_slug01'];
        }
        else {
            params.app_slug = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITHUB_TEST_INTEGRATION_ENTID'];
