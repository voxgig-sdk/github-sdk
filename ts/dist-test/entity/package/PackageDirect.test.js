...
(0, node_test_1.describe)('PackageDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-package', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-package', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["org01", "package01", "package_name01", "package_version01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'orgs/{org_id}/packages/{package_id}/{package_name}/versions',
                method: 'GET',
                params: {
                    org_id: setup.idmap['org01'],
                    package_id: setup.idmap['package01'],
                    package_name: setup.idmap['package_name01'],
                },
            });
            if (!listResult.ok) {
            ...
            params.package_name = setup.idmap['package_name01'];
            params.package_version_id = setup.idmap['package_version01'];
        }
        else {
            params.org_id = 'direct01';
            params.package_id = 'direct02';
            params.package_name = 'direct03';
            params.package_version_id = 'direct04';
        }
        const result = await client.direct({
            path: 'orgs/{org_id}/packages/{package_id}/{package_name}/versions/{package_version_id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-package', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-package', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["org01", "package01", "package_name01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITHUB_TEST_PACKAGE_ENTID'];
