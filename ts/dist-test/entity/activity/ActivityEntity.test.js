...
(0, node_test_1.describe)('ActivityEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        activity_ref01_match['owner'] = setup.idmap['owner01'];
        activity_ref01_match['repo'] = setup.idmap['repo01'];
        const activity_ref01_list = (await activity_ref01_ent.list(activity_ref01_match)).map((e) => e.data());
        // UPDATE
        const activity_ref01_data_up0 = {};
        activity_ref01_data_up0.id = activity_ref01_data.id;
        const activity_ref01_markdef_up0 = { name: 'activity_type', value: 'Mark01-activity_ref01_' + setup.now };
        activity_ref01_data_up0[activity_ref01_markdef_up0.name] = activity_ref01_markdef_up0.value;
        const activity_ref01_resdata_up0 = (await activity_ref01_ent.update(activity_ref01_data_up0)).data();
        (0, node_assert_1.default)(activity_ref01_resdata_up0.id === activity_ref01_data_up0.id);
        (0, node_assert_1.default)(activity_ref01_resdata_up0[activity_ref01_markdef_up0.name] === activity_ref01_markdef_up0.value);
        // LOAD
...
function basicSetup(extra) {
    ...
    let idmap = transform(['activity01', 'activity02', 'activity03', 'thread01', 'thread02', 'thread03', 'repo01', 'repo02', 'repo03', 'starred01', 'starred02', 'starred03', 'user01', 'user02', 'user03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_ACTIVITY_ENTID'];
    ...
    idmap = env['GITHUB_TEST_ACTIVITY_ENTID'];
