...
(0, node_test_1.describe)('EventEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_TEST_EVENT_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['event01', 'event02', 'event03', 'network01', 'network02', 'network03', 'repo01', 'repo02', 'repo03', 'user01', 'user02', 'user03', 'user01', 'user02', 'user03', 'org01', 'org02', 'org03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_EVENT_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'GITHUB_TEST_EVENT_ENTID': idmap,
        'GITHUB_TEST_LIVE': 'FALSE',
        'GITHUB_TEST_EXPLAIN': 'FALSE',
    });
    idmap = env['GITHUB_TEST_EVENT_ENTID'];
