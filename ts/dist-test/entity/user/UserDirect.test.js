...
(0, node_test_1.describe)('UserDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-user', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-user', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["target_user01", "username01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'orgs/{org_id}/organization-roles/{organization_role_id}/users',
                method: 'GET',
                params: {
                    org_id: setup.idmap['org01'],
                    organization_role_id: setup.idmap['organization_role01'],
                },
            });
            if (!listResult.ok) {
            ...
            const candidateId = listArr[0]?.target_user ?? listArr[0]?.id;
            if (null == candidateId) {
                return; // skip: list response shape does not expose load identifier
            }
            params.target_user = candidateId;
            params.target_user = setup.idmap['target_user01'];
            params.username = setup.idmap['username01'];
        }
        else {
            params.target_user = 'direct01';
    ...
    (0, node_test_1.test)('direct-list-user', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-user', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["org01", "organization_role01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            ...
            params.organization_role_id = setup.idmap['organization_role01'];
        }
        else {
            params.org_id = 'direct01';
            params.organization_role_id = 'direct02';
        }
        const result = await client.direct({
            path: 'orgs/{org_id}/organization-roles/{organization_role_id}/users',
            method: 'GET',
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITHUB_TEST_USER_ENTID'];
