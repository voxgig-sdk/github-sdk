...
(0, node_test_1.describe)('TagEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_TEST_TAG_ENTID JSON to run live');
            return;
        }
        ...
        tag_ref01_match['owner'] = setup.idmap['owner01'];
        tag_ref01_match['repo'] = setup.idmap['repo01'];
...
function basicSetup(extra) {
    ...
    let idmap = transform(['tag01', 'tag02', 'tag03', 'repo01', 'repo02', 'repo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_TAG_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'GITHUB_TEST_TAG_ENTID': idmap,
        'GITHUB_TEST_LIVE': 'FALSE',
        'GITHUB_TEST_EXPLAIN': 'FALSE',
        'GITHUB_APIKEY': '',
    });
    idmap = env['GITHUB_TEST_TAG_ENTID'];
