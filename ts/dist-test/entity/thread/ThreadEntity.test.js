...
(0, node_test_1.describe)('ThreadEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        // LOAD
        const thread_ref01_match_dt0 = {};
        thread_ref01_match_dt0.id = thread_ref01_data.id;
        const thread_ref01_data_dt0 = (await thread_ref01_ent.load(thread_ref01_match_dt0)).data();
        (0, node_assert_1.default)(thread_ref01_data_dt0.id === thread_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    let idmap = transform(['thread01', 'thread02', 'thread03', 'repo01', 'repo02', 'repo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_THREAD_ENTID'];
    ...
    idmap = env['GITHUB_TEST_THREAD_ENTID'];
