...
(0, node_test_1.describe)('MembershipEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'update', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'membership.' + op, live))
                return;
        }
        ...
        (0, node_assert_1.default)(membership_ref01_resdata_up0[membership_ref01_markdef_up0.name] === membership_ref01_markdef_up0.value);
        // LOAD
...
function basicSetup(extra) {
    ...
    let idmap = transform(['membership01', 'membership02', 'membership03', 'enterpris01', 'enterpris02', 'enterpris03', 'team01', 'team02', 'team03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['GITHUB_TEST_MEMBERSHIP_ENTID'];
