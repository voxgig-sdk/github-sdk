...
(0, node_test_1.describe)('GistDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-gist', async (t) => {
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-gist', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["gist01", "sha01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.gist_id ?? listArr[0]?.id;
            if (null == candidateId) {
                return; // skip: list response shape does not expose load identifier
            }
            params.gist_id = candidateId;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITHUB_TEST_GIST_ENTID'];
