...
(0, node_test_1.describe)('IssueDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-issue', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-issue', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["assignee01", "owner01", "repo01"]))
            return;
        ...
        if (setup.live) {
            ...
            params.assignee = setup.idmap['assignee01'];
            params.owner = setup.idmap['owner01'];
            params.repo = setup.idmap['repo01'];
        }
        else {
            params.assignee = 'direct01';
            params.id = 'direct02';
            params.owner = 'direct03';
            params.repo = 'direct04';
        }
        const result = await client.direct({
            path: 'repos/{owner}/{repo}/issues/{id}/assignees/{assignee}',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITHUB_TEST_ISSUE_ENTID'];
