...
describe('RepositoryInvitationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'update']) {
      if (maybeSkipControl(t, 'entityOp', 'repository_invitation.' + op, live)) return
    }
    ...
    let repository_invitation_ref01_data = Object.values(setup.data.existing.repository_invitation)[0] as any

    // LIST
    const repository_invitation_ref01_ent = client.RepositoryInvitation()
    const repository_invitation_ref01_match: any = {}

    const repository_invitation_ref01_list = (await repository_invitation_ref01_ent.list(repository_invitation_ref01_match)).map((e: any) => e.data())


    // UPDATE
    const repository_invitation_ref01_data_up0: any = {}
    ...
    ;(repository_invitation_ref01_data_up0 as any)[repository_invitation_ref01_markdef_up0.name] = repository_invitation_ref01_markdef_up0.value
    ...
    assert((repository_invitation_ref01_resdata_up0 as any)[repository_invitation_ref01_markdef_up0.name] === repository_invitation_ref01_markdef_up0.value)
