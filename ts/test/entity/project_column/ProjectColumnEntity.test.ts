...
describe('ProjectColumnEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'project_column.' + op, live)) return
    }
    ...
    // LIST
    const project_column_ref01_match: any = {}
    project_column_ref01_match['project_id'] = setup.idmap['project01']

    const project_column_ref01_list = (await project_column_ref01_ent.list(project_column_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(project_column_ref01_list, { id: project_column_ref01_data.id })))


    // UPDATE
    const project_column_ref01_data_up0: any = {}
    ...
    ;(project_column_ref01_data_up0 as any)[project_column_ref01_markdef_up0.name] = project_column_ref01_markdef_up0.value
    ...
    assert((project_column_ref01_resdata_up0 as any)[project_column_ref01_markdef_up0.name] === project_column_ref01_markdef_up0.value)


    // LOAD
    const project_column_ref01_match_dt0: any = {}
