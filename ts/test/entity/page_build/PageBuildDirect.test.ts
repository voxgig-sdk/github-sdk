...
describe('PageBuildDirect', async () => {
  ...
  test('direct-load-page_build', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-page_build', setup.live)) return
  ...
  test('direct-list-page_build', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-page_build', setup.live)) return
    if (skipIfMissingIds(t, setup, ["owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_PAGE_BUILD_ENTID']
