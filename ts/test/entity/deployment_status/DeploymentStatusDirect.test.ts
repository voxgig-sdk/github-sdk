...
describe('DeploymentStatusDirect', async () => {
  ...
  test('direct-load-deployment_status', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-deployment_status', setup.live)) return
    if (skipIfMissingIds(t, setup, ["deployment01","owner01","repo01"])) return
  ...
  test('direct-list-deployment_status', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-deployment_status', setup.live)) return
    if (skipIfMissingIds(t, setup, ["deployment_status01","owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_DEPLOYMENT_STATUS_ENTID']
