...
describe('RuleSuiteDirect', async () => {
  ...
  test('direct-load-rule_suite', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-rule_suite', setup.live)) return
  ...
  test('direct-list-rule_suite', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-rule_suite', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_RULE_SUITE_ENTID']
