...
describe('TeamDirect', async () => {
  ...
  test('direct-load-team', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["comment_number01","discussion01","org01","team01"])) return
    ...
    if (setup.live) {
      const listResult: any = await client.direct({
        path: 'orgs/{org_id}/teams/{team_id}/discussions/{discussion_number}/comments',
        method: 'GET',
        params: {
      ...
      const candidateId = listArr[0]?.comment_number ?? listArr[0]?.id
      if (null == candidateId) {
        return // skip: list response shape does not expose load identifier
      }
      params.comment_number = candidateId
    ...
    const result: any = await client.direct({
      path: 'orgs/{org_id}/teams/{team_id}/discussions/{discussion_id}/comments/{comment_number}',
      method: 'GET',
      params,
      query,
    })
  ...
  test('direct-list-team', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["discussion_number01","org01","team01"])) return
    const { client, calls } = setup

    const params: any = {}
    const query: any = {}
    if (setup.live) {
      params.discussion_number = setup.idmap['discussion_number01']
      params.org_id = setup.idmap['org01']
      params.team_id = setup.idmap['team01']
    } else {
      params.discussion_number = 'direct01'
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_TEAM_ENTID']
