...
describe('CampaignDirect', async () => {
  ...
  test('direct-load-campaign', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-campaign', setup.live)) return
  ...
  test('direct-list-campaign', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-campaign', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_CAMPAIGN_ENTID']
