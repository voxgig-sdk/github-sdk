...
describe('CustomPropertyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'custom_property.' + op, live)) return
    }
    ...
    let custom_property_ref01_data = Object.values(setup.data.existing.custom_property)[0] as any

    // LIST
    const custom_property_ref01_ent = client.CustomProperty()
    const custom_property_ref01_match: any = {}
    custom_property_ref01_match['org_id'] = setup.idmap['org01']

    const custom_property_ref01_list = (await custom_property_ref01_ent.list(custom_property_ref01_match)).map((e: any) => e.data())


    // UPDATE
    const custom_property_ref01_data_up0: any = {}
    ...
    ;(custom_property_ref01_data_up0 as any)[custom_property_ref01_markdef_up0.name] = custom_property_ref01_markdef_up0.value
    ...
    assert((custom_property_ref01_resdata_up0 as any)[custom_property_ref01_markdef_up0.name] === custom_property_ref01_markdef_up0.value)
