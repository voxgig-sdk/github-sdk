...
describe('RepositoryRulesetEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'repository_ruleset.' + op, live)) return
    }
    ...
    // LIST
    const repository_ruleset_ref01_match: any = {}
    repository_ruleset_ref01_match['org_id'] = setup.idmap['org01']

    const repository_ruleset_ref01_list = (await repository_ruleset_ref01_ent.list(repository_ruleset_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(repository_ruleset_ref01_list, { id: repository_ruleset_ref01_data.id })))


    // UPDATE
    const repository_ruleset_ref01_data_up0: any = {}
    ...
    ;(repository_ruleset_ref01_data_up0 as any)[repository_ruleset_ref01_markdef_up0.name] = repository_ruleset_ref01_markdef_up0.value
    ...
    assert((repository_ruleset_ref01_resdata_up0 as any)[repository_ruleset_ref01_markdef_up0.name] === repository_ruleset_ref01_markdef_up0.value)


    // LOAD
    const repository_ruleset_ref01_match_dt0: any = {}
