...
describe('GlobalAdvisoryDirect', async () => {
  ...
  test('direct-load-global_advisory', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-global_advisory', setup.live)) return
  ...
  test('direct-list-global_advisory', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-global_advisory', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_GLOBAL_ADVISORY_ENTID']
