...
describe('CombinedBillingUsageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'combined_billing_usage.' + op, live)) return
    }
    ...
    let combined_billing_usage_ref01_data = Object.values(setup.data.existing.combined_billing_usage)[0] as any
