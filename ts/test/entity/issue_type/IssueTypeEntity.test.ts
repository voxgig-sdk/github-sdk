...
describe('IssueTypeEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update']) {
      if (maybeSkipControl(t, 'entityOp', 'issue_type.' + op, live)) return
    }
    ...
    // LIST
    const issue_type_ref01_match: any = {}
    issue_type_ref01_match['org_id'] = setup.idmap['org01']

    const issue_type_ref01_list = (await issue_type_ref01_ent.list(issue_type_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(issue_type_ref01_list, { id: issue_type_ref01_data.id })))


    // UPDATE
    const issue_type_ref01_data_up0: any = {}
    ...
    ;(issue_type_ref01_data_up0 as any)[issue_type_ref01_markdef_up0.name] = issue_type_ref01_markdef_up0.value
    ...
    assert((issue_type_ref01_resdata_up0 as any)[issue_type_ref01_markdef_up0.name] === issue_type_ref01_markdef_up0.value)
