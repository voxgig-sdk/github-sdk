...
describe('MemberEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_TEST_MEMBER_ENTID JSON to run live')
      return
    }
    ...
    member_ref01_match['org_id'] = setup.idmap['org01']
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITHUB_TEST_MEMBER_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'GITHUB_TEST_MEMBER_ENTID': idmap,
    'GITHUB_TEST_LIVE': 'FALSE',
    'GITHUB_TEST_EXPLAIN': 'FALSE',
    'GITHUB_APIKEY': '',
  })

  idmap = env['GITHUB_TEST_MEMBER_ENTID']
