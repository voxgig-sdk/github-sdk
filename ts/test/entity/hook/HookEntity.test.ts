...
describe('HookEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    // LIST
    const hook_ref01_match: any = {}
    hook_ref01_match['owner'] = setup.idmap['owner01']
    hook_ref01_match['repo'] = setup.idmap['repo01']

    const hook_ref01_list = (await hook_ref01_ent.list(hook_ref01_match)).map((e: any) => e.data())
    ...
    ;(hook_ref01_data_up0 as any)[hook_ref01_markdef_up0.name] = hook_ref01_markdef_up0.value
    ...
    assert((hook_ref01_resdata_up0 as any)[hook_ref01_markdef_up0.name] === hook_ref01_markdef_up0.value)


    // LOAD
    const hook_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITHUB_TEST_HOOK_ENTID']
