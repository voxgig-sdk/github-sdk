...
describe('PullRequestReviewCommentDirect', async () => {
  ...
  test('direct-load-pull_request_review_comment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-pull_request_review_comment', setup.live)) return
  ...
  test('direct-list-pull_request_review_comment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-pull_request_review_comment', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_PULL_REQUEST_REVIEW_COMMENT_ENTID']
