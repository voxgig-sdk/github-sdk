...
describe('DependabotAlertDirect', async () => {
  ...
  test('direct-load-dependabot_alert', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-dependabot_alert', setup.live)) return
  ...
  test('direct-list-dependabot_alert', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-dependabot_alert', setup.live)) return
    if (skipIfMissingIds(t, setup, ["owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_DEPENDABOT_ALERT_ENTID']
