...
describe('CopilotEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'load', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'copilot.' + op, live)) return
    }
    ...
    // LIST
    const copilot_ref01_match: any = {}
    copilot_ref01_match['org_id'] = setup.idmap['org01']

    const copilot_ref01_list = (await copilot_ref01_ent.list(copilot_ref01_match)).map((e: any) => e.data())




    // LIST
    const copilot_ref01_match_rt0: any = {}
    copilot_ref01_match_rt0['org_id'] = setup.idmap['org01']

    const copilot_ref01_list_rt0 = (await copilot_ref01_ent.list(copilot_ref01_match_rt0)).map((e: any) => e.data())
