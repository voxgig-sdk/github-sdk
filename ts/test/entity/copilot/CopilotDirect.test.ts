...
describe('CopilotDirect', async () => {
  ...
  test('direct-load-copilot', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-copilot', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01","username01"])) return
  ...
  test('direct-list-copilot', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-copilot', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_COPILOT_ENTID']
