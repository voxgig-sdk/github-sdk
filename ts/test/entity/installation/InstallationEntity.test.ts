...
describe('InstallationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'installation.' + op, live)) return
    }
    ...
    let installation_ref01_data = Object.values(setup.data.existing.installation)[0] as any

    // LIST
    const installation_ref01_ent = client.Installation()
    const installation_ref01_match: any = {}
    installation_ref01_match['username'] = setup.idmap['username01']

    const installation_ref01_list = (await installation_ref01_ent.list(installation_ref01_match)).map((e: any) => e.data())


    // UPDATE
    const installation_ref01_data_up0: any = {}
    ...
    ;(installation_ref01_data_up0 as any)[installation_ref01_markdef_up0.name] = installation_ref01_markdef_up0.value
    ...
    assert((installation_ref01_resdata_up0 as any)[installation_ref01_markdef_up0.name] === installation_ref01_markdef_up0.value)


    // LOAD
    const installation_ref01_match_dt0: any = {}
