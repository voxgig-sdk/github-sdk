...
describe('ArtifactEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'artifact.' + op, live)) return
    }
    ...
    let artifact_ref01_data = Object.values(setup.data.existing.artifact)[0] as any

    // LOAD
    const artifact_ref01_ent = client.Artifact()
    const artifact_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['artifact01','artifact02','artifact03','repo01','repo02','repo03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['GITHUB_TEST_ARTIFACT_ENTID']
  ...
  idmap = env['GITHUB_TEST_ARTIFACT_ENTID']
