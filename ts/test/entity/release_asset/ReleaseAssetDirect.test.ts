...
describe('ReleaseAssetDirect', async () => {
  ...
  test('direct-load-release_asset', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-release_asset', setup.live)) return
  ...
  test('direct-list-release_asset', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-release_asset', setup.live)) return
    if (skipIfMissingIds(t, setup, ["release_asset01","owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_RELEASE_ASSET_ENTID']
