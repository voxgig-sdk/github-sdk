...
describe('CommitCommentDirect', async () => {
  ...
  test('direct-load-commit_comment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-commit_comment', setup.live)) return
  ...
  test('direct-list-commit_comment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-commit_comment', setup.live)) return
    if (skipIfMissingIds(t, setup, ["commit_sha01","owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_COMMIT_COMMENT_ENTID']
