...
describe('LabelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_TEST_LABEL_ENTID JSON to run live')
      return
    }
    ...
    label_ref01_data['milestone_number'] = setup.idmap['milestone_number01']
    label_ref01_data['owner'] = setup.idmap['owner01']
    label_ref01_data['repo'] = setup.idmap['repo01']
    ...
    label_ref01_match['owner'] = setup.idmap['owner01']
    label_ref01_match['repo'] = setup.idmap['repo01']
    ...
    label_ref01_data_up0 ['owner'] = setup.idmap['owner']
    label_ref01_data_up0 ['repo'] = setup.idmap['repo']

    const label_ref01_markdef_up0 = { name: 'color', value: 'Mark01-label_ref01_' + setup.now }
    ;(label_ref01_data_up0 as any)[label_ref01_markdef_up0.name] = label_ref01_markdef_up0.value
    ...
    assert((label_ref01_resdata_up0 as any)[label_ref01_markdef_up0.name] === label_ref01_markdef_up0.value)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['label01','label02','label03','repo01','repo02','repo03','repo01','repo02','repo03','milestone01','milestone02','milestone03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['GITHUB_TEST_LABEL_ENTID']
