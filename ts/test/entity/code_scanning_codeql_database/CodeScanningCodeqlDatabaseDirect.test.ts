...
describe('CodeScanningCodeqlDatabaseDirect', async () => {
  ...
  test('direct-load-code_scanning_codeql_database', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-code_scanning_codeql_database', setup.live)) return
    if (skipIfMissingIds(t, setup, ["language01","owner01","repo01"])) return
  ...
  test('direct-list-code_scanning_codeql_database', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-code_scanning_codeql_database', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_CODE_SCANNING_CODEQL_DATABASE_ENTID']
