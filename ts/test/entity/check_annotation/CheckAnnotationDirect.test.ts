...
describe('CheckAnnotationDirect', async () => {
  ...
  test('direct-list-check_annotation', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-check_annotation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["check_run01","owner01","repo01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_CHECK_ANNOTATION_ENTID']
