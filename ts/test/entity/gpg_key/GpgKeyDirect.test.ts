...
describe('GpgKeyDirect', async () => {
  ...
  test('direct-load-gpg_key', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-gpg_key', setup.live)) return
  ...
  test('direct-list-gpg_key', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-gpg_key', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_GPG_KEY_ENTID']
