...
describe('SshSigningKeyDirect', async () => {
  ...
  test('direct-load-ssh_signing_key', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ssh_signing_key', setup.live)) return
  ...
  test('direct-list-ssh_signing_key', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ssh_signing_key', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_SSH_SIGNING_KEY_ENTID']
