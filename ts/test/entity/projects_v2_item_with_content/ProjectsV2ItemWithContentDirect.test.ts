...
describe('ProjectsV2ItemWithContentDirect', async () => {
  ...
  test('direct-load-projects_v2_item_with_content', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-projects_v2_item_with_content', setup.live)) return
    if (skipIfMissingIds(t, setup, ["item01","org01","projects_v201"])) return
  ...
  test('direct-list-projects_v2_item_with_content', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-projects_v2_item_with_content', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01","project_number01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_PROJECTS_V2_ITEM_WITH_CONTENT_ENTID']
