...
describe('CodeScanningVariantAnalysisRepoTaskDirect', async () => {
  ...
  test('direct-load-code_scanning_variant_analysis_repo_task', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-code_scanning_variant_analysis_repo_task', setup.live)) return
    if (skipIfMissingIds(t, setup, ["codeql_variant_analysis_id01","owner01","repo01","repo_name01","repo_owner01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_CODE_SCANNING_VARIANT_ANALYSIS_REPO_TASK_ENTID']
