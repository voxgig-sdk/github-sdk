...
describe('EnterpriseTeamDirect', async () => {
  ...
  test('direct-load-enterprise_team', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-enterprise_team', setup.live)) return
  ...
  test('direct-list-enterprise_team', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-enterprise_team', setup.live)) return
    if (skipIfMissingIds(t, setup, ["enterprise01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_ENTERPRISE_TEAM_ENTID']
