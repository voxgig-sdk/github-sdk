...
describe('OrgHookEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'org_hook.' + op, live)) return
    }
    ...
    // LIST
    const org_hook_ref01_match: any = {}
    org_hook_ref01_match['org'] = setup.idmap['org01']

    const org_hook_ref01_list = (await org_hook_ref01_ent.list(org_hook_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(org_hook_ref01_list, { id: org_hook_ref01_data.id })))


    // UPDATE
    const org_hook_ref01_data_up0: any = {}
    ...
    ;(org_hook_ref01_data_up0 as any)[org_hook_ref01_markdef_up0.name] = org_hook_ref01_markdef_up0.value
    ...
    assert((org_hook_ref01_resdata_up0 as any)[org_hook_ref01_markdef_up0.name] === org_hook_ref01_markdef_up0.value)


    // LOAD
    const org_hook_ref01_match_dt0: any = {}
