...
describe('DeployKeyDirect', async () => {
  ...
  test('direct-load-deploy_key', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-deploy_key', setup.live)) return
  ...
  test('direct-list-deploy_key', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-deploy_key', setup.live)) return
    if (skipIfMissingIds(t, setup, ["owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_DEPLOY_KEY_ENTID']
