...
describe('CodespaceDirect', async () => {
  ...
  test('direct-load-codespace', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-codespace', setup.live)) return
  ...
  test('direct-list-codespace', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-codespace', setup.live)) return
    if (skipIfMissingIds(t, setup, ["owner01","repo01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_CODESPACE_ENTID']
