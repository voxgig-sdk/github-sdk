...
describe('RepositoryAdvisoryEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'repository_advisory.' + op, live)) return
    }
    ...
    // LIST
    const repository_advisory_ref01_match: any = {}
    repository_advisory_ref01_match['org_id'] = setup.idmap['org01']

    const repository_advisory_ref01_list = (await repository_advisory_ref01_ent.list(repository_advisory_ref01_match)).map((e: any) => e.data())


    // UPDATE
    const repository_advisory_ref01_data_up0: any = {}
    ...
    ;(repository_advisory_ref01_data_up0 as any)[repository_advisory_ref01_markdef_up0.name] = repository_advisory_ref01_markdef_up0.value
    ...
    assert((repository_advisory_ref01_resdata_up0 as any)[repository_advisory_ref01_markdef_up0.name] === repository_advisory_ref01_markdef_up0.value)
