...
describe('RepositoryAdvisoryDirect', async () => {
  ...
  test('direct-load-repository_advisory', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-repository_advisory', setup.live)) return
    if (skipIfMissingIds(t, setup, ["ghsa01","owner01","repo01"])) return
  ...
  test('direct-list-repository_advisory', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-repository_advisory', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITHUB_TEST_REPOSITORY_ADVISORY_ENTID']
