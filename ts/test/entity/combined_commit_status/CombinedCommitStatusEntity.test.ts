...
describe('CombinedCommitStatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'combined_commit_status.' + op, live)) return
    }
    ...
    let combined_commit_status_ref01_data = Object.values(setup.data.existing.combined_commit_status)[0] as any

    // LIST
    const combined_commit_status_ref01_ent = client.CombinedCommitStatus()
    const combined_commit_status_ref01_match: any = {}
    ...
    const combined_commit_status_ref01_list = (await combined_commit_status_ref01_ent.list(combined_commit_status_ref01_match)).map((e: any) => e.data())
