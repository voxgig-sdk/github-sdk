...
| `actor` | `Object` | Yes | A GitHub user. |
...
| `clone_url` | `string` | No |  |
| `code_of_conduct` | `Object` | Yes | Code Of Conduct |
...
| `head_commit` | `Object` | Yes | A commit. |
| `head_repository` | `Object` | Yes | Minimal Repository |
...
| `image_details` | `Object` | Yes | Provides details of a hosted runner image |
...
| `license` | `Object` | No | License Simple |
| `logs_url` | `string` | Yes | The URL to download the logs for the workflow run. |
| `machine_size_details` | `Object` | Yes | Provides details of a particular machine spec. |
...
| `repository` | `Object` | Yes | Minimal Repository |
...
| `triggering_actor` | `Object` | Yes | A GitHub user. |
...
```ts
const results = await client.ActionsCacheList().list()
```
...
| `image` | `Object` | Yes | The image of runner. |
| `image_details` | `Object` | Yes | Provides details of a hosted runner image |
| `last_active_on` | `string` | No | The time at which the runner was last used, in ISO 8601 format. |
| `machine_size_details` | `Object` | Yes | Provides details of a particular machine spec. |
| `maximum_runners` | `number` | No | The maximum amount of hosted runners. |
...
```ts
const results = await client.ApiInsightsRouteStat().list()
```
...
```ts
const results = await client.ApiInsightsSubjectStat().list()
```
...
```ts
const results = await client.ApiInsightsTimeStat().list()
```
...
| `suspended_by` | `Object` | Yes | A GitHub user. |
| `svn_url` | `string` | Yes |  |
| `tags_url` | `string` | Yes |  |
...
```ts
const results = await client.Assignee().list()
```
...
```ts
const results = await client.Autolink().list()
```
...
```ts
const results = await client.BillingUsageReport().list()
```
...
```ts
const results = await client.BillingUsageReportUser().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `commit` | `Object` | Yes | Commit |
| `id` | `string` | No |  |
| `links` | `Object` | Yes |  |
| `name` | `string` | Yes |  |
| `pattern` | `string` | No |  |
| `protected` | `boolean` | Yes |  |
| `protection` | `Object` | Yes | Branch Protection |
...
| `allow_fork_syncing` | `Object` | No | Whether users can pull changes from upstream when the branch is locked. |
| `block_creations` | `Object` | No |  |
| `enabled` | `boolean` | No |  |
| `enforce_admins` | `Object` | Yes | Protected Branch Admin Enforced |
| `id` | `string` | No |  |
| `lock_branch` | `Object` | No | Whether to set the branch as read-only. |
| `name` | `string` | No |  |
| `protection_url` | `string` | No |  |
| `required_conversation_resolution` | `Object` | No |  |
| `required_linear_history` | `Object` | No |  |
| `required_pull_request_reviews` | `Object` | Yes | Protected Branch Pull Request Review |
| `required_signatures` | `Object` | Yes |  |
| `required_status_checks` | `Object` | Yes | Protected Branch Required Status Check |
| `restrictions` | `Object` | Yes | Branch Restriction Policy |
| `url` | `string` | No |  |
...
```ts
const results = await client.BranchRestrictionPolicy().list()
```
...
```ts
const results = await client.BranchShort().list()
```
...
| --- | --- | --- | --- |
| `after` | `string` | Yes |  |
| `app` | `Object` | Yes | GitHub apps are a new way to extend GitHub. |
| `before` | `string` | Yes |  |
...
| `created_at` | `string` | Yes |  |
| `deployment` | `Object` | Yes | A deployment created as the result of an Actions check run from a workflow that references an environment |
...
```ts
const results = await client.CheckAnnotation().list()
```
...
| `name` | `string` | Yes | The name of the check. |
| `node_id` | `string` | Yes |  |
| `output` | `Object` | Yes | Check runs can accept a variety of data in the `output` object, including a `title` and `summary` and can optionally provide descriptive details about the run. |
| `pull_requests` | `Array` | Yes | Pull requests that are open with a `head_sha` or `head_branch` that matches the check. |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `assignment` | `Object` | Yes | A GitHub Classroom assignment |
...
| `repository` | `Object` | Yes | A GitHub repository view for Classroom |
| `students` | `Array` | Yes |  |
...
```ts
const results = await client.ClassroomAcceptedAssignment().list()
```
...
| `classroom` | `Object` | Yes | A GitHub Classroom classroom |
...
| `starter_code_repository` | `Object` | Yes | A GitHub repository view for Classroom |
...
```ts
const results = await client.ClassroomAssignmentGrade().list()
```
...
```ts
const results = await client.Clone().list()
```
...
```ts
const results = await client.CodeFrequency().list()
```
...
```ts
const results = await client.CodeFrequencyStat().list()
```
...
| `most_recent_instance` | `Object` | Yes |  |
...
| `html_url` | `string` | No |  |
| `location` | `Object` | No | Describe a region within a file for the alert. |
| `message` | `Object` | No |  |
...
```ts
const results = await client.CodeScanningAlertInstance().list()
```
...
```ts
const results = await client.CodeScanningAlertItem().list()
```
...
```ts
const results = await client.CodeScanningAnalysi().list()
```
...
| `uploader` | `Object` | Yes | A GitHub user. |
...
```ts
const results = await client.CodeScanningCodeqlDatabase().list()
```
...
```ts
const results = await client.CodeScanningDefaultSetup().list()
```
...
| `most_recent_instance` | `Object` | Yes |  |
| `number` | `number` | Yes | The security alert number. |
| `repository` | `Object` | Yes | A GitHub repository. |
| `rule` | `Object` | Yes |  |
| `state` | `string` | Yes | State of a code scanning alert. |
| `tool` | `Object` | Yes |  |
| `updated_at` | `string` | No | The time that the alert was last updated in ISO 8601 format: `YYYY-MM-DDTHH:MM:SSZ`. |
...
```ts
const results = await client.CodeScanningOrganizationAlertItem().list()
```
...
| `controller_repo` | `Object` | Yes | A GitHub repository. |
...
| `skipped_repositories` | `Object` | Yes | Information about repositories that were skipped from processing. |
| `status` | `string` | Yes |  |
...
```ts
const results = await client.CodeSecurityConfiguration().list()
```
...
```ts
const results = await client.CodeSecurityConfigurationRepository().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `configuration` | `Object` | No | A code security configuration |
| `default_for_new_repos` | `*` | No | The visibility of newly created repositories for which the code security configuration will be applied to by default |
...
```ts
const results = await client.CodeSecurityDefaultConfiguration().list()
```
...
```ts
const results = await client.CodeownersError().list()
```
...
| `archived` | `boolean` | No |  |
| `assignees_url` | `string` | Yes |  |
| `billable_owner` | `Object` | Yes | A GitHub user. |
| `blobs_url` | `string` | Yes |  |
...
| `machine` | `Object` | Yes | A description of the machine powering a codespace. |
...
```ts
const results = await client.Collaborator().list()
```
...
```ts
const results = await client.CombinedCommitStatus().list()
```
...
```ts
const results = await client.CommitActivity().list()
```
...
```ts
const results = await client.CommitComment().list()
```
...
| --- | --- | --- | --- |
| `ahead_by` | `number` | Yes |  |
| `base_commit` | `Object` | Yes | Commit |
| `behind_by` | `number` | Yes |  |
| `commits` | `Array` | Yes |  |
| `diff_url` | `string` | Yes |  |
| `files` | `Array` | No |  |
| `html_url` | `string` | Yes |  |
| `merge_base_commit` | `Object` | Yes | Commit |
...
| `code_of_conduct_file` | `Object` | Yes |  |
| `contributing` | `Object` | Yes |  |
| `issue_template` | `Object` | Yes |  |
| `license` | `Object` | Yes | License Simple |
| `pull_request_template` | `Object` | Yes |  |
| `readme` | `Object` | Yes |  |
...
```ts
const results = await client.ContentTraffic().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `author` | `Object` | Yes | A GitHub user. |
| `avatar_url` | `string` | No |  |
...
```ts
const results = await client.Contributor().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `assignee` | `Object` | Yes | A GitHub user. |
| `assigning_team` | `*` | No | The team through which the assignee is granted access to GitHub Copilot, if applicable. |
...
| `organization` | `Object` | Yes | A GitHub organization. |
...
```ts
const results = await client.Copilot().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `copilot_dotcom_chat` | `Object` | No | Usage metrics for Copilot Chat in GitHub.com |
| `copilot_dotcom_pull_requests` | `Object` | No | Usage metrics for Copilot for pull requests. |
| `copilot_ide_chat` | `Object` | No | Usage metrics for Copilot Chat in the IDE. |
| `copilot_ide_code_completions` | `Object` | No | Usage metrics for Copilot editor code completions in the IDE. |
...
```ts
const results = await client.CopilotUsageMetricsDay().list()
```
...
| `default_value` | `*` | No | Default value of the property |
...
```ts
const results = await client.CustomProperty().list()
```
...
| --- | --- | --- | --- |
| `property_name` | `string` | Yes | The name of the property |
| `value` | `*` | Yes | The value assigned to the property |
...
```ts
const results = await client.CustomPropertyValue().list()
```
...
| `id` | `string` | No |  |
| `number` | `number` | Yes | The security alert number. |
| `security_advisory` | `Object` | Yes | Details for the GitHub Security Advisory. |
| `security_vulnerability` | `Object` | Yes | Details pertaining to one vulnerable version range for the advisory. |
...
```ts
const results = await client.DependabotAlert().list()
```
...
```ts
const results = await client.DependabotAlertWithRepository().list()
```
...
```ts
const results = await client.DependabotRepositoryAccessDetail().list()
```
...
| `metadata` | `Object` | No | User-defined metadata to store domain-specific information limited to 8 keys with scalar values. |
...
```ts
const results = await client.DeployKey().list()
```
...
| `created_at` | `string` | Yes |  |
| `creator` | `Object` | Yes | A GitHub user. |
...
| `payload` | `*` | Yes |  |
| `performed_via_github_app` | `Object` | Yes | GitHub apps are a new way to extend GitHub. |
...
```ts
const results = await client.DeploymentStatus().list()
```
...
```ts
const results = await client.DiffEntry().list()
```
...
```ts
const results = await client.EnterpriseTeam().list()
```
...
| `deployment_branch_policy` | `Object` | Yes | The type of deployment branch policy for this environment. |
| `html_url` | `string` | Yes |  |
...
```ts
const results = await client.EnvironmentApproval().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `actor` | `Object` | Yes | Actor |
| `created_at` | `string` | Yes |  |
| `id` | `string` | Yes |  |
| `org` | `Object` | Yes | Actor |
| `payload` | `Object` | Yes |  |
| `public` | `boolean` | Yes |  |
| `repo` | `Object` | Yes |  |
| `type` | `string` | Yes |  |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `author` | `Object` | Yes | The author of the file. |
| `branch` | `string` | No | The branch name. |
| `commit` | `Object` | Yes |  |
| `committer` | `Object` | Yes | The person that committed the file. |
| `content` | `Object` | Yes | The new file content, using Base64 encoding. |
...
| `files` | `Object` | No | Names and content for the files that make up the gist |
| `fork_of` | `Object` | Yes | Gist |
| `forks` | `Array` | No |  |
| `forks_url` | `string` | No |  |
| `git_pull_url` | `string` | No |  |
| `git_push_url` | `string` | No |  |
...
```ts
const results = await client.GistComment().list()
```
...
```ts
const results = await client.GistCommit().list()
```
...
```ts
const results = await client.GistSimple().list()
```
...
| `tree` | `Object` | Yes | The SHA of the tree object this commit points to |
| `url` | `string` | Yes |  |
...
| `node_id` | `string` | Yes |  |
| `object` | `Object` | Yes | The SHA of the git object this is tagging. |
| `sha` | `string` | Yes |  |
| `tag` | `string` | Yes | Name of the tag |
| `tagger` | `Object` | Yes | An object with information about the individual creating the tag. |
...
| `cvss` | `Object` | Yes |  |
| `cvss_severities` | `Object` | No |  |
| `cwes` | `Array` | Yes |  |
| `description` | `string` | Yes | A detailed description of what the advisory entails. |
| `epss` | `Object` | No | The EPSS scores as calculated by the [Exploit Prediction Scoring System](https://www.first.org/epss). |
...
| `config` | `Object` | Yes | Configuration object of the webhook |
| `created_at` | `string` | Yes |  |
...
```ts
const results = await client.Hook().list()
```
...
```ts
const results = await client.HostedCompute().list()
```
...
```ts
const results = await client.Hovercard().list()
```
...
```ts
const results = await client.Import().list()
```
...
| `permissions` | `Object` | Yes | The permissions granted to the user access token. |
| `repositories_url` | `string` | Yes |  |
| `repository_selection` | `string` | Yes | Describe whether all repositories have been selected or there's a selection involved |
| `single_file_name` | `string` | Yes |  |
| `single_file_paths` | `Array` | No |  |
...
| `node_id` | `string` | No |  |
| `requester` | `Object` | Yes | A GitHub user. |
...
| `assigner` | `Object` | Yes | A GitHub user. |
| `author_association` | `string` | Yes | How the author is associated with the repository. |
...
| `closed_by` | `Object` | Yes | A GitHub user. |
...
| `issue` | `Object` | Yes | Issues are a great way to keep track of tasks, enhancements, and bugs for your projects. |
| `issue_dependencies_summary` | `Object` | Yes |  |
| `issue_field_values` | `Array` | No |  |
| `issue_id` | `number` | Yes | The id of the issue that blocks the current issue |
| `issue_url` | `string` | Yes |  |
| `label` | `Object` | Yes | Issue Event Label |
...
| `locked` | `boolean` | Yes |  |
| `milestone` | `Object` | Yes | Issue Event Milestone |
...
| `performed_via_github_app` | `Object` | Yes | GitHub apps are a new way to extend GitHub. |
| `project_card` | `Object` | Yes | Issue Event Project Card |
| `pull_request` | `Object` | Yes |  |
| `reactions` | `Object` | Yes |  |
| `rename` | `Object` | Yes | Issue Event Rename |
| `replace_parent` | `boolean` | No | Option that, when true, instructs the operation to replace the sub-issues current parent issue |
| `repository` | `Object` | Yes | A repository on GitHub. |
| `repository_url` | `string` | Yes |  |
| `requested_reviewer` | `Object` | Yes | A GitHub user. |
| `requested_team` | `Object` | Yes | Groups of organization members that gives permissions on specified repositories. |
| `review_requester` | `Object` | Yes | A GitHub user. |
...
| `sub_issues_summary` | `Object` | Yes |  |
| `timeline_url` | `string` | No |  |
| `title` | `string` | Yes | Title of the issue |
| `type` | `Object` | Yes | The type of issue. |
| `updated_at` | `string` | Yes |  |
...
```ts
const results = await client.IssueType().list()
```
...
```ts
const results = await client.MarketplacePurchase().list()
```
...
```ts
const results = await client.OidcCustomSub().list()
```
...
```ts
const results = await client.OidcCustomSubRepo().list()
```
...
| `attestations` | `Array` | No |  |
| `attestations_subject_digests` | `Object` | No | Mapping of subject digest to bundles. |
| `avatar_url` | `string` | Yes |  |
...
| `organization` | `Object` | Yes | A GitHub organization. |
| `organization_url` | `string` | Yes |  |
| `page_info` | `Object` | No | Information about the current page. |
...
| `config` | `Object` | Yes | Key/value pairs to provide settings for this webhook. |
| `created_at` | `string` | Yes |  |
| `deliveries_url` | `string` | No |  |
...
```ts
const results = await client.OrgHook().list()
```
...
```ts
const results = await client.OrgRepoCustomPropertyValue().list()
```
...
| `inviter` | `Object` | Yes | A GitHub user. |
| `login` | `string` | Yes |  |
| `node_id` | `string` | Yes |  |
...
```ts
const results = await client.OrganizationInvitation().list()
```
...
```ts
const results = await client.OrganizationProgrammaticAccessGrant().list()
```
...
| `first_location_detected` | `*` | No | Details on the location where the token was initially detected. |
...
| `push_protection_bypass_request_reviewer` | `Object` | Yes | A GitHub user. |
...
| `push_protection_bypassed_by` | `Object` | Yes | A GitHub user. |
| `repository` | `Object` | Yes | A GitHub repository. |
...
| `resolved_by` | `Object` | Yes | A GitHub user. |
...
```ts
const results = await client.OrganizationSecretScanningAlert().list()
```
...
```ts
const results = await client.OutsideCollaborator().list()
```
...
| `source` | `Object` | Yes | The source branch and directory used to publish your Pages site. |
...
| `error` | `Object` | Yes |  |
| `id` | `string` | No |  |
| `pusher` | `Object` | Yes | A GitHub user. |
| `status` | `string` | Yes |  |
| `updated_at` | `string` | Yes |  |
| `url` | `string` | Yes |  |
...
```ts
const results = await client.PageBuild().list()
```
...
```ts
const results = await client.Participation().list()
```
...
```ts
const results = await client.PendingDeployment().list()
```
...
```ts
const results = await client.PorterAuthor().list()
```
...
```ts
const results = await client.PorterLargeFile().list()
```
...
```ts
const results = await client.PrivateRegistry().list()
```
...
```ts
const results = await client.ProjectColumn().list()
```
...
| `deleted_by` | `Object` | Yes | A GitHub user. |
...
| `latest_status_update` | `Object` | Yes | An status update belonging to a project |
...
```ts
const results = await client.ProjectsV2().list()
```
...
```ts
const results = await client.ProjectsV2Field().list()
```
...
```ts
const results = await client.ProjectsV2ItemWithContent().list()
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `allow_deletions` | `Object` | Yes | Allows deletion of the protected branch by anyone with write access to the repository. |
| `allow_force_pushes` | `Object` | Yes | Permits force pushes to the protected branch by anyone with write access to the repository. |
| `allow_fork_syncing` | `Object` | No | Whether users can pull changes from upstream when the branch is locked. |
| `block_creations` | `Object` | Yes | If set to `true`, the `restrictions` branch protection settings which limits who can push will also block pushes which create new branches, unless the push is initiated by a user, team, or app which has the ability to push. |
| `enforce_admins` | `Object` | Yes | Enforce all configured restrictions for administrators. |
| `lock_branch` | `Object` | No | Whether to set the branch as read-only. |
| `required_conversation_resolution` | `Object` | No | Requires all conversations on code to be resolved before a pull request can be merged into a branch that matches this rule. |
| `required_linear_history` | `Object` | Yes | Enforces a linear commit Git history, which prevents anyone from pushing merge commits to a branch. |
| `required_pull_request_reviews` | `Object` | Yes | Require at least one approving review on a pull request, before merging. |
| `required_signatures` | `Object` | Yes |  |
| `required_status_checks` | `Object` | Yes | Status Check Policy |
| `restrictions` | `Object` | Yes | Branch Restriction Policy |
| `url` | `string` | Yes |  |
...
| `dismissal_restrictions` | `Object` | No | Specify which users, teams, and apps can dismiss pull request reviews. |
...
```ts
const results = await client.PublicMember().list()
```
...
| `base` | `Object` | Yes | The name of the branch you want the changes pulled into. |
...
| `head` | `Object` | Yes | The name of the branch where your changes are implemented. |
...
| `merged_by` | `Object` | Yes | A GitHub user. |
| `message` | `string` | Yes |  |
...
```ts
const results = await client.PullRequestReview().list()
```
...
```ts
const results = await client.PullRequestReviewComment().list()
```
...
```ts
const results = await client.Referrer().list()
```
...
| `type` | `string` | Yes |  |
| `updated_at` | `string` | No |  |
| `uploader` | `Object` | No | A GitHub user. |
| `url` | `string` | Yes |  |
| `user_view_type` | `string` | No |  |
...
```ts
const results = await client.ReleaseAsset().list()
```
...
| `allow_update_branch` | `boolean` | No | Whether or not a pull request head branch that is behind its base branch can always be updated even if it is not required to be up to date before merging. |
| `anonymous_access_enabled` | `boolean` | No | Whether anonymous git access is enabled for this repository |
| `app` | `Object` | Yes | A GitHub App that is providing a custom deployment protection rule. |
| `archive_url` | `string` | Yes |  |
| `archived` | `boolean` | Yes | Whether the repository is archived. |
| `assignees_url` | `string` | Yes |  |
| `attestations` | `Array` | No |  |
...
| `bundle` | `Object` | Yes | The attestation's Sigstore Bundle. |
| `client_payload` | `Object` | No | JSON payload with extra information about the webhook event that your action or workflow may use. |
| `clone_url` | `string` | Yes |  |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `author` | `*` | Yes | The author of the advisory. |
...
| `html_url` | `string` | Yes | The URL for the advisory. |
| `identifiers` | `Array` | Yes |  |
| `private_fork` | `*` | Yes | A temporary private fork of the advisory's repository for collaborating on a fix. |
| `published_at` | `string` | Yes | The date and time of when the advisory was published, in ISO 8601 format. |
| `publisher` | `*` | Yes | The publisher of the advisory. |
| `severity` | `string` | Yes | The severity of the advisory. |
...
```ts
const results = await client.RepositoryAdvisory().list()
```
...
| `invitee` | `Object` | Yes | A GitHub user. |
| `inviter` | `Object` | Yes | A GitHub user. |
| `node_id` | `string` | Yes |  |
| `permission` | `string` | No | The permission to grant the collaborator. |
...
| `conditions` | `*` | No | Parameters for a repository ruleset ref name condition |
| `created_at` | `string` | No |  |
...
```ts
const results = await client.RepositoryRuleset().list()
```
...
```ts
const results = await client.ReviewComment().list()
```
...
```ts
const results = await client.RuleSuite().list()
```
...
```ts
const results = await client.RulesetVersion().list()
```
...
| --- | --- | --- | --- |
| `actor` | `Object` | Yes | The actor who updated the ruleset |
| `state` | `Object` | Yes | The state of the ruleset version |
| `updated_at` | `string` | Yes |  |
| `version_id` | `number` | Yes | The ID of the previous version of the ruleset |
...
```ts
const results = await client.RunnerApplication().list()
```
...
```ts
const results = await client.SecretScanningAlert().list()
```
...
```ts
const results = await client.SecretScanningLocation().list()
```
...
```ts
const results = await client.SecretScanningPatternConfiguration().list()
```
...
```ts
const results = await client.SecretScanningScanHistory().list()
```
...
```ts
const results = await client.SelectedAction().list()
```
...
| `name` | `string` | Yes |  |
| `protected` | `boolean` | Yes |  |
| `protection` | `Object` | No | Branch Protection |
| `protection_url` | `string` | No |  |
...
```ts
const results = await client.ShortBranch().list()
```
...
```ts
const results = await client.SimpleClassroomAssignment().list()
```
...
```ts
const results = await client.Status().list()
```
...
```ts
const results = await client.StatusCheckPolicy().list()
```
...
```ts
const results = await client.TagProtection().list()
```
...
| `open_issues` | `number` | Yes |  |
| `open_issues_count` | `number` | Yes |  |
| `organization` | `Object` | Yes | Team Organization |
| `organization_permission` | `string` | No | The organization permission for this project. |
| `organizations_url` | `string` | Yes |  |
| `owner` | `Object` | Yes | A GitHub user. |
| `owner_url` | `string` | Yes |  |
| `parent` | `Object` | Yes | Groups of organization members that gives permissions on specified repositories. |
...
```ts
const results = await client.TeamSimple().list()
```
...
| `plan` | `Object` | Yes | Marketplace Listing Plan |
...
```ts
const results = await client.View().list()
```
