...
class SubscriberEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_SUBSCRIBER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $subscriber_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.subscriber")));
        $subscriber_ref01_data = null;
        if (count($subscriber_ref01_data_raw) > 0) {
            $subscriber_ref01_data = Helpers::to_map($subscriber_ref01_data_raw[0][1]);
        }
...
function subscriber_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["subscriber01", "subscriber02", "subscriber03", "repo01", "repo02", "repo03", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_SUBSCRIBER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GITHUB_TEST_SUBSCRIBER_ENTID"]);
