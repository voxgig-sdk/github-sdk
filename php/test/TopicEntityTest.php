...
class TopicEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_TOPIC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function topic_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["topic01", "topic02", "topic03", "repo01", "repo02", "repo03", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_TOPIC_ENTID");
