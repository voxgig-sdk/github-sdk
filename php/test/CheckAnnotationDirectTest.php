...
class CheckAnnotationDirectTest extends TestCase
{
    public function test_direct_list_check_annotation(): void
    {
        $setup = check_annotation_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-check_annotation", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["check_run01", "owner01", "repo01"] as $_liveKey) {
