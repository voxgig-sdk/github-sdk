...
class HookEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Hook(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Hook(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_HOOK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $hook_ref01_list_result = $hook_ref01_ent->list($hook_ref01_match, null);
        $this->assertIsArray($hook_ref01_list_result);
        ...
        $hook_ref01_markdef_up0_name = "created_at";
        $hook_ref01_markdef_up0_value = "Mark01-hook_ref01_" . $setup["now"];
        ...
        $this->assertEquals($hook_ref01_resdata_up0[$hook_ref01_markdef_up0_name], $hook_ref01_markdef_up0_value);

        // LOAD
        $hook_ref01_match_dt0 = [
            "id" => $hook_ref01_data["id"],
        ];
        $hook_ref01_data_dt0_loaded = $hook_ref01_ent->load($hook_ref01_match_dt0, null);
        $hook_ref01_data_dt0_load_result = Helpers::to_map(is_object($hook_ref01_data_dt0_loaded) && method_exists($hook_ref01_data_dt0_loaded, 'data_get') ? $hook_ref01_data_dt0_loaded->data_get() : $hook_ref01_data_dt0_loaded);
        $this->assertNotNull($hook_ref01_data_dt0_load_result);
        $this->assertEquals($hook_ref01_data_dt0_load_result["id"], $hook_ref01_data["id"]);
...
function hook_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["hook01", "hook02", "hook03", "repo01", "repo02", "repo03", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_HOOK_ENTID");
