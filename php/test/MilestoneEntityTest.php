...
class MilestoneEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_MILESTONE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function milestone_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["milestone01", "milestone02", "milestone03", "repo01", "repo02", "repo03", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_MILESTONE_ENTID");
