...
class EmailEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_EMAIL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        // UPDATE
        $email_ref01_data_up0_up = [
        ];

        $email_ref01_markdef_up0_name = "email";
        $email_ref01_markdef_up0_value = "Mark01-email_ref01_" . $setup["now"];
        $email_ref01_data_up0_up[$email_ref01_markdef_up0_name] = $email_ref01_markdef_up0_value;

        $email_ref01_resdata_up0_result = $email_ref01_ent->update($email_ref01_data_up0_up, null);
        $email_ref01_resdata_up0 = Helpers::to_map(is_object($email_ref01_resdata_up0_result) && method_exists($email_ref01_resdata_up0_result, 'data_get') ? $email_ref01_resdata_up0_result->data_get() : $email_ref01_resdata_up0_result);
        $this->assertNotNull($email_ref01_resdata_up0);
        $this->assertEquals($email_ref01_resdata_up0[$email_ref01_markdef_up0_name], $email_ref01_markdef_up0_value);
...
function email_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITHUB_TEST_EMAIL_ENTID");
