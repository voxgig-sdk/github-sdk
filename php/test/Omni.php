...
//    only - a typed GithubContext there would make every ctx
...
final class GithubOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GithubContext($clean, null);
    ...
    public static function err_from_map($m): ?GithubError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GithubError(is_string($code) ? $code : '', $msg);
    }
