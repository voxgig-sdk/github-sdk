...
class ProjectsV2ItemWithContentDirectTest extends TestCase
{
    public function test_direct_list_projects_v2_item_with_content(): void
    {
        $setup = projects_v2_item_with_content_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-projects_v2_item_with_content", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["org01", "project_number01"] as $_liveKey) {
    ...
    public function test_direct_load_projects_v2_item_with_content(): void
    {
        $setup = projects_v2_item_with_content_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-projects_v2_item_with_content", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
