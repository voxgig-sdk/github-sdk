...
class PullEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_PULL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        // UPDATE
        $pull_ref01_data_up0_up = [
            "id" => $pull_ref01_data["id"],
            "repo_id" => $setup["idmap"]["repo_id"],
        ];

        $pull_ref01_markdef_up0_name = "title";
        $pull_ref01_markdef_up0_value = "Mark01-pull_ref01_" . $setup["now"];
        $pull_ref01_data_up0_up[$pull_ref01_markdef_up0_name] = $pull_ref01_markdef_up0_value;

        $pull_ref01_resdata_up0_result = $pull_ref01_ent->update($pull_ref01_data_up0_up, null);
        $pull_ref01_resdata_up0 = Helpers::to_map(is_object($pull_ref01_resdata_up0_result) && method_exists($pull_ref01_resdata_up0_result, 'data_get') ? $pull_ref01_resdata_up0_result->data_get() : $pull_ref01_resdata_up0_result);
        $this->assertNotNull($pull_ref01_resdata_up0);
        $this->assertEquals($pull_ref01_resdata_up0["id"], $pull_ref01_data_up0_up["id"]);
        $this->assertEquals($pull_ref01_resdata_up0[$pull_ref01_markdef_up0_name], $pull_ref01_markdef_up0_value);
...
function pull_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["pull01", "pull02", "pull03", "repo01", "repo02", "repo03"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_PULL_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GITHUB_TEST_PULL_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }
    if (!isset($idmap_resolved["repo_id"])) {
        $idmap_resolved["repo_id"] = $idmap_resolved["repo01"];
    }
