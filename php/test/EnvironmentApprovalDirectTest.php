...
class EnvironmentApprovalDirectTest extends TestCase
{
    public function test_direct_list_environment_approval(): void
    {
        $setup = environment_approval_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-environment_approval", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["owner01", "repo01", "run01"] as $_liveKey) {
