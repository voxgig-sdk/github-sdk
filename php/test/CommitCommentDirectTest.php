...
class CommitCommentDirectTest extends TestCase
{
    public function test_direct_list_commit_comment(): void
    {
        $setup = commit_comment_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-commit_comment", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["commit_sha01", "owner01", "repo01"] as $_liveKey) {
    ...
    public function test_direct_load_commit_comment(): void
    {
        $setup = commit_comment_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-commit_comment", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
