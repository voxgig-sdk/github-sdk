...
class CodeScanningAnalysiDirectTest extends TestCase
{
    public function test_direct_list_code_scanning_analysi(): void
    {
        $setup = code_scanning_analysi_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-code_scanning_analysi", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_code_scanning_analysi(): void
    {
        $setup = code_scanning_analysi_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-code_scanning_analysi", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
