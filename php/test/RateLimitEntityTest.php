...
class RateLimitEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_RATE_LIMIT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function rate_limit_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITHUB_TEST_RATE_LIMIT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GITHUB_TEST_RATE_LIMIT_ENTID"]);
