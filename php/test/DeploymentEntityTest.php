...
class DeploymentEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_DEPLOYMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function deployment_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["deployment01", "deployment02", "deployment03", "repo01", "repo02", "repo03", "run01", "run02", "run03", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_DEPLOYMENT_ENTID");
