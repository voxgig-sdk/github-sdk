...
class EventEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_EVENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function event_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["event01", "event02", "event03", "network01", "network02", "network03", "repo01", "repo02", "repo03", "user01", "user02", "user03", "org01", "org02", "org03", "username01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_EVENT_ENTID");
