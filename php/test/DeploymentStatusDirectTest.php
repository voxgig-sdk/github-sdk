...
class DeploymentStatusDirectTest extends TestCase
{
    public function test_direct_list_deployment_status(): void
    {
        $setup = deployment_status_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-deployment_status", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["deployment_status01", "owner01", "repo01"] as $_liveKey) {
    ...
    public function test_direct_load_deployment_status(): void
    {
        $setup = deployment_status_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-deployment_status", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
