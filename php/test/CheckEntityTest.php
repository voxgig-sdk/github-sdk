...
class CheckEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_CHECK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function check_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["check01", "check02", "check03", "repo01", "repo02", "repo03", "check_suite01", "check_suite02", "check_suite03", "commit01", "commit02", "commit03", "owner01", "ref01"] as $k) {
    ...
    $entid_env_raw = getenv("GITHUB_TEST_CHECK_ENTID");
