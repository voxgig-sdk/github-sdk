...
class GithubMakeError
{
    public static function call(?GithubContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GithubContext([], null);
        }
        $op = $ctx->op ?? new GithubOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GithubResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GithubError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GithubError('', $msg, $ctx);
