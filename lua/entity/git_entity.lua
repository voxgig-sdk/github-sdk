...
local GitEntity = {}
GitEntity.__index = GitEntity


function GitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitEntity)
  self._name = "git"
  self._client = client
...
function GitEntity:get_name()
  return self._name
end


function GitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitEntity.new(self._client, opts)
end
...
function GitEntity:mark_deleted()
  self._deleted = true
end


function GitEntity:deleted()
  return true == self._deleted
end


function GitEntity:data_set(args)
  if args ~= nil then
...
function GitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitEntity:match_set(args)
  if args ~= nil then
...
function GitEntity:stream(action, args, callopts)
...
---@param reqmatch GitRemoveMatch
---@param ctrl? table
---@return Git
---@return string? err
function GitEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function GitEntity:_run_op(ctx, post_done)
  local utility = self._utility
