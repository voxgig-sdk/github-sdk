-- Github SDK CodeSecurityDefaultConfiguration entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CodeSecurityDefaultConfigurationEntity = {}
CodeSecurityDefaultConfigurationEntity.__index = CodeSecurityDefaultConfigurationEntity


function CodeSecurityDefaultConfigurationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeSecurityDefaultConfigurationEntity)
  self._name = "code_security_default_configuration"
  self._client = client
...
function CodeSecurityDefaultConfigurationEntity:get_name()
  return self._name
end


function CodeSecurityDefaultConfigurationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeSecurityDefaultConfigurationEntity.new(self._client, opts)
end
...
function CodeSecurityDefaultConfigurationEntity:mark_deleted()
  self._deleted = true
end


function CodeSecurityDefaultConfigurationEntity:deleted()
  return true == self._deleted
end


function CodeSecurityDefaultConfigurationEntity:data_set(args)
  if args ~= nil then
...
function CodeSecurityDefaultConfigurationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeSecurityDefaultConfigurationEntity:match_set(args)
  if args ~= nil then
...
function CodeSecurityDefaultConfigurationEntity:match_get()
...
function CodeSecurityDefaultConfigurationEntity:stream(action, args, callopts)
...
---@param reqmatch CodeSecurityDefaultConfigurationListMatch
---@param ctrl? table
---@return CodeSecurityDefaultConfiguration[]
---@return string? err
function CodeSecurityDefaultConfigurationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CodeSecurityDefaultConfigurationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CodeSecurityDefaultConfigurationEntity
