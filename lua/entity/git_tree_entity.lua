...
local GitTreeEntity = {}
GitTreeEntity.__index = GitTreeEntity


function GitTreeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitTreeEntity)
  self._name = "git_tree"
  self._client = client
...
function GitTreeEntity:get_name()
  return self._name
end


function GitTreeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitTreeEntity.new(self._client, opts)
end
...
function GitTreeEntity:mark_deleted()
  self._deleted = true
end


function GitTreeEntity:deleted()
  return true == self._deleted
end


function GitTreeEntity:data_set(args)
  if args ~= nil then
...
function GitTreeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitTreeEntity:match_set(args)
  if args ~= nil then
...
function GitTreeEntity:match_get()
...
function GitTreeEntity:stream(action, args, callopts)
...
---@param reqmatch GitTreeLoadMatch
---@param ctrl? table
---@return GitTree
---@return string? err
function GitTreeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GitTreeCreateData
---@param ctrl? table
---@return GitTree
---@return string? err
function GitTreeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function GitTreeEntity:_run_op(ctx, post_done)
  local utility = self._utility
