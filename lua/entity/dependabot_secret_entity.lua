...
local DependabotSecretEntity = {}
DependabotSecretEntity.__index = DependabotSecretEntity


function DependabotSecretEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependabotSecretEntity)
  self._name = "dependabot_secret"
  self._client = client
...
function DependabotSecretEntity:get_name()
  return self._name
end


function DependabotSecretEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependabotSecretEntity.new(self._client, opts)
end
...
function DependabotSecretEntity:mark_deleted()
  self._deleted = true
end


function DependabotSecretEntity:deleted()
  return true == self._deleted
end


function DependabotSecretEntity:data_set(args)
  if args ~= nil then
...
function DependabotSecretEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependabotSecretEntity:match_set(args)
  if args ~= nil then
...
function DependabotSecretEntity:match_get()
...
function DependabotSecretEntity:stream(action, args, callopts)
...
---@param reqmatch DependabotSecretLoadMatch
---@param ctrl? table
---@return DependabotSecret
---@return string? err
function DependabotSecretEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DependabotSecretEntity:_run_op(ctx, post_done)
  local utility = self._utility
