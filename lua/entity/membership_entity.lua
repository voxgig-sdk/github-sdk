...
local MembershipEntity = {}
MembershipEntity.__index = MembershipEntity


function MembershipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MembershipEntity)
  self._name = "membership"
  self._client = client
...
function MembershipEntity:get_name()
  return self._name
end


function MembershipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MembershipEntity.new(self._client, opts)
end
...
function MembershipEntity:mark_deleted()
  self._deleted = true
end


function MembershipEntity:deleted()
  return true == self._deleted
end


function MembershipEntity:data_set(args)
  if args ~= nil then
...
function MembershipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MembershipEntity:match_set(args)
  if args ~= nil then
...
function MembershipEntity:match_get()
...
function MembershipEntity:stream(action, args, callopts)
...
---@param reqmatch MembershipLoadMatch
---@param ctrl? table
---@return Membership
---@return string? err
function MembershipEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch MembershipListMatch
---@param ctrl? table
---@return Membership[]
---@return string? err
function MembershipEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata MembershipUpdateData
---@param ctrl? table
---@return Membership
---@return string? err
function MembershipEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function MembershipEntity:_run_op(ctx, post_done)
  local utility = self._utility
