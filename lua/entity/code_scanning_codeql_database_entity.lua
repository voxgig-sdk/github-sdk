...
local CodeScanningCodeqlDatabaseEntity = {}
CodeScanningCodeqlDatabaseEntity.__index = CodeScanningCodeqlDatabaseEntity


function CodeScanningCodeqlDatabaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeScanningCodeqlDatabaseEntity)
  self._name = "code_scanning_codeql_database"
  self._client = client
...
function CodeScanningCodeqlDatabaseEntity:get_name()
  return self._name
end


function CodeScanningCodeqlDatabaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeScanningCodeqlDatabaseEntity.new(self._client, opts)
end
...
function CodeScanningCodeqlDatabaseEntity:mark_deleted()
  self._deleted = true
end


function CodeScanningCodeqlDatabaseEntity:deleted()
  return true == self._deleted
end


function CodeScanningCodeqlDatabaseEntity:data_set(args)
  if args ~= nil then
...
function CodeScanningCodeqlDatabaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeScanningCodeqlDatabaseEntity:match_set(args)
  if args ~= nil then
...
function CodeScanningCodeqlDatabaseEntity:match_get()
...
function CodeScanningCodeqlDatabaseEntity:stream(action, args, callopts)
...
---@param reqmatch CodeScanningCodeqlDatabaseLoadMatch
---@param ctrl? table
---@return CodeScanningCodeqlDatabase
---@return string? err
function CodeScanningCodeqlDatabaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CodeScanningCodeqlDatabaseListMatch
---@param ctrl? table
---@return CodeScanningCodeqlDatabase[]
---@return string? err
function CodeScanningCodeqlDatabaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CodeScanningCodeqlDatabaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CodeScanningCodeqlDatabaseEntity
