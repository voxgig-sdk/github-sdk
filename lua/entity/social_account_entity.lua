...
local SocialAccountEntity = {}
SocialAccountEntity.__index = SocialAccountEntity


function SocialAccountEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SocialAccountEntity)
  self._name = "social_account"
  self._client = client
...
function SocialAccountEntity:get_name()
  return self._name
end


function SocialAccountEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SocialAccountEntity.new(self._client, opts)
end
...
function SocialAccountEntity:mark_deleted()
  self._deleted = true
end


function SocialAccountEntity:deleted()
  return true == self._deleted
end


function SocialAccountEntity:data_set(args)
  if args ~= nil then
...
function SocialAccountEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SocialAccountEntity:match_set(args)
  if args ~= nil then
...
function SocialAccountEntity:match_get()
...
function SocialAccountEntity:stream(action, args, callopts)
...
---@param reqmatch SocialAccountListMatch
---@param ctrl? table
---@return SocialAccount[]
---@return string? err
function SocialAccountEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata SocialAccountCreateData
---@param ctrl? table
---@return SocialAccount
---@return string? err
function SocialAccountEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function SocialAccountEntity:_run_op(ctx, post_done)
  local utility = self._utility
