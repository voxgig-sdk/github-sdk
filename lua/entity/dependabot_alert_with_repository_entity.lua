-- Github SDK DependabotAlertWithRepository entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DependabotAlertWithRepositoryEntity = {}
DependabotAlertWithRepositoryEntity.__index = DependabotAlertWithRepositoryEntity


function DependabotAlertWithRepositoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependabotAlertWithRepositoryEntity)
  self._name = "dependabot_alert_with_repository"
  self._client = client
...
function DependabotAlertWithRepositoryEntity:get_name()
  return self._name
end


function DependabotAlertWithRepositoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependabotAlertWithRepositoryEntity.new(self._client, opts)
end
...
function DependabotAlertWithRepositoryEntity:mark_deleted()
  self._deleted = true
end


function DependabotAlertWithRepositoryEntity:deleted()
  return true == self._deleted
end


function DependabotAlertWithRepositoryEntity:data_set(args)
  if args ~= nil then
...
function DependabotAlertWithRepositoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependabotAlertWithRepositoryEntity:match_set(args)
  if args ~= nil then
...
function DependabotAlertWithRepositoryEntity:match_get()
...
function DependabotAlertWithRepositoryEntity:stream(action, args, callopts)
...
---@param reqmatch DependabotAlertWithRepositoryListMatch
---@param ctrl? table
---@return DependabotAlertWithRepository[]
---@return string? err
function DependabotAlertWithRepositoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DependabotAlertWithRepositoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DependabotAlertWithRepositoryEntity
