...
local HookDeliveryEntity = {}
HookDeliveryEntity.__index = HookDeliveryEntity


function HookDeliveryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HookDeliveryEntity)
  self._name = "hook_delivery"
  self._client = client
...
function HookDeliveryEntity:get_name()
  return self._name
end


function HookDeliveryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HookDeliveryEntity.new(self._client, opts)
end
...
function HookDeliveryEntity:mark_deleted()
  self._deleted = true
end


function HookDeliveryEntity:deleted()
  return true == self._deleted
end


function HookDeliveryEntity:data_set(args)
  if args ~= nil then
...
function HookDeliveryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HookDeliveryEntity:match_set(args)
  if args ~= nil then
...
function HookDeliveryEntity:match_get()
...
function HookDeliveryEntity:stream(action, args, callopts)
...
---@param reqmatch HookDeliveryLoadMatch
---@param ctrl? table
---@return HookDelivery
---@return string? err
function HookDeliveryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function HookDeliveryEntity:_run_op(ctx, post_done)
  local utility = self._utility
