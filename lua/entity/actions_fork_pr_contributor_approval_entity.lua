-- Github SDK ActionsForkPrContributorApproval entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsForkPrContributorApprovalEntity = {}
ActionsForkPrContributorApprovalEntity.__index = ActionsForkPrContributorApprovalEntity


function ActionsForkPrContributorApprovalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsForkPrContributorApprovalEntity)
  self._name = "actions_fork_pr_contributor_approval"
  self._client = client
...
function ActionsForkPrContributorApprovalEntity:get_name()
  return self._name
end


function ActionsForkPrContributorApprovalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsForkPrContributorApprovalEntity.new(self._client, opts)
end
...
function ActionsForkPrContributorApprovalEntity:mark_deleted()
  self._deleted = true
end


function ActionsForkPrContributorApprovalEntity:deleted()
  return true == self._deleted
end


function ActionsForkPrContributorApprovalEntity:data_set(args)
  if args ~= nil then
...
function ActionsForkPrContributorApprovalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsForkPrContributorApprovalEntity:match_set(args)
  if args ~= nil then
...
function ActionsForkPrContributorApprovalEntity:match_get()
...
function ActionsForkPrContributorApprovalEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsForkPrContributorApprovalLoadMatch
---@param ctrl? table
---@return ActionsForkPrContributorApproval
---@return string? err
function ActionsForkPrContributorApprovalEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsForkPrContributorApprovalEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsForkPrContributorApprovalEntity
