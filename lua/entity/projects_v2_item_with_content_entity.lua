...
local ProjectsV2ItemWithContentEntity = {}
ProjectsV2ItemWithContentEntity.__index = ProjectsV2ItemWithContentEntity


function ProjectsV2ItemWithContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectsV2ItemWithContentEntity)
  self._name = "projects_v2_item_with_content"
  self._client = client
...
function ProjectsV2ItemWithContentEntity:get_name()
  return self._name
end


function ProjectsV2ItemWithContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectsV2ItemWithContentEntity.new(self._client, opts)
end
...
function ProjectsV2ItemWithContentEntity:mark_deleted()
  self._deleted = true
end


function ProjectsV2ItemWithContentEntity:deleted()
  return true == self._deleted
end


function ProjectsV2ItemWithContentEntity:data_set(args)
  if args ~= nil then
...
function ProjectsV2ItemWithContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectsV2ItemWithContentEntity:match_set(args)
  if args ~= nil then
...
function ProjectsV2ItemWithContentEntity:match_get()
...
function ProjectsV2ItemWithContentEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectsV2ItemWithContentLoadMatch
---@param ctrl? table
---@return ProjectsV2ItemWithContent
---@return string? err
function ProjectsV2ItemWithContentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectsV2ItemWithContentListMatch
---@param ctrl? table
---@return ProjectsV2ItemWithContent[]
---@return string? err
function ProjectsV2ItemWithContentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ProjectsV2ItemWithContentUpdateData
---@param ctrl? table
---@return ProjectsV2ItemWithContent
---@return string? err
function ProjectsV2ItemWithContentEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ProjectsV2ItemWithContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProjectsV2ItemWithContentEntity
