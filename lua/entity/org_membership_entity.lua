...
local OrgMembershipEntity = {}
OrgMembershipEntity.__index = OrgMembershipEntity


function OrgMembershipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrgMembershipEntity)
  self._name = "org_membership"
  self._client = client
...
function OrgMembershipEntity:get_name()
  return self._name
end


function OrgMembershipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrgMembershipEntity.new(self._client, opts)
end
...
function OrgMembershipEntity:mark_deleted()
  self._deleted = true
end


function OrgMembershipEntity:deleted()
  return true == self._deleted
end


function OrgMembershipEntity:data_set(args)
  if args ~= nil then
...
function OrgMembershipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrgMembershipEntity:match_set(args)
  if args ~= nil then
...
function OrgMembershipEntity:match_get()
...
function OrgMembershipEntity:stream(action, args, callopts)
...
---@param reqmatch OrgMembershipLoadMatch
---@param ctrl? table
---@return OrgMembership
---@return string? err
function OrgMembershipEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata OrgMembershipUpdateData
---@param ctrl? table
---@return OrgMembership
---@return string? err
function OrgMembershipEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function OrgMembershipEntity:_run_op(ctx, post_done)
  local utility = self._utility
