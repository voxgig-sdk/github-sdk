...
local MergedUpstreamEntity = {}
MergedUpstreamEntity.__index = MergedUpstreamEntity


function MergedUpstreamEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MergedUpstreamEntity)
  self._name = "merged_upstream"
  self._client = client
...
function MergedUpstreamEntity:get_name()
  return self._name
end


function MergedUpstreamEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MergedUpstreamEntity.new(self._client, opts)
end
...
function MergedUpstreamEntity:mark_deleted()
  self._deleted = true
end


function MergedUpstreamEntity:deleted()
  return true == self._deleted
end


function MergedUpstreamEntity:data_set(args)
  if args ~= nil then
...
function MergedUpstreamEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MergedUpstreamEntity:match_set(args)
  if args ~= nil then
...
function MergedUpstreamEntity:match_get()
...
function MergedUpstreamEntity:stream(action, args, callopts)
...
---@param reqdata MergedUpstreamCreateData
---@param ctrl? table
---@return MergedUpstream
---@return string? err
function MergedUpstreamEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MergedUpstreamEntity:_run_op(ctx, post_done)
  local utility = self._utility
