...
local RemoveEntity = {}
RemoveEntity.__index = RemoveEntity


function RemoveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RemoveEntity)
  self._name = "remove"
  self._client = client
...
function RemoveEntity:get_name()
  return self._name
end


function RemoveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RemoveEntity.new(self._client, opts)
end
...
function RemoveEntity:mark_deleted()
  self._deleted = true
end


function RemoveEntity:deleted()
  return true == self._deleted
end


function RemoveEntity:data_set(args)
  if args ~= nil then
...
function RemoveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RemoveEntity:match_set(args)
  if args ~= nil then
...
function RemoveEntity:match_get()
...
function RemoveEntity:stream(action, args, callopts)
...
---@param reqdata RemoveCreateData
---@param ctrl? table
---@return Remove
---@return string? err
function RemoveEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RemoveEntity:_run_op(ctx, post_done)
  local utility = self._utility
