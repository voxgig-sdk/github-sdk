...
local CheckSuitePreferenceEntity = {}
CheckSuitePreferenceEntity.__index = CheckSuitePreferenceEntity


function CheckSuitePreferenceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CheckSuitePreferenceEntity)
  self._name = "check_suite_preference"
  self._client = client
...
function CheckSuitePreferenceEntity:get_name()
  return self._name
end


function CheckSuitePreferenceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CheckSuitePreferenceEntity.new(self._client, opts)
end
...
function CheckSuitePreferenceEntity:mark_deleted()
  self._deleted = true
end


function CheckSuitePreferenceEntity:deleted()
  return true == self._deleted
end


function CheckSuitePreferenceEntity:data_set(args)
  if args ~= nil then
...
function CheckSuitePreferenceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CheckSuitePreferenceEntity:match_set(args)
  if args ~= nil then
...
function CheckSuitePreferenceEntity:match_get()
...
function CheckSuitePreferenceEntity:stream(action, args, callopts)
...
---@param reqdata CheckSuitePreferenceUpdateData
---@param ctrl? table
---@return CheckSuitePreference
---@return string? err
function CheckSuitePreferenceEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function CheckSuitePreferenceEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CheckSuitePreferenceEntity
