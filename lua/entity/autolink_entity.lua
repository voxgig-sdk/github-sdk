...
local AutolinkEntity = {}
AutolinkEntity.__index = AutolinkEntity


function AutolinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AutolinkEntity)
  self._name = "autolink"
  self._client = client
...
function AutolinkEntity:get_name()
  return self._name
end


function AutolinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AutolinkEntity.new(self._client, opts)
end
...
function AutolinkEntity:mark_deleted()
  self._deleted = true
end


function AutolinkEntity:deleted()
  return true == self._deleted
end


function AutolinkEntity:data_set(args)
  if args ~= nil then
...
function AutolinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AutolinkEntity:match_set(args)
  if args ~= nil then
...
function AutolinkEntity:match_get()
...
function AutolinkEntity:stream(action, args, callopts)
...
---@param reqmatch AutolinkLoadMatch
---@param ctrl? table
---@return Autolink
---@return string? err
function AutolinkEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch AutolinkListMatch
---@param ctrl? table
---@return Autolink[]
---@return string? err
function AutolinkEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata AutolinkCreateData
---@param ctrl? table
---@return Autolink
---@return string? err
function AutolinkEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AutolinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
