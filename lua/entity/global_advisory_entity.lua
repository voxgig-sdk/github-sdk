...
local GlobalAdvisoryEntity = {}
GlobalAdvisoryEntity.__index = GlobalAdvisoryEntity


function GlobalAdvisoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GlobalAdvisoryEntity)
  self._name = "global_advisory"
  self._client = client
...
function GlobalAdvisoryEntity:get_name()
  return self._name
end


function GlobalAdvisoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GlobalAdvisoryEntity.new(self._client, opts)
end
...
function GlobalAdvisoryEntity:mark_deleted()
  self._deleted = true
end


function GlobalAdvisoryEntity:deleted()
  return true == self._deleted
end


function GlobalAdvisoryEntity:data_set(args)
  if args ~= nil then
...
function GlobalAdvisoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GlobalAdvisoryEntity:match_set(args)
  if args ~= nil then
...
function GlobalAdvisoryEntity:match_get()
...
function GlobalAdvisoryEntity:stream(action, args, callopts)
...
---@param reqmatch GlobalAdvisoryLoadMatch
---@param ctrl? table
---@return GlobalAdvisory
---@return string? err
function GlobalAdvisoryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GlobalAdvisoryListMatch
---@param ctrl? table
---@return GlobalAdvisory[]
---@return string? err
function GlobalAdvisoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GlobalAdvisoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
