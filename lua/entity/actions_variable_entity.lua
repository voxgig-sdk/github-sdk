...
local ActionsVariableEntity = {}
ActionsVariableEntity.__index = ActionsVariableEntity


function ActionsVariableEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsVariableEntity)
  self._name = "actions_variable"
  self._client = client
...
function ActionsVariableEntity:get_name()
  return self._name
end


function ActionsVariableEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsVariableEntity.new(self._client, opts)
end
...
function ActionsVariableEntity:mark_deleted()
  self._deleted = true
end


function ActionsVariableEntity:deleted()
  return true == self._deleted
end


function ActionsVariableEntity:data_set(args)
  if args ~= nil then
...
function ActionsVariableEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsVariableEntity:match_set(args)
  if args ~= nil then
...
function ActionsVariableEntity:match_get()
...
function ActionsVariableEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsVariableLoadMatch
---@param ctrl? table
---@return ActionsVariable
---@return string? err
function ActionsVariableEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsVariableEntity:_run_op(ctx, post_done)
  local utility = self._utility
