...
local ProjectColumnEntity = {}
ProjectColumnEntity.__index = ProjectColumnEntity


function ProjectColumnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectColumnEntity)
  self._name = "project_column"
  self._client = client
...
function ProjectColumnEntity:get_name()
  return self._name
end


function ProjectColumnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectColumnEntity.new(self._client, opts)
end
...
function ProjectColumnEntity:mark_deleted()
  self._deleted = true
end


function ProjectColumnEntity:deleted()
  return true == self._deleted
end


function ProjectColumnEntity:data_set(args)
  if args ~= nil then
...
function ProjectColumnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectColumnEntity:match_set(args)
  if args ~= nil then
...
function ProjectColumnEntity:match_get()
...
function ProjectColumnEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectColumnLoadMatch
---@param ctrl? table
---@return ProjectColumn
---@return string? err
function ProjectColumnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectColumnListMatch
---@param ctrl? table
---@return ProjectColumn[]
---@return string? err
function ProjectColumnEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ProjectColumnCreateData
---@param ctrl? table
---@return ProjectColumn
---@return string? err
function ProjectColumnEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata ProjectColumnUpdateData
---@param ctrl? table
---@return ProjectColumn
---@return string? err
function ProjectColumnEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ProjectColumnEntity:_run_op(ctx, post_done)
  local utility = self._utility
