...
local ProjectsV2Entity = {}
ProjectsV2Entity.__index = ProjectsV2Entity


function ProjectsV2Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectsV2Entity)
  self._name = "projects_v2"
  self._client = client
...
function ProjectsV2Entity:get_name()
  return self._name
end


function ProjectsV2Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectsV2Entity.new(self._client, opts)
end
...
function ProjectsV2Entity:mark_deleted()
  self._deleted = true
end


function ProjectsV2Entity:deleted()
  return true == self._deleted
end


function ProjectsV2Entity:data_set(args)
  if args ~= nil then
...
function ProjectsV2Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectsV2Entity:match_set(args)
  if args ~= nil then
...
function ProjectsV2Entity:match_get()
...
function ProjectsV2Entity:stream(action, args, callopts)
...
---@param reqmatch ProjectsV2LoadMatch
---@param ctrl? table
---@return ProjectsV2
---@return string? err
function ProjectsV2Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ProjectsV2ListMatch
---@param ctrl? table
---@return ProjectsV2[]
---@return string? err
function ProjectsV2Entity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ProjectsV2Entity:_run_op(ctx, post_done)
  local utility = self._utility
