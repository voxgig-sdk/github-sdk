...
local AddEntity = {}
AddEntity.__index = AddEntity


function AddEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AddEntity)
  self._name = "add"
  self._client = client
...
function AddEntity:make()
  ...
  return AddEntity.new(self._client, opts)
end
...
function AddEntity:mark_deleted()
  self._deleted = true
end


function AddEntity:deleted()
  return true == self._deleted
end


function AddEntity:data_set(args)
  if args ~= nil then
...
function AddEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AddEntity:match_set(args)
  if args ~= nil then
...
function AddEntity:stream(action, args, callopts)
...
---@param reqdata AddCreateData
---@param ctrl? table
---@return Add
---@return string? err
function AddEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AddEntity:_run_op(ctx, post_done)
  local utility = self._utility
