...
local CheckRunEntity = {}
CheckRunEntity.__index = CheckRunEntity


function CheckRunEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CheckRunEntity)
  self._name = "check_run"
  self._client = client
...
function CheckRunEntity:get_name()
  return self._name
end


function CheckRunEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CheckRunEntity.new(self._client, opts)
end
...
function CheckRunEntity:mark_deleted()
  self._deleted = true
end


function CheckRunEntity:deleted()
  return true == self._deleted
end


function CheckRunEntity:data_set(args)
  if args ~= nil then
...
function CheckRunEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CheckRunEntity:match_set(args)
  if args ~= nil then
...
function CheckRunEntity:match_get()
...
function CheckRunEntity:stream(action, args, callopts)
...
---@param reqmatch CheckRunLoadMatch
---@param ctrl? table
---@return CheckRun
---@return string? err
function CheckRunEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CheckRunCreateData
---@param ctrl? table
---@return CheckRun
---@return string? err
function CheckRunEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata CheckRunUpdateData
---@param ctrl? table
---@return CheckRun
---@return string? err
function CheckRunEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function CheckRunEntity:_run_op(ctx, post_done)
  local utility = self._utility
