...
local PageDeploymentEntity = {}
PageDeploymentEntity.__index = PageDeploymentEntity


function PageDeploymentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PageDeploymentEntity)
  self._name = "page_deployment"
  self._client = client
...
function PageDeploymentEntity:get_name()
  return self._name
end


function PageDeploymentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PageDeploymentEntity.new(self._client, opts)
end
...
function PageDeploymentEntity:mark_deleted()
  self._deleted = true
end


function PageDeploymentEntity:deleted()
  return true == self._deleted
end


function PageDeploymentEntity:data_set(args)
  if args ~= nil then
...
function PageDeploymentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PageDeploymentEntity:match_set(args)
  if args ~= nil then
...
function PageDeploymentEntity:match_get()
...
function PageDeploymentEntity:stream(action, args, callopts)
...
---@param reqdata PageDeploymentCreateData
---@param ctrl? table
---@return PageDeployment
---@return string? err
function PageDeploymentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PageDeploymentEntity:_run_op(ctx, post_done)
  local utility = self._utility
