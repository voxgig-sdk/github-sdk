-- Github SDK ActionsWorkflowAccessToRepository entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsWorkflowAccessToRepositoryEntity = {}
ActionsWorkflowAccessToRepositoryEntity.__index = ActionsWorkflowAccessToRepositoryEntity


function ActionsWorkflowAccessToRepositoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsWorkflowAccessToRepositoryEntity)
  self._name = "actions_workflow_access_to_repository"
  self._client = client
...
function ActionsWorkflowAccessToRepositoryEntity:get_name()
  return self._name
end


function ActionsWorkflowAccessToRepositoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsWorkflowAccessToRepositoryEntity.new(self._client, opts)
end
...
function ActionsWorkflowAccessToRepositoryEntity:mark_deleted()
  self._deleted = true
end


function ActionsWorkflowAccessToRepositoryEntity:deleted()
  return true == self._deleted
end


function ActionsWorkflowAccessToRepositoryEntity:data_set(args)
  if args ~= nil then
...
function ActionsWorkflowAccessToRepositoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsWorkflowAccessToRepositoryEntity:match_set(args)
  if args ~= nil then
...
function ActionsWorkflowAccessToRepositoryEntity:match_get()
...
function ActionsWorkflowAccessToRepositoryEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsWorkflowAccessToRepositoryLoadMatch
---@param ctrl? table
---@return ActionsWorkflowAccessToRepository
---@return string? err
function ActionsWorkflowAccessToRepositoryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsWorkflowAccessToRepositoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsWorkflowAccessToRepositoryEntity
