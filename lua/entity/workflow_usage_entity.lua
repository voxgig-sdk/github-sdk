...
local WorkflowUsageEntity = {}
WorkflowUsageEntity.__index = WorkflowUsageEntity


function WorkflowUsageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkflowUsageEntity)
  self._name = "workflow_usage"
  self._client = client
...
function WorkflowUsageEntity:get_name()
  return self._name
end


function WorkflowUsageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkflowUsageEntity.new(self._client, opts)
end
...
function WorkflowUsageEntity:mark_deleted()
  self._deleted = true
end


function WorkflowUsageEntity:deleted()
  return true == self._deleted
end


function WorkflowUsageEntity:data_set(args)
  if args ~= nil then
...
function WorkflowUsageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkflowUsageEntity:match_set(args)
  if args ~= nil then
...
function WorkflowUsageEntity:match_get()
...
function WorkflowUsageEntity:stream(action, args, callopts)
...
---@param reqmatch WorkflowUsageLoadMatch
---@param ctrl? table
---@return WorkflowUsage
---@return string? err
function WorkflowUsageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WorkflowUsageEntity:_run_op(ctx, post_done)
  local utility = self._utility
