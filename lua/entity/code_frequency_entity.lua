...
local CodeFrequencyEntity = {}
CodeFrequencyEntity.__index = CodeFrequencyEntity


function CodeFrequencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeFrequencyEntity)
  self._name = "code_frequency"
  self._client = client
...
function CodeFrequencyEntity:get_name()
  return self._name
end


function CodeFrequencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeFrequencyEntity.new(self._client, opts)
end
...
function CodeFrequencyEntity:mark_deleted()
  self._deleted = true
end


function CodeFrequencyEntity:deleted()
  return true == self._deleted
end


function CodeFrequencyEntity:data_set(args)
  if args ~= nil then
...
function CodeFrequencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeFrequencyEntity:match_set(args)
  if args ~= nil then
...
function CodeFrequencyEntity:match_get()
...
function CodeFrequencyEntity:stream(action, args, callopts)
...
---@param reqmatch CodeFrequencyListMatch
---@param ctrl? table
---@return CodeFrequency[]
---@return string? err
function CodeFrequencyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CodeFrequencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
