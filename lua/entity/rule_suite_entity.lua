...
local RuleSuiteEntity = {}
RuleSuiteEntity.__index = RuleSuiteEntity


function RuleSuiteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RuleSuiteEntity)
  self._name = "rule_suite"
  self._client = client
...
function RuleSuiteEntity:get_name()
  return self._name
end


function RuleSuiteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RuleSuiteEntity.new(self._client, opts)
end
...
function RuleSuiteEntity:mark_deleted()
  self._deleted = true
end


function RuleSuiteEntity:deleted()
  return true == self._deleted
end


function RuleSuiteEntity:data_set(args)
  if args ~= nil then
...
function RuleSuiteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RuleSuiteEntity:match_set(args)
  if args ~= nil then
...
function RuleSuiteEntity:match_get()
...
function RuleSuiteEntity:stream(action, args, callopts)
...
---@param reqmatch RuleSuiteLoadMatch
---@param ctrl? table
---@return RuleSuite
---@return string? err
function RuleSuiteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch RuleSuiteListMatch
---@param ctrl? table
---@return RuleSuite[]
---@return string? err
function RuleSuiteEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function RuleSuiteEntity:_run_op(ctx, post_done)
  local utility = self._utility
