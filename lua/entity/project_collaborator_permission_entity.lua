-- Github SDK ProjectCollaboratorPermission entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectCollaboratorPermissionEntity = {}
ProjectCollaboratorPermissionEntity.__index = ProjectCollaboratorPermissionEntity


function ProjectCollaboratorPermissionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectCollaboratorPermissionEntity)
  self._name = "project_collaborator_permission"
  self._client = client
...
function ProjectCollaboratorPermissionEntity:get_name()
  return self._name
end


function ProjectCollaboratorPermissionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectCollaboratorPermissionEntity.new(self._client, opts)
end
...
function ProjectCollaboratorPermissionEntity:mark_deleted()
  self._deleted = true
end


function ProjectCollaboratorPermissionEntity:deleted()
  return true == self._deleted
end


function ProjectCollaboratorPermissionEntity:data_set(args)
  if args ~= nil then
...
function ProjectCollaboratorPermissionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectCollaboratorPermissionEntity:match_set(args)
  if args ~= nil then
...
function ProjectCollaboratorPermissionEntity:match_get()
...
function ProjectCollaboratorPermissionEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectCollaboratorPermissionLoadMatch
---@param ctrl? table
---@return ProjectCollaboratorPermission
---@return string? err
function ProjectCollaboratorPermissionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ProjectCollaboratorPermissionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProjectCollaboratorPermissionEntity
