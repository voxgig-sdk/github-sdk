...
local MetarootEntity = {}
MetarootEntity.__index = MetarootEntity


function MetarootEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MetarootEntity)
  self._name = "metaroot"
  self._client = client
...
function MetarootEntity:get_name()
  return self._name
end


function MetarootEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MetarootEntity.new(self._client, opts)
end
...
function MetarootEntity:mark_deleted()
  self._deleted = true
end


function MetarootEntity:deleted()
  return true == self._deleted
end


function MetarootEntity:data_set(args)
  if args ~= nil then
...
function MetarootEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MetarootEntity:match_set(args)
  if args ~= nil then
...
function MetarootEntity:match_get()
...
function MetarootEntity:stream(action, args, callopts)
...
---@param reqmatch MetarootLoadMatch
---@param ctrl? table
---@return Metaroot
---@return string? err
function MetarootEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MetarootEntity:_run_op(ctx, post_done)
  local utility = self._utility
