...
local ActionsPublicKeyEntity = {}
ActionsPublicKeyEntity.__index = ActionsPublicKeyEntity


function ActionsPublicKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsPublicKeyEntity)
  self._name = "actions_public_key"
  self._client = client
...
function ActionsPublicKeyEntity:get_name()
  return self._name
end


function ActionsPublicKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsPublicKeyEntity.new(self._client, opts)
end
...
function ActionsPublicKeyEntity:mark_deleted()
  self._deleted = true
end


function ActionsPublicKeyEntity:deleted()
  return true == self._deleted
end


function ActionsPublicKeyEntity:data_set(args)
  if args ~= nil then
...
function ActionsPublicKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsPublicKeyEntity:match_set(args)
  if args ~= nil then
...
function ActionsPublicKeyEntity:match_get()
...
function ActionsPublicKeyEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsPublicKeyLoadMatch
---@param ctrl? table
---@return ActionsPublicKey
---@return string? err
function ActionsPublicKeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsPublicKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
