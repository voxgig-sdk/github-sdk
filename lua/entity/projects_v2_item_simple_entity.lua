-- Github SDK ProjectsV2ItemSimple entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectsV2ItemSimpleEntity = {}
ProjectsV2ItemSimpleEntity.__index = ProjectsV2ItemSimpleEntity


function ProjectsV2ItemSimpleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectsV2ItemSimpleEntity)
  self._name = "projects_v2_item_simple"
  self._client = client
...
function ProjectsV2ItemSimpleEntity:get_name()
  return self._name
end


function ProjectsV2ItemSimpleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectsV2ItemSimpleEntity.new(self._client, opts)
end
...
function ProjectsV2ItemSimpleEntity:mark_deleted()
  self._deleted = true
end


function ProjectsV2ItemSimpleEntity:deleted()
  return true == self._deleted
end


function ProjectsV2ItemSimpleEntity:data_set(args)
  if args ~= nil then
...
function ProjectsV2ItemSimpleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectsV2ItemSimpleEntity:match_set(args)
  if args ~= nil then
...
function ProjectsV2ItemSimpleEntity:match_get()
...
function ProjectsV2ItemSimpleEntity:stream(action, args, callopts)
...
---@param reqdata ProjectsV2ItemSimpleCreateData
---@param ctrl? table
---@return ProjectsV2ItemSimple
---@return string? err
function ProjectsV2ItemSimpleEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ProjectsV2ItemSimpleEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProjectsV2ItemSimpleEntity
