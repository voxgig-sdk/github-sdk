...
local KeyEntity = {}
KeyEntity.__index = KeyEntity


function KeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, KeyEntity)
  self._name = "key"
  self._client = client
...
function KeyEntity:get_name()
  return self._name
end


function KeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return KeyEntity.new(self._client, opts)
end
...
function KeyEntity:mark_deleted()
  self._deleted = true
end


function KeyEntity:deleted()
  return true == self._deleted
end


function KeyEntity:data_set(args)
  if args ~= nil then
...
function KeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function KeyEntity:match_set(args)
  if args ~= nil then
...
function KeyEntity:stream(action, args, callopts)
...
---@param reqmatch KeyLoadMatch
---@param ctrl? table
---@return Key
---@return string? err
function KeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch KeyListMatch
---@param ctrl? table
---@return Key[]
---@return string? err
function KeyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata KeyCreateData
---@param ctrl? table
---@return Key
---@return string? err
function KeyEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function KeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
