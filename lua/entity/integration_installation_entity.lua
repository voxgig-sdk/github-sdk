...
local IntegrationInstallationEntity = {}
IntegrationInstallationEntity.__index = IntegrationInstallationEntity


function IntegrationInstallationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IntegrationInstallationEntity)
  self._name = "integration_installation"
  self._client = client
...
function IntegrationInstallationEntity:get_name()
  return self._name
end


function IntegrationInstallationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IntegrationInstallationEntity.new(self._client, opts)
end
...
function IntegrationInstallationEntity:mark_deleted()
  self._deleted = true
end


function IntegrationInstallationEntity:deleted()
  return true == self._deleted
end


function IntegrationInstallationEntity:data_set(args)
  if args ~= nil then
...
function IntegrationInstallationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IntegrationInstallationEntity:match_set(args)
  if args ~= nil then
...
function IntegrationInstallationEntity:match_get()
...
function IntegrationInstallationEntity:stream(action, args, callopts)
...
---@param reqmatch IntegrationInstallationListMatch
---@param ctrl? table
---@return IntegrationInstallation[]
---@return string? err
function IntegrationInstallationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function IntegrationInstallationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return IntegrationInstallationEntity
