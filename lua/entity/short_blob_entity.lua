...
local ShortBlobEntity = {}
ShortBlobEntity.__index = ShortBlobEntity


function ShortBlobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShortBlobEntity)
  self._name = "short_blob"
  self._client = client
...
function ShortBlobEntity:get_name()
  return self._name
end


function ShortBlobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShortBlobEntity.new(self._client, opts)
end
...
function ShortBlobEntity:mark_deleted()
  self._deleted = true
end


function ShortBlobEntity:deleted()
  return true == self._deleted
end


function ShortBlobEntity:data_set(args)
  if args ~= nil then
...
function ShortBlobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShortBlobEntity:match_set(args)
  if args ~= nil then
...
function ShortBlobEntity:match_get()
...
function ShortBlobEntity:stream(action, args, callopts)
...
---@param reqdata ShortBlobCreateData
---@param ctrl? table
---@return ShortBlob
---@return string? err
function ShortBlobEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ShortBlobEntity:_run_op(ctx, post_done)
  local utility = self._utility
