...
local OrganizationRoleEntity = {}
OrganizationRoleEntity.__index = OrganizationRoleEntity


function OrganizationRoleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrganizationRoleEntity)
  self._name = "organization_role"
  self._client = client
...
function OrganizationRoleEntity:get_name()
  return self._name
end


function OrganizationRoleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrganizationRoleEntity.new(self._client, opts)
end
...
function OrganizationRoleEntity:mark_deleted()
  self._deleted = true
end


function OrganizationRoleEntity:deleted()
  return true == self._deleted
end


function OrganizationRoleEntity:data_set(args)
  if args ~= nil then
...
function OrganizationRoleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrganizationRoleEntity:match_set(args)
  if args ~= nil then
...
function OrganizationRoleEntity:match_get()
...
function OrganizationRoleEntity:stream(action, args, callopts)
...
---@param reqmatch OrganizationRoleLoadMatch
---@param ctrl? table
---@return OrganizationRole
---@return string? err
function OrganizationRoleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function OrganizationRoleEntity:_run_op(ctx, post_done)
  local utility = self._utility
