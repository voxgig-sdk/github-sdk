...
local RunnerGroupEntity = {}
RunnerGroupEntity.__index = RunnerGroupEntity


function RunnerGroupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RunnerGroupEntity)
  self._name = "runner_group"
  self._client = client
...
function RunnerGroupEntity:get_name()
  return self._name
end


function RunnerGroupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RunnerGroupEntity.new(self._client, opts)
end
...
function RunnerGroupEntity:mark_deleted()
  self._deleted = true
end


function RunnerGroupEntity:deleted()
  return true == self._deleted
end


function RunnerGroupEntity:data_set(args)
  if args ~= nil then
...
function RunnerGroupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RunnerGroupEntity:match_set(args)
  if args ~= nil then
...
function RunnerGroupEntity:match_get()
...
function RunnerGroupEntity:stream(action, args, callopts)
...
---@param reqmatch RunnerGroupLoadMatch
---@param ctrl? table
---@return RunnerGroup
---@return string? err
function RunnerGroupEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RunnerGroupCreateData
---@param ctrl? table
---@return RunnerGroup
---@return string? err
function RunnerGroupEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata RunnerGroupUpdateData
---@param ctrl? table
---@return RunnerGroup
---@return string? err
function RunnerGroupEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function RunnerGroupEntity:_run_op(ctx, post_done)
  local utility = self._utility
