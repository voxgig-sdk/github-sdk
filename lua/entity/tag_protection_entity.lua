...
local TagProtectionEntity = {}
TagProtectionEntity.__index = TagProtectionEntity


function TagProtectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TagProtectionEntity)
  self._name = "tag_protection"
  self._client = client
...
function TagProtectionEntity:get_name()
  return self._name
end


function TagProtectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TagProtectionEntity.new(self._client, opts)
end
...
function TagProtectionEntity:mark_deleted()
  self._deleted = true
end


function TagProtectionEntity:deleted()
  return true == self._deleted
end


function TagProtectionEntity:data_set(args)
  if args ~= nil then
...
function TagProtectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TagProtectionEntity:match_set(args)
  if args ~= nil then
...
function TagProtectionEntity:match_get()
...
function TagProtectionEntity:stream(action, args, callopts)
...
---@param reqmatch TagProtectionListMatch
---@param ctrl? table
---@return TagProtection[]
---@return string? err
function TagProtectionEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TagProtectionCreateData
---@param ctrl? table
---@return TagProtection
---@return string? err
function TagProtectionEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function TagProtectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
