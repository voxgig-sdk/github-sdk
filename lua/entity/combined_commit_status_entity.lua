...
local CombinedCommitStatusEntity = {}
CombinedCommitStatusEntity.__index = CombinedCommitStatusEntity


function CombinedCommitStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CombinedCommitStatusEntity)
  self._name = "combined_commit_status"
  self._client = client
...
function CombinedCommitStatusEntity:get_name()
  return self._name
end


function CombinedCommitStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CombinedCommitStatusEntity.new(self._client, opts)
end
...
function CombinedCommitStatusEntity:mark_deleted()
  self._deleted = true
end


function CombinedCommitStatusEntity:deleted()
  return true == self._deleted
end


function CombinedCommitStatusEntity:data_set(args)
  if args ~= nil then
...
function CombinedCommitStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CombinedCommitStatusEntity:match_set(args)
  if args ~= nil then
...
function CombinedCommitStatusEntity:match_get()
...
function CombinedCommitStatusEntity:stream(action, args, callopts)
...
---@param reqmatch CombinedCommitStatusListMatch
---@param ctrl? table
---@return CombinedCommitStatus[]
---@return string? err
function CombinedCommitStatusEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CombinedCommitStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CombinedCommitStatusEntity
