...
local DependabotAlertEntity = {}
DependabotAlertEntity.__index = DependabotAlertEntity


function DependabotAlertEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependabotAlertEntity)
  self._name = "dependabot_alert"
  self._client = client
...
function DependabotAlertEntity:get_name()
  return self._name
end


function DependabotAlertEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependabotAlertEntity.new(self._client, opts)
end
...
function DependabotAlertEntity:mark_deleted()
  self._deleted = true
end


function DependabotAlertEntity:deleted()
  return true == self._deleted
end


function DependabotAlertEntity:data_set(args)
  if args ~= nil then
...
function DependabotAlertEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependabotAlertEntity:match_set(args)
  if args ~= nil then
...
function DependabotAlertEntity:match_get()
...
function DependabotAlertEntity:stream(action, args, callopts)
...
---@param reqmatch DependabotAlertLoadMatch
---@param ctrl? table
---@return DependabotAlert
---@return string? err
function DependabotAlertEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DependabotAlertListMatch
---@param ctrl? table
---@return DependabotAlert[]
---@return string? err
function DependabotAlertEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata DependabotAlertUpdateData
---@param ctrl? table
---@return DependabotAlert
---@return string? err
function DependabotAlertEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function DependabotAlertEntity:_run_op(ctx, post_done)
  local utility = self._utility
