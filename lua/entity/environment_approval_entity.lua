...
local EnvironmentApprovalEntity = {}
EnvironmentApprovalEntity.__index = EnvironmentApprovalEntity


function EnvironmentApprovalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnvironmentApprovalEntity)
  self._name = "environment_approval"
  self._client = client
...
function EnvironmentApprovalEntity:get_name()
  return self._name
end


function EnvironmentApprovalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnvironmentApprovalEntity.new(self._client, opts)
end
...
function EnvironmentApprovalEntity:mark_deleted()
  self._deleted = true
end


function EnvironmentApprovalEntity:deleted()
  return true == self._deleted
end


function EnvironmentApprovalEntity:data_set(args)
  if args ~= nil then
...
function EnvironmentApprovalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnvironmentApprovalEntity:match_set(args)
  if args ~= nil then
...
function EnvironmentApprovalEntity:match_get()
...
function EnvironmentApprovalEntity:stream(action, args, callopts)
...
---@param reqmatch EnvironmentApprovalListMatch
---@param ctrl? table
---@return EnvironmentApproval[]
---@return string? err
function EnvironmentApprovalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function EnvironmentApprovalEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EnvironmentApprovalEntity
