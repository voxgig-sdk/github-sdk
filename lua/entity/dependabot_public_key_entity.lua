...
local DependabotPublicKeyEntity = {}
DependabotPublicKeyEntity.__index = DependabotPublicKeyEntity


function DependabotPublicKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependabotPublicKeyEntity)
  self._name = "dependabot_public_key"
  self._client = client
...
function DependabotPublicKeyEntity:get_name()
  return self._name
end


function DependabotPublicKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependabotPublicKeyEntity.new(self._client, opts)
end
...
function DependabotPublicKeyEntity:mark_deleted()
  self._deleted = true
end


function DependabotPublicKeyEntity:deleted()
  return true == self._deleted
end


function DependabotPublicKeyEntity:data_set(args)
  if args ~= nil then
...
function DependabotPublicKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependabotPublicKeyEntity:match_set(args)
  if args ~= nil then
...
function DependabotPublicKeyEntity:match_get()
...
function DependabotPublicKeyEntity:stream(action, args, callopts)
...
---@param reqmatch DependabotPublicKeyLoadMatch
---@param ctrl? table
---@return DependabotPublicKey
---@return string? err
function DependabotPublicKeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DependabotPublicKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DependabotPublicKeyEntity
