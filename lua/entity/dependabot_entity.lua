...
local DependabotEntity = {}
DependabotEntity.__index = DependabotEntity


function DependabotEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependabotEntity)
  self._name = "dependabot"
  self._client = client
...
function DependabotEntity:get_name()
  return self._name
end


function DependabotEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependabotEntity.new(self._client, opts)
end
...
function DependabotEntity:mark_deleted()
  self._deleted = true
end


function DependabotEntity:deleted()
  return true == self._deleted
end


function DependabotEntity:data_set(args)
  if args ~= nil then
...
function DependabotEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependabotEntity:match_set(args)
  if args ~= nil then
...
function DependabotEntity:match_get()
...
function DependabotEntity:stream(action, args, callopts)
...
---@param reqmatch DependabotListMatch
---@param ctrl? table
---@return Dependabot[]
---@return string? err
function DependabotEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DependabotUpdateData
---@param ctrl? table
---@return Dependabot
---@return string? err
function DependabotEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch DependabotRemoveMatch
---@param ctrl? table
---@return Dependabot
---@return string? err
function DependabotEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function DependabotEntity:_run_op(ctx, post_done)
  local utility = self._utility
