...
local DiffEntryEntity = {}
DiffEntryEntity.__index = DiffEntryEntity


function DiffEntryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DiffEntryEntity)
  self._name = "diff_entry"
  self._client = client
...
function DiffEntryEntity:get_name()
  return self._name
end


function DiffEntryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DiffEntryEntity.new(self._client, opts)
end
...
function DiffEntryEntity:mark_deleted()
  self._deleted = true
end


function DiffEntryEntity:deleted()
  return true == self._deleted
end


function DiffEntryEntity:data_set(args)
  if args ~= nil then
...
function DiffEntryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DiffEntryEntity:match_set(args)
  if args ~= nil then
...
function DiffEntryEntity:match_get()
...
function DiffEntryEntity:stream(action, args, callopts)
...
---@param reqmatch DiffEntryListMatch
---@param ctrl? table
---@return DiffEntry[]
---@return string? err
function DiffEntryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DiffEntryEntity:_run_op(ctx, post_done)
  local utility = self._utility
