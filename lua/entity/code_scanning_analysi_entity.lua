...
local CodeScanningAnalysiEntity = {}
CodeScanningAnalysiEntity.__index = CodeScanningAnalysiEntity


function CodeScanningAnalysiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeScanningAnalysiEntity)
  self._name = "code_scanning_analysi"
  self._client = client
...
function CodeScanningAnalysiEntity:get_name()
  return self._name
end


function CodeScanningAnalysiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeScanningAnalysiEntity.new(self._client, opts)
end
...
function CodeScanningAnalysiEntity:mark_deleted()
  self._deleted = true
end


function CodeScanningAnalysiEntity:deleted()
  return true == self._deleted
end


function CodeScanningAnalysiEntity:data_set(args)
  if args ~= nil then
...
function CodeScanningAnalysiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeScanningAnalysiEntity:match_set(args)
  if args ~= nil then
...
function CodeScanningAnalysiEntity:match_get()
...
function CodeScanningAnalysiEntity:stream(action, args, callopts)
...
---@param reqmatch CodeScanningAnalysiLoadMatch
---@param ctrl? table
---@return CodeScanningAnalysi
---@return string? err
function CodeScanningAnalysiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CodeScanningAnalysiListMatch
---@param ctrl? table
---@return CodeScanningAnalysi[]
---@return string? err
function CodeScanningAnalysiEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CodeScanningAnalysiEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CodeScanningAnalysiEntity
