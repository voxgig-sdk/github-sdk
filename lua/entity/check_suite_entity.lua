...
local CheckSuiteEntity = {}
CheckSuiteEntity.__index = CheckSuiteEntity


function CheckSuiteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CheckSuiteEntity)
  self._name = "check_suite"
  self._client = client
...
function CheckSuiteEntity:get_name()
  return self._name
end


function CheckSuiteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CheckSuiteEntity.new(self._client, opts)
end
...
function CheckSuiteEntity:mark_deleted()
  self._deleted = true
end


function CheckSuiteEntity:deleted()
  return true == self._deleted
end


function CheckSuiteEntity:data_set(args)
  if args ~= nil then
...
function CheckSuiteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CheckSuiteEntity:match_set(args)
  if args ~= nil then
...
function CheckSuiteEntity:match_get()
...
function CheckSuiteEntity:stream(action, args, callopts)
...
---@param reqmatch CheckSuiteLoadMatch
---@param ctrl? table
---@return CheckSuite
---@return string? err
function CheckSuiteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CheckSuiteCreateData
---@param ctrl? table
---@return CheckSuite
---@return string? err
function CheckSuiteEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CheckSuiteEntity:_run_op(ctx, post_done)
  local utility = self._utility
