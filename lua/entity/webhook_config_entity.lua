...
local WebhookConfigEntity = {}
WebhookConfigEntity.__index = WebhookConfigEntity


function WebhookConfigEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhookConfigEntity)
  self._name = "webhook_config"
  self._client = client
...
function WebhookConfigEntity:get_name()
  return self._name
end


function WebhookConfigEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhookConfigEntity.new(self._client, opts)
end
...
function WebhookConfigEntity:mark_deleted()
  self._deleted = true
end


function WebhookConfigEntity:deleted()
  return true == self._deleted
end


function WebhookConfigEntity:data_set(args)
  if args ~= nil then
...
function WebhookConfigEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhookConfigEntity:match_set(args)
  if args ~= nil then
...
function WebhookConfigEntity:match_get()
...
function WebhookConfigEntity:stream(action, args, callopts)
...
---@param reqmatch WebhookConfigLoadMatch
---@param ctrl? table
---@return WebhookConfig
---@return string? err
function WebhookConfigEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata WebhookConfigUpdateData
---@param ctrl? table
---@return WebhookConfig
---@return string? err
function WebhookConfigEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function WebhookConfigEntity:_run_op(ctx, post_done)
  local utility = self._utility
