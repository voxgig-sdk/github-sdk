...
local CommitCommentEntity = {}
CommitCommentEntity.__index = CommitCommentEntity


function CommitCommentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CommitCommentEntity)
  self._name = "commit_comment"
  self._client = client
...
function CommitCommentEntity:get_name()
  return self._name
end


function CommitCommentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CommitCommentEntity.new(self._client, opts)
end
...
function CommitCommentEntity:mark_deleted()
  self._deleted = true
end


function CommitCommentEntity:deleted()
  return true == self._deleted
end


function CommitCommentEntity:data_set(args)
  if args ~= nil then
...
function CommitCommentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CommitCommentEntity:match_set(args)
  if args ~= nil then
...
function CommitCommentEntity:match_get()
...
function CommitCommentEntity:stream(action, args, callopts)
...
---@param reqmatch CommitCommentLoadMatch
---@param ctrl? table
---@return CommitComment
---@return string? err
function CommitCommentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CommitCommentListMatch
---@param ctrl? table
---@return CommitComment[]
---@return string? err
function CommitCommentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CommitCommentCreateData
---@param ctrl? table
---@return CommitComment
---@return string? err
function CommitCommentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata CommitCommentUpdateData
---@param ctrl? table
---@return CommitComment
---@return string? err
function CommitCommentEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function CommitCommentEntity:_run_op(ctx, post_done)
  local utility = self._utility
