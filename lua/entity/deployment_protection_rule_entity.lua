...
local DeploymentProtectionRuleEntity = {}
DeploymentProtectionRuleEntity.__index = DeploymentProtectionRuleEntity


function DeploymentProtectionRuleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeploymentProtectionRuleEntity)
  self._name = "deployment_protection_rule"
  self._client = client
...
function DeploymentProtectionRuleEntity:get_name()
  return self._name
end


function DeploymentProtectionRuleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeploymentProtectionRuleEntity.new(self._client, opts)
end
...
function DeploymentProtectionRuleEntity:mark_deleted()
  self._deleted = true
end


function DeploymentProtectionRuleEntity:deleted()
  return true == self._deleted
end


function DeploymentProtectionRuleEntity:data_set(args)
  if args ~= nil then
...
function DeploymentProtectionRuleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeploymentProtectionRuleEntity:match_set(args)
  if args ~= nil then
...
function DeploymentProtectionRuleEntity:match_get()
...
function DeploymentProtectionRuleEntity:stream(action, args, callopts)
...
---@param reqmatch DeploymentProtectionRuleLoadMatch
---@param ctrl? table
---@return DeploymentProtectionRule
---@return string? err
function DeploymentProtectionRuleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DeploymentProtectionRuleCreateData
---@param ctrl? table
---@return DeploymentProtectionRule
---@return string? err
function DeploymentProtectionRuleEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function DeploymentProtectionRuleEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DeploymentProtectionRuleEntity
