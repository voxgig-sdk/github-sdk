...
local ActionsSecretEntity = {}
ActionsSecretEntity.__index = ActionsSecretEntity


function ActionsSecretEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsSecretEntity)
  self._name = "actions_secret"
  self._client = client
...
function ActionsSecretEntity:get_name()
  return self._name
end


function ActionsSecretEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsSecretEntity.new(self._client, opts)
end
...
function ActionsSecretEntity:mark_deleted()
  self._deleted = true
end


function ActionsSecretEntity:deleted()
  return true == self._deleted
end


function ActionsSecretEntity:data_set(args)
  if args ~= nil then
...
function ActionsSecretEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsSecretEntity:match_set(args)
  if args ~= nil then
...
function ActionsSecretEntity:match_get()
...
function ActionsSecretEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsSecretLoadMatch
---@param ctrl? table
---@return ActionsSecret
---@return string? err
function ActionsSecretEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsSecretEntity:_run_op(ctx, post_done)
  local utility = self._utility
