-- Github SDK OrganizationProgrammaticAccessGrant entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OrganizationProgrammaticAccessGrantEntity = {}
OrganizationProgrammaticAccessGrantEntity.__index = OrganizationProgrammaticAccessGrantEntity


function OrganizationProgrammaticAccessGrantEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrganizationProgrammaticAccessGrantEntity)
  self._name = "organization_programmatic_access_grant"
  self._client = client
...
function OrganizationProgrammaticAccessGrantEntity:get_name()
  return self._name
end


function OrganizationProgrammaticAccessGrantEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrganizationProgrammaticAccessGrantEntity.new(self._client, opts)
end
...
function OrganizationProgrammaticAccessGrantEntity:mark_deleted()
  self._deleted = true
end


function OrganizationProgrammaticAccessGrantEntity:deleted()
  return true == self._deleted
end


function OrganizationProgrammaticAccessGrantEntity:data_set(args)
  if args ~= nil then
...
function OrganizationProgrammaticAccessGrantEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrganizationProgrammaticAccessGrantEntity:match_set(args)
  if args ~= nil then
...
function OrganizationProgrammaticAccessGrantEntity:match_get()
...
function OrganizationProgrammaticAccessGrantEntity:stream(action, args, callopts)
...
---@param reqmatch OrganizationProgrammaticAccessGrantListMatch
---@param ctrl? table
---@return OrganizationProgrammaticAccessGrant[]
---@return string? err
function OrganizationProgrammaticAccessGrantEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function OrganizationProgrammaticAccessGrantEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return OrganizationProgrammaticAccessGrantEntity
