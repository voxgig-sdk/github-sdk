-- Github SDK SimpleClassroom entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SimpleClassroomEntity = {}
SimpleClassroomEntity.__index = SimpleClassroomEntity


function SimpleClassroomEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SimpleClassroomEntity)
  self._name = "simple_classroom"
  self._client = client
...
function SimpleClassroomEntity:get_name()
  return self._name
end


function SimpleClassroomEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SimpleClassroomEntity.new(self._client, opts)
end
...
function SimpleClassroomEntity:mark_deleted()
  self._deleted = true
end


function SimpleClassroomEntity:deleted()
  return true == self._deleted
end


function SimpleClassroomEntity:data_set(args)
  if args ~= nil then
...
function SimpleClassroomEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SimpleClassroomEntity:match_set(args)
  if args ~= nil then
...
function SimpleClassroomEntity:match_get()
...
function SimpleClassroomEntity:stream(action, args, callopts)
...
function SimpleClassroomEntity:_run_op(ctx, post_done)
  local utility = self._utility
