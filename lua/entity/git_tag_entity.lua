...
local GitTagEntity = {}
GitTagEntity.__index = GitTagEntity


function GitTagEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitTagEntity)
  self._name = "git_tag"
  self._client = client
...
function GitTagEntity:get_name()
  return self._name
end


function GitTagEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitTagEntity.new(self._client, opts)
end
...
function GitTagEntity:mark_deleted()
  self._deleted = true
end


function GitTagEntity:deleted()
  return true == self._deleted
end


function GitTagEntity:data_set(args)
  if args ~= nil then
...
function GitTagEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitTagEntity:match_set(args)
  if args ~= nil then
...
function GitTagEntity:match_get()
...
function GitTagEntity:stream(action, args, callopts)
...
---@param reqmatch GitTagLoadMatch
---@param ctrl? table
---@return GitTag
---@return string? err
function GitTagEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GitTagCreateData
---@param ctrl? table
---@return GitTag
---@return string? err
function GitTagEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function GitTagEntity:_run_op(ctx, post_done)
  local utility = self._utility
