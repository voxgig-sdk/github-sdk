...
local ReferrerEntity = {}
ReferrerEntity.__index = ReferrerEntity


function ReferrerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReferrerEntity)
  self._name = "referrer"
  self._client = client
...
function ReferrerEntity:get_name()
  return self._name
end


function ReferrerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReferrerEntity.new(self._client, opts)
end
...
function ReferrerEntity:mark_deleted()
  self._deleted = true
end


function ReferrerEntity:deleted()
  return true == self._deleted
end


function ReferrerEntity:data_set(args)
  if args ~= nil then
...
function ReferrerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReferrerEntity:match_set(args)
  if args ~= nil then
...
function ReferrerEntity:match_get()
...
function ReferrerEntity:stream(action, args, callopts)
...
---@param reqmatch ReferrerListMatch
---@param ctrl? table
---@return Referrer[]
---@return string? err
function ReferrerEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ReferrerEntity:_run_op(ctx, post_done)
  local utility = self._utility
