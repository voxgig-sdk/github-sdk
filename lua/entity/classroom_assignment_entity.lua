...
local ClassroomAssignmentEntity = {}
ClassroomAssignmentEntity.__index = ClassroomAssignmentEntity


function ClassroomAssignmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClassroomAssignmentEntity)
  self._name = "classroom_assignment"
  self._client = client
...
function ClassroomAssignmentEntity:get_name()
  return self._name
end


function ClassroomAssignmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClassroomAssignmentEntity.new(self._client, opts)
end
...
function ClassroomAssignmentEntity:mark_deleted()
  self._deleted = true
end


function ClassroomAssignmentEntity:deleted()
  return true == self._deleted
end


function ClassroomAssignmentEntity:data_set(args)
  if args ~= nil then
...
function ClassroomAssignmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClassroomAssignmentEntity:match_set(args)
  if args ~= nil then
...
function ClassroomAssignmentEntity:match_get()
...
function ClassroomAssignmentEntity:stream(action, args, callopts)
...
---@param reqmatch ClassroomAssignmentLoadMatch
---@param ctrl? table
---@return ClassroomAssignment
---@return string? err
function ClassroomAssignmentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ClassroomAssignmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ClassroomAssignmentEntity
