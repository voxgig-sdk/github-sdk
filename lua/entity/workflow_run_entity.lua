...
local WorkflowRunEntity = {}
WorkflowRunEntity.__index = WorkflowRunEntity


function WorkflowRunEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkflowRunEntity)
  self._name = "workflow_run"
  self._client = client
...
function WorkflowRunEntity:get_name()
  return self._name
end


function WorkflowRunEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkflowRunEntity.new(self._client, opts)
end
...
function WorkflowRunEntity:mark_deleted()
  self._deleted = true
end


function WorkflowRunEntity:deleted()
  return true == self._deleted
end


function WorkflowRunEntity:data_set(args)
  if args ~= nil then
...
function WorkflowRunEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkflowRunEntity:match_set(args)
  if args ~= nil then
...
function WorkflowRunEntity:match_get()
...
function WorkflowRunEntity:stream(action, args, callopts)
...
---@param reqmatch WorkflowRunLoadMatch
---@param ctrl? table
---@return WorkflowRun
---@return string? err
function WorkflowRunEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata WorkflowRunCreateData
---@param ctrl? table
---@return WorkflowRun
---@return string? err
function WorkflowRunEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function WorkflowRunEntity:_run_op(ctx, post_done)
  local utility = self._utility
