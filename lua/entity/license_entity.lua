...
local LicenseEntity = {}
LicenseEntity.__index = LicenseEntity


function LicenseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LicenseEntity)
  self._name = "license"
  self._client = client
...
function LicenseEntity:get_name()
  return self._name
end


function LicenseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LicenseEntity.new(self._client, opts)
end
...
function LicenseEntity:mark_deleted()
  self._deleted = true
end


function LicenseEntity:deleted()
  return true == self._deleted
end


function LicenseEntity:data_set(args)
  if args ~= nil then
...
function LicenseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LicenseEntity:match_set(args)
  if args ~= nil then
...
function LicenseEntity:match_get()
...
function LicenseEntity:stream(action, args, callopts)
...
---@param reqmatch LicenseLoadMatch
---@param ctrl? table
---@return License
---@return string? err
function LicenseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch LicenseListMatch
---@param ctrl? table
---@return License[]
---@return string? err
function LicenseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function LicenseEntity:_run_op(ctx, post_done)
  local utility = self._utility
