...
local GistCommitEntity = {}
GistCommitEntity.__index = GistCommitEntity


function GistCommitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GistCommitEntity)
  self._name = "gist_commit"
  self._client = client
...
function GistCommitEntity:get_name()
  return self._name
end


function GistCommitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GistCommitEntity.new(self._client, opts)
end
...
function GistCommitEntity:mark_deleted()
  self._deleted = true
end


function GistCommitEntity:deleted()
  return true == self._deleted
end


function GistCommitEntity:data_set(args)
  if args ~= nil then
...
function GistCommitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GistCommitEntity:match_set(args)
  if args ~= nil then
...
function GistCommitEntity:match_get()
...
function GistCommitEntity:stream(action, args, callopts)
...
---@param reqmatch GistCommitListMatch
---@param ctrl? table
---@return GistCommit[]
---@return string? err
function GistCommitEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GistCommitEntity:_run_op(ctx, post_done)
  local utility = self._utility
