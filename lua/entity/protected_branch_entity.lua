...
local ProtectedBranchEntity = {}
ProtectedBranchEntity.__index = ProtectedBranchEntity


function ProtectedBranchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProtectedBranchEntity)
  self._name = "protected_branch"
  self._client = client
...
function ProtectedBranchEntity:get_name()
  return self._name
end


function ProtectedBranchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProtectedBranchEntity.new(self._client, opts)
end
...
function ProtectedBranchEntity:mark_deleted()
  self._deleted = true
end


function ProtectedBranchEntity:deleted()
  return true == self._deleted
end


function ProtectedBranchEntity:data_set(args)
  if args ~= nil then
...
function ProtectedBranchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProtectedBranchEntity:match_set(args)
  if args ~= nil then
...
function ProtectedBranchEntity:match_get()
...
function ProtectedBranchEntity:stream(action, args, callopts)
...
---@param reqdata ProtectedBranchUpdateData
---@param ctrl? table
---@return ProtectedBranch
---@return string? err
function ProtectedBranchEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function ProtectedBranchEntity:_run_op(ctx, post_done)
  local utility = self._utility
