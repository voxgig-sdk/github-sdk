...
local CustomPropertyEntity = {}
CustomPropertyEntity.__index = CustomPropertyEntity


function CustomPropertyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomPropertyEntity)
  self._name = "custom_property"
  self._client = client
...
function CustomPropertyEntity:get_name()
  return self._name
end


function CustomPropertyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomPropertyEntity.new(self._client, opts)
end
...
function CustomPropertyEntity:mark_deleted()
  self._deleted = true
end


function CustomPropertyEntity:deleted()
  return true == self._deleted
end


function CustomPropertyEntity:data_set(args)
  if args ~= nil then
...
function CustomPropertyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomPropertyEntity:match_set(args)
  if args ~= nil then
...
function CustomPropertyEntity:match_get()
...
function CustomPropertyEntity:stream(action, args, callopts)
...
---@param reqmatch CustomPropertyLoadMatch
---@param ctrl? table
---@return CustomProperty
---@return string? err
function CustomPropertyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CustomPropertyListMatch
---@param ctrl? table
---@return CustomProperty[]
---@return string? err
function CustomPropertyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CustomPropertyUpdateData
---@param ctrl? table
---@return CustomProperty
---@return string? err
function CustomPropertyEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function CustomPropertyEntity:_run_op(ctx, post_done)
  local utility = self._utility
