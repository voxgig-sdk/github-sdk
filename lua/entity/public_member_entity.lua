...
local PublicMemberEntity = {}
PublicMemberEntity.__index = PublicMemberEntity


function PublicMemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicMemberEntity)
  self._name = "public_member"
  self._client = client
...
function PublicMemberEntity:get_name()
  return self._name
end


function PublicMemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicMemberEntity.new(self._client, opts)
end
...
function PublicMemberEntity:mark_deleted()
  self._deleted = true
end


function PublicMemberEntity:deleted()
  return true == self._deleted
end


function PublicMemberEntity:data_set(args)
  if args ~= nil then
...
function PublicMemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicMemberEntity:match_set(args)
  if args ~= nil then
...
function PublicMemberEntity:match_get()
...
function PublicMemberEntity:stream(action, args, callopts)
...
---@param reqmatch PublicMemberListMatch
---@param ctrl? table
---@return PublicMember[]
---@return string? err
function PublicMemberEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PublicMemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
