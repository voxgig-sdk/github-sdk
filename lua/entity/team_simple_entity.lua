...
local TeamSimpleEntity = {}
TeamSimpleEntity.__index = TeamSimpleEntity


function TeamSimpleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TeamSimpleEntity)
  self._name = "team_simple"
  self._client = client
...
function TeamSimpleEntity:get_name()
  return self._name
end


function TeamSimpleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TeamSimpleEntity.new(self._client, opts)
end
...
function TeamSimpleEntity:mark_deleted()
  self._deleted = true
end


function TeamSimpleEntity:deleted()
  return true == self._deleted
end


function TeamSimpleEntity:data_set(args)
  if args ~= nil then
...
function TeamSimpleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TeamSimpleEntity:match_set(args)
  if args ~= nil then
...
function TeamSimpleEntity:match_get()
...
function TeamSimpleEntity:stream(action, args, callopts)
...
---@param reqmatch TeamSimpleListMatch
---@param ctrl? table
---@return TeamSimple[]
---@return string? err
function TeamSimpleEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TeamSimpleEntity:_run_op(ctx, post_done)
  local utility = self._utility
