...
local GitCommitEntity = {}
GitCommitEntity.__index = GitCommitEntity


function GitCommitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitCommitEntity)
...
function GitCommitEntity:get_name()
  return self._name
end


function GitCommitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitCommitEntity.new(self._client, opts)
end
...
function GitCommitEntity:mark_deleted()
  self._deleted = true
end


function GitCommitEntity:deleted()
  return true == self._deleted
end


function GitCommitEntity:data_set(args)
  if args ~= nil then
...
function GitCommitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitCommitEntity:match_set(args)
  if args ~= nil then
...
function GitCommitEntity:match_get()
...
function GitCommitEntity:stream(action, args, callopts)
...
---@param reqmatch GitCommitLoadMatch
---@param ctrl? table
---@return GitCommit
---@return string? err
function GitCommitEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GitCommitCreateData
---@param ctrl? table
---@return GitCommit
---@return string? err
function GitCommitEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function GitCommitEntity:_run_op(ctx, post_done)
  local utility = self._utility
