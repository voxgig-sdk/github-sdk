...
local StatusCheckPolicyEntity = {}
StatusCheckPolicyEntity.__index = StatusCheckPolicyEntity


function StatusCheckPolicyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StatusCheckPolicyEntity)
  self._name = "status_check_policy"
  self._client = client
...
function StatusCheckPolicyEntity:get_name()
  return self._name
end


function StatusCheckPolicyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StatusCheckPolicyEntity.new(self._client, opts)
end
...
function StatusCheckPolicyEntity:mark_deleted()
  self._deleted = true
end


function StatusCheckPolicyEntity:deleted()
  return true == self._deleted
end


function StatusCheckPolicyEntity:data_set(args)
  if args ~= nil then
...
function StatusCheckPolicyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StatusCheckPolicyEntity:match_set(args)
  if args ~= nil then
...
function StatusCheckPolicyEntity:match_get()
...
function StatusCheckPolicyEntity:stream(action, args, callopts)
...
---@param reqmatch StatusCheckPolicyListMatch
---@param ctrl? table
---@return StatusCheckPolicy[]
---@return string? err
function StatusCheckPolicyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata StatusCheckPolicyUpdateData
---@param ctrl? table
---@return StatusCheckPolicy
---@return string? err
function StatusCheckPolicyEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function StatusCheckPolicyEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return StatusCheckPolicyEntity
