...
local MarkdownEntity = {}
MarkdownEntity.__index = MarkdownEntity


function MarkdownEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarkdownEntity)
  self._name = "markdown"
  self._client = client
...
function MarkdownEntity:get_name()
  return self._name
end


function MarkdownEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarkdownEntity.new(self._client, opts)
end
...
function MarkdownEntity:mark_deleted()
  self._deleted = true
end


function MarkdownEntity:deleted()
  return true == self._deleted
end


function MarkdownEntity:data_set(args)
  if args ~= nil then
...
function MarkdownEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarkdownEntity:match_set(args)
  if args ~= nil then
...
function MarkdownEntity:match_get()
...
function MarkdownEntity:stream(action, args, callopts)
...
---@param reqdata MarkdownCreateData
---@param ctrl? table
---@return Markdown
---@return string? err
function MarkdownEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MarkdownEntity:_run_op(ctx, post_done)
  local utility = self._utility
