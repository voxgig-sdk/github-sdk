...
local DependencyGraphEntity = {}
DependencyGraphEntity.__index = DependencyGraphEntity


function DependencyGraphEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependencyGraphEntity)
  self._name = "dependency_graph"
  self._client = client
...
function DependencyGraphEntity:get_name()
  return self._name
end


function DependencyGraphEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependencyGraphEntity.new(self._client, opts)
end
...
function DependencyGraphEntity:mark_deleted()
  self._deleted = true
end


function DependencyGraphEntity:deleted()
  return true == self._deleted
end


function DependencyGraphEntity:data_set(args)
  if args ~= nil then
...
function DependencyGraphEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependencyGraphEntity:match_set(args)
  if args ~= nil then
...
function DependencyGraphEntity:match_get()
...
function DependencyGraphEntity:stream(action, args, callopts)
...
---@param reqdata DependencyGraphCreateData
---@param ctrl? table
---@return DependencyGraph
---@return string? err
function DependencyGraphEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DependencyGraphEntity:_run_op(ctx, post_done)
  local utility = self._utility
