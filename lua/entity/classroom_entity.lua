...
local ClassroomEntity = {}
ClassroomEntity.__index = ClassroomEntity


function ClassroomEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClassroomEntity)
  self._name = "classroom"
  self._client = client
...
function ClassroomEntity:get_name()
  return self._name
end


function ClassroomEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClassroomEntity.new(self._client, opts)
end
...
function ClassroomEntity:mark_deleted()
  self._deleted = true
end


function ClassroomEntity:deleted()
  return true == self._deleted
end


function ClassroomEntity:data_set(args)
  if args ~= nil then
...
function ClassroomEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClassroomEntity:match_set(args)
  if args ~= nil then
...
function ClassroomEntity:match_get()
...
function ClassroomEntity:stream(action, args, callopts)
...
---@param reqmatch ClassroomLoadMatch
---@param ctrl? table
---@return Classroom
---@return string? err
function ClassroomEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ClassroomListMatch
---@param ctrl? table
---@return Classroom[]
---@return string? err
function ClassroomEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ClassroomEntity:_run_op(ctx, post_done)
  local utility = self._utility
