...
local MarketplacePurchaseEntity = {}
MarketplacePurchaseEntity.__index = MarketplacePurchaseEntity


function MarketplacePurchaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MarketplacePurchaseEntity)
  self._name = "marketplace_purchase"
  self._client = client
...
function MarketplacePurchaseEntity:get_name()
  return self._name
end


function MarketplacePurchaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MarketplacePurchaseEntity.new(self._client, opts)
end
...
function MarketplacePurchaseEntity:mark_deleted()
  self._deleted = true
end


function MarketplacePurchaseEntity:deleted()
  return true == self._deleted
end


function MarketplacePurchaseEntity:data_set(args)
  if args ~= nil then
...
function MarketplacePurchaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MarketplacePurchaseEntity:match_set(args)
  if args ~= nil then
...
function MarketplacePurchaseEntity:match_get()
...
function MarketplacePurchaseEntity:stream(action, args, callopts)
...
---@param reqmatch MarketplacePurchaseLoadMatch
---@param ctrl? table
---@return MarketplacePurchase
---@return string? err
function MarketplacePurchaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch MarketplacePurchaseListMatch
---@param ctrl? table
---@return MarketplacePurchase[]
---@return string? err
function MarketplacePurchaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function MarketplacePurchaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
