...
local FileCommitEntity = {}
FileCommitEntity.__index = FileCommitEntity


function FileCommitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FileCommitEntity)
  self._name = "file_commit"
  self._client = client
...
function FileCommitEntity:get_name()
  return self._name
end


function FileCommitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FileCommitEntity.new(self._client, opts)
end
...
function FileCommitEntity:mark_deleted()
  self._deleted = true
end


function FileCommitEntity:deleted()
  return true == self._deleted
end


function FileCommitEntity:data_set(args)
  if args ~= nil then
...
function FileCommitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FileCommitEntity:match_set(args)
  if args ~= nil then
...
function FileCommitEntity:match_get()
...
function FileCommitEntity:stream(action, args, callopts)
...
---@param reqdata FileCommitUpdateData
---@param ctrl? table
---@return FileCommit
---@return string? err
function FileCommitEntity:update(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch FileCommitRemoveMatch
---@param ctrl? table
---@return FileCommit
---@return string? err
function FileCommitEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function FileCommitEntity:_run_op(ctx, post_done)
  local utility = self._utility
