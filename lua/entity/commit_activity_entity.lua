...
local CommitActivityEntity = {}
CommitActivityEntity.__index = CommitActivityEntity


function CommitActivityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CommitActivityEntity)
  self._name = "commit_activity"
  self._client = client
...
function CommitActivityEntity:get_name()
  return self._name
end


function CommitActivityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CommitActivityEntity.new(self._client, opts)
end
...
function CommitActivityEntity:mark_deleted()
  self._deleted = true
end


function CommitActivityEntity:deleted()
  return true == self._deleted
end


function CommitActivityEntity:data_set(args)
  if args ~= nil then
...
function CommitActivityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CommitActivityEntity:match_set(args)
  if args ~= nil then
...
function CommitActivityEntity:match_get()
...
function CommitActivityEntity:stream(action, args, callopts)
...
---@param reqmatch CommitActivityListMatch
---@param ctrl? table
---@return CommitActivity[]
---@return string? err
function CommitActivityEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CommitActivityEntity:_run_op(ctx, post_done)
  local utility = self._utility
