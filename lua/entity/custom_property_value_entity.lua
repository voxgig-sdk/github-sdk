...
local CustomPropertyValueEntity = {}
CustomPropertyValueEntity.__index = CustomPropertyValueEntity


function CustomPropertyValueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomPropertyValueEntity)
  self._name = "custom_property_value"
  self._client = client
...
function CustomPropertyValueEntity:get_name()
  return self._name
end


function CustomPropertyValueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomPropertyValueEntity.new(self._client, opts)
end
...
function CustomPropertyValueEntity:mark_deleted()
  self._deleted = true
end


function CustomPropertyValueEntity:deleted()
  return true == self._deleted
end


function CustomPropertyValueEntity:data_set(args)
  if args ~= nil then
...
function CustomPropertyValueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomPropertyValueEntity:match_set(args)
  if args ~= nil then
...
function CustomPropertyValueEntity:match_get()
...
function CustomPropertyValueEntity:stream(action, args, callopts)
...
---@param reqmatch CustomPropertyValueListMatch
---@param ctrl? table
---@return CustomPropertyValue[]
---@return string? err
function CustomPropertyValueEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CustomPropertyValueEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CustomPropertyValueEntity
