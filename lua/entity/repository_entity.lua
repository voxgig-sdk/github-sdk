...
local RepositoryEntity = {}
RepositoryEntity.__index = RepositoryEntity


function RepositoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepositoryEntity)
...
function RepositoryEntity:get_name()
  return self._name
end


function RepositoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepositoryEntity.new(self._client, opts)
end
...
function RepositoryEntity:mark_deleted()
  self._deleted = true
end


function RepositoryEntity:deleted()
  return true == self._deleted
end


function RepositoryEntity:data_set(args)
  if args ~= nil then
...
function RepositoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepositoryEntity:match_set(args)
  if args ~= nil then
...
function RepositoryEntity:match_get()
...
function RepositoryEntity:stream(action, args, callopts)
...
---@param reqmatch RepositoryListMatch
---@param ctrl? table
---@return Repository[]
---@return string? err
function RepositoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RepositoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
