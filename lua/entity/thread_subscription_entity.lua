...
local ThreadSubscriptionEntity = {}
ThreadSubscriptionEntity.__index = ThreadSubscriptionEntity


function ThreadSubscriptionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ThreadSubscriptionEntity)
  self._name = "thread_subscription"
  self._client = client
...
function ThreadSubscriptionEntity:get_name()
  return self._name
end


function ThreadSubscriptionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ThreadSubscriptionEntity.new(self._client, opts)
end
...
function ThreadSubscriptionEntity:mark_deleted()
  self._deleted = true
end


function ThreadSubscriptionEntity:deleted()
  return true == self._deleted
end


function ThreadSubscriptionEntity:data_set(args)
  if args ~= nil then
...
function ThreadSubscriptionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ThreadSubscriptionEntity:match_set(args)
  if args ~= nil then
...
function ThreadSubscriptionEntity:match_get()
...
function ThreadSubscriptionEntity:stream(action, args, callopts)
...
---@param reqmatch ThreadSubscriptionLoadMatch
---@param ctrl? table
---@return ThreadSubscription
---@return string? err
function ThreadSubscriptionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ThreadSubscriptionUpdateData
---@param ctrl? table
---@return ThreadSubscription
---@return string? err
function ThreadSubscriptionEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ThreadSubscriptionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ThreadSubscriptionEntity
