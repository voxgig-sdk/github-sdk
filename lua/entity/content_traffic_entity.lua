...
local ContentTrafficEntity = {}
ContentTrafficEntity.__index = ContentTrafficEntity


function ContentTrafficEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContentTrafficEntity)
  self._name = "content_traffic"
  self._client = client
...
function ContentTrafficEntity:get_name()
  return self._name
end


function ContentTrafficEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContentTrafficEntity.new(self._client, opts)
end
...
function ContentTrafficEntity:mark_deleted()
  self._deleted = true
end


function ContentTrafficEntity:deleted()
  return true == self._deleted
end


function ContentTrafficEntity:data_set(args)
  if args ~= nil then
...
function ContentTrafficEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContentTrafficEntity:match_set(args)
  if args ~= nil then
...
function ContentTrafficEntity:match_get()
...
function ContentTrafficEntity:stream(action, args, callopts)
...
---@param reqmatch ContentTrafficListMatch
---@param ctrl? table
---@return ContentTraffic[]
---@return string? err
function ContentTrafficEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ContentTrafficEntity:_run_op(ctx, post_done)
  local utility = self._utility
