...
local ReviewCommentEntity = {}
ReviewCommentEntity.__index = ReviewCommentEntity


function ReviewCommentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReviewCommentEntity)
  self._name = "review_comment"
  self._client = client
...
function ReviewCommentEntity:get_name()
  return self._name
end


function ReviewCommentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReviewCommentEntity.new(self._client, opts)
end
...
function ReviewCommentEntity:mark_deleted()
  self._deleted = true
end


function ReviewCommentEntity:deleted()
  return true == self._deleted
end


function ReviewCommentEntity:data_set(args)
  if args ~= nil then
...
function ReviewCommentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReviewCommentEntity:match_set(args)
  if args ~= nil then
...
function ReviewCommentEntity:match_get()
...
function ReviewCommentEntity:stream(action, args, callopts)
...
---@param reqmatch ReviewCommentListMatch
---@param ctrl? table
---@return ReviewComment[]
---@return string? err
function ReviewCommentEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ReviewCommentEntity:_run_op(ctx, post_done)
  local utility = self._utility
