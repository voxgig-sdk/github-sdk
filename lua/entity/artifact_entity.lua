...
local ArtifactEntity = {}
ArtifactEntity.__index = ArtifactEntity


function ArtifactEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArtifactEntity)
  self._name = "artifact"
  self._client = client
...
function ArtifactEntity:get_name()
  return self._name
end


function ArtifactEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArtifactEntity.new(self._client, opts)
end
...
function ArtifactEntity:mark_deleted()
  self._deleted = true
end


function ArtifactEntity:deleted()
  return true == self._deleted
end


function ArtifactEntity:data_set(args)
  if args ~= nil then
...
function ArtifactEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArtifactEntity:match_set(args)
  if args ~= nil then
...
function ArtifactEntity:match_get()
...
function ArtifactEntity:stream(action, args, callopts)
...
---@param reqmatch ArtifactLoadMatch
---@param ctrl? table
---@return Artifact
---@return string? err
function ArtifactEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ArtifactEntity:_run_op(ctx, post_done)
  local utility = self._utility
