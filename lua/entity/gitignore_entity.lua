...
local GitignoreEntity = {}
GitignoreEntity.__index = GitignoreEntity


function GitignoreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitignoreEntity)
  self._name = "gitignore"
  self._client = client
...
function GitignoreEntity:get_name()
  return self._name
end


function GitignoreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitignoreEntity.new(self._client, opts)
end
...
function GitignoreEntity:mark_deleted()
  self._deleted = true
end


function GitignoreEntity:deleted()
  return true == self._deleted
end


function GitignoreEntity:data_set(args)
  if args ~= nil then
...
function GitignoreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitignoreEntity:match_set(args)
  if args ~= nil then
...
function GitignoreEntity:match_get()
...
function GitignoreEntity:stream(action, args, callopts)
...
---@param reqmatch GitignoreListMatch
---@param ctrl? table
---@return Gitignore[]
---@return string? err
function GitignoreEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GitignoreEntity:_run_op(ctx, post_done)
  local utility = self._utility
