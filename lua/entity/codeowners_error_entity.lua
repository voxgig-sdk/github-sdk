...
local CodeownersErrorEntity = {}
CodeownersErrorEntity.__index = CodeownersErrorEntity


function CodeownersErrorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeownersErrorEntity)
  self._name = "codeowners_error"
  self._client = client
...
function CodeownersErrorEntity:get_name()
  return self._name
end


function CodeownersErrorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeownersErrorEntity.new(self._client, opts)
end
...
function CodeownersErrorEntity:mark_deleted()
  self._deleted = true
end


function CodeownersErrorEntity:deleted()
  return true == self._deleted
end


function CodeownersErrorEntity:data_set(args)
  if args ~= nil then
...
function CodeownersErrorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeownersErrorEntity:match_set(args)
  if args ~= nil then
...
function CodeownersErrorEntity:match_get()
...
function CodeownersErrorEntity:stream(action, args, callopts)
...
---@param reqmatch CodeownersErrorListMatch
---@param ctrl? table
---@return CodeownersError[]
---@return string? err
function CodeownersErrorEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CodeownersErrorEntity:_run_op(ctx, post_done)
  local utility = self._utility
