...
local RunnerApplicationEntity = {}
RunnerApplicationEntity.__index = RunnerApplicationEntity


function RunnerApplicationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RunnerApplicationEntity)
  self._name = "runner_application"
  self._client = client
...
function RunnerApplicationEntity:get_name()
  return self._name
end


function RunnerApplicationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RunnerApplicationEntity.new(self._client, opts)
end
...
function RunnerApplicationEntity:mark_deleted()
  self._deleted = true
end


function RunnerApplicationEntity:deleted()
  return true == self._deleted
end


function RunnerApplicationEntity:data_set(args)
  if args ~= nil then
...
function RunnerApplicationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RunnerApplicationEntity:match_set(args)
  if args ~= nil then
...
function RunnerApplicationEntity:match_get()
...
function RunnerApplicationEntity:stream(action, args, callopts)
...
---@param reqmatch RunnerApplicationListMatch
---@param ctrl? table
---@return RunnerApplication[]
---@return string? err
function RunnerApplicationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RunnerApplicationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return RunnerApplicationEntity
