...
local BlobEntity = {}
BlobEntity.__index = BlobEntity


function BlobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BlobEntity)
  self._name = "blob"
  self._client = client
...
function BlobEntity:get_name()
  return self._name
end


function BlobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BlobEntity.new(self._client, opts)
end
...
function BlobEntity:mark_deleted()
  self._deleted = true
end


function BlobEntity:deleted()
  return true == self._deleted
end


function BlobEntity:data_set(args)
  if args ~= nil then
...
function BlobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BlobEntity:match_set(args)
  if args ~= nil then
...
function BlobEntity:stream(action, args, callopts)
...
---@param reqmatch BlobLoadMatch
---@param ctrl? table
---@return Blob
---@return string? err
function BlobEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BlobEntity:_run_op(ctx, post_done)
  local utility = self._utility
