...
local GitRefEntity = {}
GitRefEntity.__index = GitRefEntity


function GitRefEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GitRefEntity)
  self._name = "git_ref"
  self._client = client
...
function GitRefEntity:get_name()
  return self._name
end


function GitRefEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GitRefEntity.new(self._client, opts)
end
...
function GitRefEntity:mark_deleted()
  self._deleted = true
end


function GitRefEntity:deleted()
  return true == self._deleted
end


function GitRefEntity:data_set(args)
  if args ~= nil then
...
function GitRefEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GitRefEntity:match_set(args)
  if args ~= nil then
...
function GitRefEntity:match_get()
...
function GitRefEntity:stream(action, args, callopts)
...
---@param reqmatch GitRefLoadMatch
---@param ctrl? table
---@return GitRef
---@return string? err
function GitRefEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GitRefCreateData
---@param ctrl? table
---@return GitRef
---@return string? err
function GitRefEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata GitRefUpdateData
---@param ctrl? table
---@return GitRef
---@return string? err
function GitRefEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function GitRefEntity:_run_op(ctx, post_done)
  local utility = self._utility
