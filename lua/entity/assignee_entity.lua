...
local AssigneeEntity = {}
AssigneeEntity.__index = AssigneeEntity


function AssigneeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AssigneeEntity)
  self._name = "assignee"
  self._client = client
...
function AssigneeEntity:get_name()
  return self._name
end


function AssigneeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AssigneeEntity.new(self._client, opts)
end
...
function AssigneeEntity:mark_deleted()
  self._deleted = true
end


function AssigneeEntity:deleted()
  return true == self._deleted
end


function AssigneeEntity:data_set(args)
  if args ~= nil then
...
function AssigneeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AssigneeEntity:match_set(args)
  if args ~= nil then
...
function AssigneeEntity:match_get()
...
function AssigneeEntity:stream(action, args, callopts)
...
---@param reqmatch AssigneeListMatch
---@param ctrl? table
---@return Assignee[]
---@return string? err
function AssigneeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AssigneeEntity:_run_op(ctx, post_done)
  local utility = self._utility
