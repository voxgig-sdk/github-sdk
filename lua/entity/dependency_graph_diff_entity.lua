...
local DependencyGraphDiffEntity = {}
DependencyGraphDiffEntity.__index = DependencyGraphDiffEntity


function DependencyGraphDiffEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependencyGraphDiffEntity)
  self._name = "dependency_graph_diff"
  self._client = client
...
function DependencyGraphDiffEntity:get_name()
  return self._name
end


function DependencyGraphDiffEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependencyGraphDiffEntity.new(self._client, opts)
end
...
function DependencyGraphDiffEntity:mark_deleted()
  self._deleted = true
end


function DependencyGraphDiffEntity:deleted()
  return true == self._deleted
end


function DependencyGraphDiffEntity:data_set(args)
  if args ~= nil then
...
function DependencyGraphDiffEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependencyGraphDiffEntity:match_set(args)
  if args ~= nil then
...
function DependencyGraphDiffEntity:match_get()
...
function DependencyGraphDiffEntity:stream(action, args, callopts)
...
---@param reqmatch DependencyGraphDiffLoadMatch
---@param ctrl? table
---@return DependencyGraphDiff
---@return string? err
function DependencyGraphDiffEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DependencyGraphDiffEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DependencyGraphDiffEntity
