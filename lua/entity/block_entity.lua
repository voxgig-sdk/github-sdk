...
local BlockEntity = {}
BlockEntity.__index = BlockEntity


function BlockEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BlockEntity)
  self._name = "block"
  self._client = client
...
function BlockEntity:get_name()
  return self._name
end


function BlockEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BlockEntity.new(self._client, opts)
end
...
function BlockEntity:mark_deleted()
  self._deleted = true
end


function BlockEntity:deleted()
  return true == self._deleted
end


function BlockEntity:data_set(args)
  if args ~= nil then
...
function BlockEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BlockEntity:match_set(args)
  if args ~= nil then
...
function BlockEntity:match_get()
...
function BlockEntity:stream(action, args, callopts)
...
---@param reqmatch BlockListMatch
---@param ctrl? table
---@return Block[]
---@return string? err
function BlockEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BlockEntity:_run_op(ctx, post_done)
  local utility = self._utility
