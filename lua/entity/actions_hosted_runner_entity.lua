...
local ActionsHostedRunnerEntity = {}
ActionsHostedRunnerEntity.__index = ActionsHostedRunnerEntity


function ActionsHostedRunnerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsHostedRunnerEntity)
  self._name = "actions_hosted_runner"
  self._client = client
...
function ActionsHostedRunnerEntity:get_name()
  return self._name
end


function ActionsHostedRunnerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsHostedRunnerEntity.new(self._client, opts)
end
...
function ActionsHostedRunnerEntity:mark_deleted()
  self._deleted = true
end


function ActionsHostedRunnerEntity:deleted()
  return true == self._deleted
end


function ActionsHostedRunnerEntity:data_set(args)
  if args ~= nil then
...
function ActionsHostedRunnerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsHostedRunnerEntity:match_set(args)
  if args ~= nil then
...
function ActionsHostedRunnerEntity:match_get()
...
function ActionsHostedRunnerEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsHostedRunnerLoadMatch
---@param ctrl? table
---@return ActionsHostedRunner
---@return string? err
function ActionsHostedRunnerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ActionsHostedRunnerCreateData
---@param ctrl? table
---@return ActionsHostedRunner
---@return string? err
function ActionsHostedRunnerEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata ActionsHostedRunnerUpdateData
---@param ctrl? table
---@return ActionsHostedRunner
---@return string? err
function ActionsHostedRunnerEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ActionsHostedRunnerEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsHostedRunnerEntity
