-- Github SDK ActionsHostedRunnerLimit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsHostedRunnerLimitEntity = {}
ActionsHostedRunnerLimitEntity.__index = ActionsHostedRunnerLimitEntity


function ActionsHostedRunnerLimitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsHostedRunnerLimitEntity)
  self._name = "actions_hosted_runner_limit"
  self._client = client
...
function ActionsHostedRunnerLimitEntity:get_name()
  return self._name
end


function ActionsHostedRunnerLimitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsHostedRunnerLimitEntity.new(self._client, opts)
end
...
function ActionsHostedRunnerLimitEntity:mark_deleted()
  self._deleted = true
end


function ActionsHostedRunnerLimitEntity:deleted()
  return true == self._deleted
end


function ActionsHostedRunnerLimitEntity:data_set(args)
  if args ~= nil then
...
function ActionsHostedRunnerLimitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsHostedRunnerLimitEntity:match_set(args)
  if args ~= nil then
...
function ActionsHostedRunnerLimitEntity:match_get()
...
function ActionsHostedRunnerLimitEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsHostedRunnerLimitLoadMatch
---@param ctrl? table
---@return ActionsHostedRunnerLimit
---@return string? err
function ActionsHostedRunnerLimitEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsHostedRunnerLimitEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsHostedRunnerLimitEntity
