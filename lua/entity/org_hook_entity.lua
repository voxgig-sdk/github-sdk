...
local OrgHookEntity = {}
OrgHookEntity.__index = OrgHookEntity


function OrgHookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrgHookEntity)
  self._name = "org_hook"
  self._client = client
...
function OrgHookEntity:get_name()
  return self._name
end


function OrgHookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrgHookEntity.new(self._client, opts)
end
...
function OrgHookEntity:mark_deleted()
  self._deleted = true
end


function OrgHookEntity:deleted()
  return true == self._deleted
end


function OrgHookEntity:data_set(args)
  if args ~= nil then
...
function OrgHookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrgHookEntity:match_set(args)
  if args ~= nil then
...
function OrgHookEntity:match_get()
...
function OrgHookEntity:stream(action, args, callopts)
...
---@param reqmatch OrgHookLoadMatch
---@param ctrl? table
---@return OrgHook
---@return string? err
function OrgHookEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch OrgHookListMatch
---@param ctrl? table
---@return OrgHook[]
---@return string? err
function OrgHookEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata OrgHookCreateData
---@param ctrl? table
---@return OrgHook
---@return string? err
function OrgHookEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata OrgHookUpdateData
---@param ctrl? table
---@return OrgHook
---@return string? err
function OrgHookEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function OrgHookEntity:_run_op(ctx, post_done)
  local utility = self._utility
