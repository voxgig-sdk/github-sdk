...
local GistSimpleEntity = {}
GistSimpleEntity.__index = GistSimpleEntity


function GistSimpleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GistSimpleEntity)
  self._name = "gist_simple"
  self._client = client
...
function GistSimpleEntity:get_name()
  return self._name
end


function GistSimpleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GistSimpleEntity.new(self._client, opts)
end
...
function GistSimpleEntity:mark_deleted()
  self._deleted = true
end


function GistSimpleEntity:deleted()
  return true == self._deleted
end


function GistSimpleEntity:data_set(args)
  if args ~= nil then
...
function GistSimpleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GistSimpleEntity:match_set(args)
  if args ~= nil then
...
function GistSimpleEntity:match_get()
...
function GistSimpleEntity:stream(action, args, callopts)
...
---@param reqmatch GistSimpleListMatch
---@param ctrl? table
---@return GistSimple[]
---@return string? err
function GistSimpleEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GistSimpleEntity:_run_op(ctx, post_done)
  local utility = self._utility
