-- Github SDK ActionsForkPrWorkflowsPrivateRepo entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsForkPrWorkflowsPrivateRepoEntity = {}
ActionsForkPrWorkflowsPrivateRepoEntity.__index = ActionsForkPrWorkflowsPrivateRepoEntity


function ActionsForkPrWorkflowsPrivateRepoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsForkPrWorkflowsPrivateRepoEntity)
  self._name = "actions_fork_pr_workflows_private_repo"
  self._client = client
...
function ActionsForkPrWorkflowsPrivateRepoEntity:get_name()
  return self._name
end


function ActionsForkPrWorkflowsPrivateRepoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsForkPrWorkflowsPrivateRepoEntity.new(self._client, opts)
end
...
function ActionsForkPrWorkflowsPrivateRepoEntity:mark_deleted()
  self._deleted = true
end


function ActionsForkPrWorkflowsPrivateRepoEntity:deleted()
  return true == self._deleted
end


function ActionsForkPrWorkflowsPrivateRepoEntity:data_set(args)
  if args ~= nil then
...
function ActionsForkPrWorkflowsPrivateRepoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsForkPrWorkflowsPrivateRepoEntity:match_set(args)
  if args ~= nil then
...
function ActionsForkPrWorkflowsPrivateRepoEntity:match_get()
...
function ActionsForkPrWorkflowsPrivateRepoEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsForkPrWorkflowsPrivateRepoLoadMatch
---@param ctrl? table
---@return ActionsForkPrWorkflowsPrivateRepo
---@return string? err
function ActionsForkPrWorkflowsPrivateRepoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsForkPrWorkflowsPrivateRepoEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsForkPrWorkflowsPrivateRepoEntity
