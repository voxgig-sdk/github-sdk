...
local FullRepositoryEntity = {}
FullRepositoryEntity.__index = FullRepositoryEntity


function FullRepositoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FullRepositoryEntity)
  self._name = "full_repository"
  self._client = client
...
function FullRepositoryEntity:get_name()
  return self._name
end


function FullRepositoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FullRepositoryEntity.new(self._client, opts)
end
...
function FullRepositoryEntity:mark_deleted()
  self._deleted = true
end


function FullRepositoryEntity:deleted()
  return true == self._deleted
end


function FullRepositoryEntity:data_set(args)
  if args ~= nil then
...
function FullRepositoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FullRepositoryEntity:match_set(args)
  if args ~= nil then
...
function FullRepositoryEntity:match_get()
...
function FullRepositoryEntity:stream(action, args, callopts)
...
---@param reqmatch FullRepositoryLoadMatch
---@param ctrl? table
---@return FullRepository
---@return string? err
function FullRepositoryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata FullRepositoryCreateData
---@param ctrl? table
---@return FullRepository
---@return string? err
function FullRepositoryEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata FullRepositoryUpdateData
---@param ctrl? table
---@return FullRepository
---@return string? err
function FullRepositoryEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function FullRepositoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
