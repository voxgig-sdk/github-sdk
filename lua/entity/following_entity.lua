...
local FollowingEntity = {}
FollowingEntity.__index = FollowingEntity


function FollowingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FollowingEntity)
  self._name = "following"
  self._client = client
...
function FollowingEntity:get_name()
  return self._name
end


function FollowingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FollowingEntity.new(self._client, opts)
end
...
function FollowingEntity:mark_deleted()
  self._deleted = true
end


function FollowingEntity:deleted()
  return true == self._deleted
end


function FollowingEntity:data_set(args)
  if args ~= nil then
...
function FollowingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FollowingEntity:match_set(args)
  if args ~= nil then
...
function FollowingEntity:match_get()
...
function FollowingEntity:stream(action, args, callopts)
...
---@param reqmatch FollowingListMatch
---@param ctrl? table
---@return Following[]
---@return string? err
function FollowingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function FollowingEntity:_run_op(ctx, post_done)
  local utility = self._utility
