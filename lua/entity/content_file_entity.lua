...
local ContentFileEntity = {}
ContentFileEntity.__index = ContentFileEntity


function ContentFileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContentFileEntity)
  self._name = "content_file"
  self._client = client
...
function ContentFileEntity:get_name()
  return self._name
end


function ContentFileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContentFileEntity.new(self._client, opts)
end
...
function ContentFileEntity:mark_deleted()
  self._deleted = true
end


function ContentFileEntity:deleted()
  return true == self._deleted
end


function ContentFileEntity:data_set(args)
  if args ~= nil then
...
function ContentFileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContentFileEntity:match_set(args)
  if args ~= nil then
...
function ContentFileEntity:match_get()
...
function ContentFileEntity:stream(action, args, callopts)
...
---@param reqmatch ContentFileLoadMatch
---@param ctrl? table
---@return ContentFile
---@return string? err
function ContentFileEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ContentFileEntity:_run_op(ctx, post_done)
  local utility = self._utility
