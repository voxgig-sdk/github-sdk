...
local CodeFrequencyStatEntity = {}
CodeFrequencyStatEntity.__index = CodeFrequencyStatEntity


function CodeFrequencyStatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CodeFrequencyStatEntity)
  self._name = "code_frequency_stat"
  self._client = client
...
function CodeFrequencyStatEntity:get_name()
  return self._name
end


function CodeFrequencyStatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CodeFrequencyStatEntity.new(self._client, opts)
end
...
function CodeFrequencyStatEntity:mark_deleted()
  self._deleted = true
end


function CodeFrequencyStatEntity:deleted()
  return true == self._deleted
end


function CodeFrequencyStatEntity:data_set(args)
  if args ~= nil then
...
function CodeFrequencyStatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CodeFrequencyStatEntity:match_set(args)
  if args ~= nil then
...
function CodeFrequencyStatEntity:match_get()
...
function CodeFrequencyStatEntity:stream(action, args, callopts)
...
---@param reqmatch CodeFrequencyStatListMatch
---@param ctrl? table
---@return CodeFrequencyStat[]
---@return string? err
function CodeFrequencyStatEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CodeFrequencyStatEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CodeFrequencyStatEntity
