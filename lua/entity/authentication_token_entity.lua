...
local AuthenticationTokenEntity = {}
AuthenticationTokenEntity.__index = AuthenticationTokenEntity


function AuthenticationTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthenticationTokenEntity)
  self._name = "authentication_token"
  self._client = client
...
function AuthenticationTokenEntity:get_name()
  return self._name
end


function AuthenticationTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthenticationTokenEntity.new(self._client, opts)
end
...
function AuthenticationTokenEntity:mark_deleted()
  self._deleted = true
end


function AuthenticationTokenEntity:deleted()
  return true == self._deleted
end


function AuthenticationTokenEntity:data_set(args)
  if args ~= nil then
...
function AuthenticationTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthenticationTokenEntity:match_set(args)
  if args ~= nil then
...
function AuthenticationTokenEntity:match_get()
...
function AuthenticationTokenEntity:stream(action, args, callopts)
...
---@param reqdata AuthenticationTokenCreateData
---@param ctrl? table
---@return AuthenticationToken
---@return string? err
function AuthenticationTokenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AuthenticationTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
