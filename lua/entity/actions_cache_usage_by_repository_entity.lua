-- Github SDK ActionsCacheUsageByRepository entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsCacheUsageByRepositoryEntity = {}
ActionsCacheUsageByRepositoryEntity.__index = ActionsCacheUsageByRepositoryEntity


function ActionsCacheUsageByRepositoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsCacheUsageByRepositoryEntity)
  self._name = "actions_cache_usage_by_repository"
  self._client = client
...
function ActionsCacheUsageByRepositoryEntity:get_name()
  return self._name
end


function ActionsCacheUsageByRepositoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsCacheUsageByRepositoryEntity.new(self._client, opts)
end
...
function ActionsCacheUsageByRepositoryEntity:mark_deleted()
  self._deleted = true
end


function ActionsCacheUsageByRepositoryEntity:deleted()
  return true == self._deleted
end


function ActionsCacheUsageByRepositoryEntity:data_set(args)
  if args ~= nil then
...
function ActionsCacheUsageByRepositoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsCacheUsageByRepositoryEntity:match_set(args)
  if args ~= nil then
...
function ActionsCacheUsageByRepositoryEntity:match_get()
...
function ActionsCacheUsageByRepositoryEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsCacheUsageByRepositoryLoadMatch
---@param ctrl? table
---@return ActionsCacheUsageByRepository
---@return string? err
function ActionsCacheUsageByRepositoryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsCacheUsageByRepositoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsCacheUsageByRepositoryEntity
