...
local GpgKeyEntity = {}
GpgKeyEntity.__index = GpgKeyEntity


function GpgKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GpgKeyEntity)
  self._name = "gpg_key"
  self._client = client
...
function GpgKeyEntity:get_name()
  return self._name
end


function GpgKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GpgKeyEntity.new(self._client, opts)
end
...
function GpgKeyEntity:mark_deleted()
  self._deleted = true
end


function GpgKeyEntity:deleted()
  return true == self._deleted
end


function GpgKeyEntity:data_set(args)
  if args ~= nil then
...
function GpgKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GpgKeyEntity:match_set(args)
  if args ~= nil then
...
function GpgKeyEntity:match_get()
...
function GpgKeyEntity:stream(action, args, callopts)
...
---@param reqmatch GpgKeyLoadMatch
---@param ctrl? table
---@return GpgKey
---@return string? err
function GpgKeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GpgKeyListMatch
---@param ctrl? table
---@return GpgKey[]
---@return string? err
function GpgKeyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata GpgKeyCreateData
---@param ctrl? table
---@return GpgKey
---@return string? err
function GpgKeyEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function GpgKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
