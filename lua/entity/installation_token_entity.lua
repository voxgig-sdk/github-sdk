...
local InstallationTokenEntity = {}
InstallationTokenEntity.__index = InstallationTokenEntity


function InstallationTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InstallationTokenEntity)
  self._name = "installation_token"
  self._client = client
...
function InstallationTokenEntity:get_name()
  return self._name
end


function InstallationTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InstallationTokenEntity.new(self._client, opts)
end
...
function InstallationTokenEntity:mark_deleted()
  self._deleted = true
end


function InstallationTokenEntity:deleted()
  return true == self._deleted
end


function InstallationTokenEntity:data_set(args)
  if args ~= nil then
...
function InstallationTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InstallationTokenEntity:match_set(args)
  if args ~= nil then
...
function InstallationTokenEntity:match_get()
...
function InstallationTokenEntity:stream(action, args, callopts)
...
---@param reqdata InstallationTokenCreateData
---@param ctrl? table
---@return InstallationToken
---@return string? err
function InstallationTokenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function InstallationTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return InstallationTokenEntity
