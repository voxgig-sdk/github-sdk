...
local HovercardEntity = {}
HovercardEntity.__index = HovercardEntity


function HovercardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HovercardEntity)
  self._name = "hovercard"
  self._client = client
...
function HovercardEntity:get_name()
  return self._name
end


function HovercardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HovercardEntity.new(self._client, opts)
end
...
function HovercardEntity:mark_deleted()
  self._deleted = true
end


function HovercardEntity:deleted()
  return true == self._deleted
end


function HovercardEntity:data_set(args)
  if args ~= nil then
...
function HovercardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HovercardEntity:match_set(args)
  if args ~= nil then
...
function HovercardEntity:match_get()
...
function HovercardEntity:stream(action, args, callopts)
...
---@param reqmatch HovercardListMatch
---@param ctrl? table
---@return Hovercard[]
---@return string? err
function HovercardEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function HovercardEntity:_run_op(ctx, post_done)
  local utility = self._utility
