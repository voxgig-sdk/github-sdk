...
local ReleaseNotesContentEntity = {}
ReleaseNotesContentEntity.__index = ReleaseNotesContentEntity


function ReleaseNotesContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReleaseNotesContentEntity)
  self._name = "release_notes_content"
  self._client = client
...
function ReleaseNotesContentEntity:get_name()
  return self._name
end


function ReleaseNotesContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReleaseNotesContentEntity.new(self._client, opts)
end
...
function ReleaseNotesContentEntity:mark_deleted()
  self._deleted = true
end


function ReleaseNotesContentEntity:deleted()
  return true == self._deleted
end


function ReleaseNotesContentEntity:data_set(args)
  if args ~= nil then
...
function ReleaseNotesContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReleaseNotesContentEntity:match_set(args)
  if args ~= nil then
...
function ReleaseNotesContentEntity:match_get()
...
function ReleaseNotesContentEntity:stream(action, args, callopts)
...
---@param reqdata ReleaseNotesContentCreateData
---@param ctrl? table
---@return ReleaseNotesContent
---@return string? err
function ReleaseNotesContentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ReleaseNotesContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ReleaseNotesContentEntity
