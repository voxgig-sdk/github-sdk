...
local AuthorizationEntity = {}
AuthorizationEntity.__index = AuthorizationEntity


function AuthorizationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthorizationEntity)
  self._name = "authorization"
  self._client = client
...
function AuthorizationEntity:get_name()
  return self._name
end


function AuthorizationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthorizationEntity.new(self._client, opts)
end
...
function AuthorizationEntity:mark_deleted()
  self._deleted = true
end


function AuthorizationEntity:deleted()
  return true == self._deleted
end


function AuthorizationEntity:data_set(args)
  if args ~= nil then
...
function AuthorizationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthorizationEntity:match_set(args)
  if args ~= nil then
...
function AuthorizationEntity:match_get()
...
function AuthorizationEntity:stream(action, args, callopts)
...
---@param reqdata AuthorizationCreateData
---@param ctrl? table
---@return Authorization
---@return string? err
function AuthorizationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata AuthorizationUpdateData
---@param ctrl? table
---@return Authorization
---@return string? err
function AuthorizationEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AuthorizationEntity:_run_op(ctx, post_done)
  local utility = self._utility
