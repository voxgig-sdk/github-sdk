...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Action()` — each with the same small set of operations (`list`, `load`, `create`, `update`, `remove`, `patch`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
for _, item in ipairs(actions) do
  print(item["id"], item["access_level"])
end
```
...
```lua
local custompropertys, err = client:CustomProperty():list()
if err then error(err) end
```
...
local result, err = client:CustomProperty():list()
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local action, err = client:Action():load()
    if err then error(err) end
...
| `clone_url` | `string` |  |
| `code_of_conduct` | `table` | Code Of Conduct |
...
| `head_commit` | `table` | A commit. |
| `head_repository` | `table` | Minimal Repository |
...
| `image_details` | `table` | Provides details of a hosted runner image |
| `inherited` | `boolean` |  |
| `inherited_allows_public_repositories` | `boolean` |  |
| `inputs` | `table` | Input keys and values configured in the workflow file. |
...
| `keys_url` | `string` |  |
| `labels` | `table` | The names of the custom labels to add to the runner. |
| `labels_url` | `string` |  |
| `language` | `string` |  |
...
| `machine_size_details` | `table` | Provides details of a particular machine spec. |
...
| `patterns_allowed` | `table` | Specifies a list of string-matching patterns to allow specific action(s) and reusable workflow(s). |
| `permissions` | `table` |  |
...
| `public_ips` | `table` | The public IP ranges when public IP is enabled for the hosted runners. |
| `pull_requests` | `table` | Pull requests that are open with a `head_sha` or `head_branch` that matches the workflow run. |
| `pulls_url` | `string` |  |
| `pushed_at` | `string` |  |
| `ref` | `string` | The git reference for the workflow. |
| `referenced_workflows` | `table` |  |
| `releases_url` | `string` |  |
| `repository` | `table` | Minimal Repository |
...
| `runners` | `table` | List of runner IDs to add to the runner group. |
| `runners_url` | `string` |  |
| `security_and_analysis` | `table` |  |
| `selected_repositories_url` | `string` | Link to the selected repositories resource for this runner group. |
| `selected_repository_ids` | `table` | The IDs of the repositories that can access the organization variable. |
| `selected_workflows` | `table` | List of workflows the runner group should be allowed to run. |
...
| `total_count` | `number` |  |
| `trees_url` | `string` |  |
| `triggering_actor` | `table` | A GitHub user. |
...
```lua
local action, err = client:Action():load({ archive_format = "archive_format", artifact_id = 1, owner = "owner", repo = "repo" })
```
...
local action, err = client:Action():create({
  ...
  access_level = "example_access_level", -- string
  active_caches_count = 1, -- number
  active_caches_size_in_bytes = 1, -- number
  actor = {}, -- table
  allows_public_repositories = true, -- boolean
  approval_policy = "example_approval_policy", -- string
  archive_download_url = "example_archive_download_url", -- string
  archive_url = "example_archive_url", -- string
  artifacts_url = "example_artifacts_url", -- string
  assignees_url = "example_assignees_url", -- string
  badge_url = "example_badge_url", -- string
  blobs_url = "example_blobs_url", -- string
  branches_url = "example_branches_url", -- string
  busy = true, -- boolean
  cancel_url = "example_cancel_url", -- string
  check_run_url = "example_check_run_url", -- string
  check_suite_url = "example_check_suite_url", -- string
  ...
  conclusion = "example_conclusion", -- string
  ...
  display_title = "example_display_title", -- string
  downloads_url = "example_downloads_url", -- string
  enabled = true, -- boolean
  enabled_repositories = "example_enabled_repositories", -- string
  ...
  head_branch = "example_head_branch", -- string
  head_commit = {}, -- table
  head_repository = {}, -- table
  head_sha = "example_head_sha", -- string
  ...
  os = "example_os", -- string
  owner = {}, -- table
  path = "example_path", -- string
  platform = "example_platform", -- string
  platforms = {}, -- table
  private = true, -- boolean
  public_ip_enabled = true, -- boolean
  pull_requests = {}, -- table
  pulls_url = "example_pulls_url", -- string
  ref = "example_ref", -- string
  releases_url = "example_releases_url", -- string
  repository = {}, -- table
  rerun_url = "example_rerun_url", -- string
  run_id = 1, -- number
  run_number = 1, -- number
  run_url = "example_run_url", -- string
  run_workflows_from_fork_pull_requests = true, -- boolean
  runner_group_id = 1, -- number
  runner_group_name = "example_runner_group_name", -- string
  runner_id = 1, -- number
  runner_name = "example_runner_name", -- string
  runners = {}, -- table
  runners_url = "example_runners_url", -- string
  selected_repository_ids = {}, -- table
  size_gb = 1, -- number
  size_in_bytes = 1, -- number
  ...
  workflow_name = "example_workflow_name", -- string
  workflow_url = "example_workflow_url", -- string
})
```
...
Create an instance: `local actions_artifact_and_log_retention = client:ActionsArtifactAndLogRetention(nil)`
...
```lua
local actions_artifact_and_log_retention, err = client:ActionsArtifactAndLogRetention():load({ org_id = "org_id" })
```


### ActionsCacheList

Create an instance: `local actions_cache_list = client:ActionsCacheList(nil)`
...
```lua
local actions_cache_lists, err = client:ActionsCacheList():list()
```


### ActionsCacheUsageByRepository

Create an instance: `local actions_cache_usage_by_repository = client:ActionsCacheUsageByRepository(nil)`
...
```lua
local actions_cache_usage_by_repository, err = client:ActionsCacheUsageByRepository():load({ owner = "owner", repo = "repo" })
```


### ActionsCacheUsageOrgEnterprise

Create an instance: `local actions_cache_usage_org_enterprise = client:ActionsCacheUsageOrgEnterprise(nil)`
...
```lua
local actions_cache_usage_org_enterprise, err = client:ActionsCacheUsageOrgEnterprise():load({ org_id = "org_id" })
```


### ActionsForkPrContributorApproval

Create an instance: `local actions_fork_pr_contributor_approval = client:ActionsForkPrContributorApproval(nil)`
...
```lua
local actions_fork_pr_contributor_approval, err = client:ActionsForkPrContributorApproval():load({ org_id = "org_id" })
```


### ActionsForkPrWorkflowsPrivateRepo

Create an instance: `local actions_fork_pr_workflows_private_repo = client:ActionsForkPrWorkflowsPrivateRepo(nil)`
...
```lua
local actions_fork_pr_workflows_private_repo, err = client:ActionsForkPrWorkflowsPrivateRepo():load({ org_id = "org_id" })
```


### ActionsGetDefaultWorkflowPermission

Create an instance: `local actions_get_default_workflow_permission = client:ActionsGetDefaultWorkflowPermission(nil)`
...
```lua
local actions_get_default_workflow_permission, err = client:ActionsGetDefaultWorkflowPermission():load({ org_id = "org_id" })
```


### ActionsHostedRunner

Create an instance: `local actions_hosted_runner = client:ActionsHostedRunner(nil)`
...
| `image` | `table` | The image of runner. |
| `image_details` | `table` | Provides details of a hosted runner image |
| `last_active_on` | `string` | The time at which the runner was last used, in ISO 8601 format. |
| `machine_size_details` | `table` | Provides details of a particular machine spec. |
| `maximum_runners` | `number` | The maximum amount of hosted runners. |
...
```lua
local actions_hosted_runner, err = client:ActionsHostedRunner():load({ id = 1, org_id = "org_id" })
```

#### Example: Create

```lua
local actions_hosted_runner, err = client:ActionsHostedRunner():create({
  org_id = "example_org_id", -- string
  id = 1, -- number
  image = {}, -- table
  image_details = {}, -- table
...
Create an instance: `local actions_hosted_runner_limit = client:ActionsHostedRunnerLimit(nil)`
...
```lua
local actions_hosted_runner_limit, err = client:ActionsHostedRunnerLimit():load({ org_id = "org_id" })
```


### ActionsOrganizationPermission

Create an instance: `local actions_organization_permission = client:ActionsOrganizationPermission(nil)`
...
```lua
local actions_organization_permission, err = client:ActionsOrganizationPermission():load({ org_id = "org_id" })
```


### ActionsPublicKey

Create an instance: `local actions_public_key = client:ActionsPublicKey(nil)`
...
```lua
local actions_public_key, err = client:ActionsPublicKey():load({ org_id = "org_id" })
```


### ActionsRepositoryPermission

Create an instance: `local actions_repository_permission = client:ActionsRepositoryPermission(nil)`
...
```lua
local actions_repository_permission, err = client:ActionsRepositoryPermission():load({ owner = "owner", repo = "repo" })
```


### ActionsSecret

Create an instance: `local actions_secret = client:ActionsSecret(nil)`
...
```lua
local actions_secret, err = client:ActionsSecret():load({ id = "actions_secret_id", owner = "owner", repo = "repo" })
```


### ActionsVariable

Create an instance: `local actions_variable = client:ActionsVariable(nil)`
...
```lua
local actions_variable, err = client:ActionsVariable():load({ id = "actions_variable_id", owner = "owner", repo = "repo" })
```


### ActionsWorkflowAccessToRepository

Create an instance: `local actions_workflow_access_to_repository = client:ActionsWorkflowAccessToRepository(nil)`
...
```lua
local actions_workflow_access_to_repository, err = client:ActionsWorkflowAccessToRepository():load({ owner = "owner", repo = "repo" })
```
...
```lua
local activity, err = client:Activity():load({ owner = "owner", repo = "repo" })
```
...
| Field | Type | Description |
| --- | --- | --- |
| `usernames` | `table` | The GitHub user handles to add to the team. |

#### Example: Create

```lua
local add, err = client:Add():create({
  enterprise = "example_enterprise", -- string
...
Create an instance: `local api_insights_route_stat = client:ApiInsightsRouteStat(nil)`
...
```lua
local api_insights_route_stats, err = client:ApiInsightsRouteStat():list()
```


### ApiInsightsSubjectStat

Create an instance: `local api_insights_subject_stat = client:ApiInsightsSubjectStat(nil)`
...
```lua
local api_insights_subject_stats, err = client:ApiInsightsSubjectStat():list()
```


### ApiInsightsSummaryStat

Create an instance: `local api_insights_summary_stat = client:ApiInsightsSummaryStat(nil)`
...
```lua
local api_insights_summary_stat, err = client:ApiInsightsSummaryStat():load({ min_timestamp = "min_timestamp" })
```


### ApiInsightsTimeStat

Create an instance: `local api_insights_time_stat = client:ApiInsightsTimeStat(nil)`
...
```lua
local api_insights_time_stat, err = client:ApiInsightsTimeStat():load({ org_id = "org_id", user_id = "user_id", min_timestamp = "min_timestamp", timestamp_increment = "timestamp_increment" })
```

#### Example: List

```lua
local api_insights_time_stats, err = client:ApiInsightsTimeStat():list()
```


### ApiInsightsUserStat

Create an instance: `local api_insights_user_stat = client:ApiInsightsUserStat(nil)`
...
```lua
local api_insights_user_stat, err = client:ApiInsightsUserStat():load({ id = "api_insights_user_stat_id", org_id = "org_id", min_timestamp = "min_timestamp" })
```


### ApiOverview

Create an instance: `local api_overview = client:ApiOverview(nil)`
...
| `domains` | `table` |  |
| `git` | `table` |  |
| `github_enterprise_importer` | `table` |  |
| `hooks` | `table` |  |
| `importer` | `table` |  |
| `packages` | `table` |  |
| `pages` | `table` |  |
| `ssh_key_fingerprints` | `table` |  |
| `ssh_keys` | `table` |  |
...
```lua
local api_overviews, err = client:ApiOverview():list()
```
...
| `owner` | `table` | A GitHub user. |
| `pem` | `string` |  |
| `permissions` | `table` | The permissions granted to the user access token. |
| `private` | `boolean` | Whether the repository is private or public. |
| `pulls_url` | `string` |  |
| `pushed_at` | `string` |  |
| `releases_url` | `string` |  |
...
| `subscription_url` | `string` |  |
| `suspended_at` | `string` |  |
| `suspended_by` | `table` | A GitHub user. |
| `svn_url` | `string` |  |
| `tags_url` | `string` |  |
...
```lua
local apps, err = client:App():list()
```

#### Example: Create

```lua
local app, err = client:App():create({
  code = "example_code", -- string
  access_tokens_url = "example_access_tokens_url", -- string
  account = "example_account", -- any
  app_id = 1, -- number
  app_slug = "example_app_slug", -- string
  archive_url = "example_archive_url", -- string
  ...
  releases_url = "example_releases_url", -- string
  repositories_url = "example_repositories_url", -- string
  repository_selection = "example_repository_selection", -- string
  single_file_name = "example_single_file_name", -- string
  size = 1, -- number
  ...
  subscribers_url = "example_subscribers_url", -- string
  subscription_url = "example_subscription_url", -- string
  suspended_at = "example_suspended_at", -- string
  suspended_by = {}, -- table
  svn_url = "example_svn_url", -- string
  tags_url = "example_tags_url", -- string
  target_id = 1, -- number
  target_type = "example_target_type", -- string
  teams_url = "example_teams_url", -- string
  trees_url = "example_trees_url", -- string
  url = "example_url", -- string
  watchers = 1, -- number
...
```lua
local artifact, err = client:Artifact():load({ id = 1, owner = "owner", repo = "repo" })
```


### Assignee

Create an instance: `local assignee = client:Assignee(nil)`
...
```lua
local assignees, err = client:Assignee():list()
```


### AuthenticationToken

Create an instance: `local authentication_token = client:AuthenticationToken(nil)`
...
```lua
local authentication_token, err = client:AuthenticationToken():create({
  org_id = "example_org_id", -- string
})
```
...
| `note_url` | `string` |  |
| `permissions` | `table` | The permissions granted to the user access token. |
| `repositories` | `table` | The list of repository names to scope the user access token to. |
| `repository_ids` | `table` | The list of repository IDs to scope the user access token to. |
| `scopes` | `table` | A list of scopes that this authorization is in. |
...
local authorization, err = client:Authorization():create({
  ...
  created_at = "example_created_at", -- string
  expires_at = "example_expires_at", -- string
  fingerprint = "example_fingerprint", -- string
  hashed_token = "example_hashed_token", -- string
  id = 1, -- number
  installation = {}, -- table
  note = "example_note", -- string
  note_url = "example_note_url", -- string
  scopes = {}, -- table
  token = "example_token", -- string
  token_last_eight = "example_token_last_eight", -- string
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
  user = {}, -- table
})
...
Create an instance: `local autolink = client:Autolink(nil)`
...
```lua
local autolink, err = client:Autolink():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local autolinks, err = client:Autolink():list()
```

#### Example: Create

```lua
local autolink, err = client:Autolink():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  id = 1, -- number
  is_alphanumeric = true, -- boolean
  key_prefix = "example_key_prefix", -- string
  url_template = "example_url_template", -- string
})
```
...
Create an instance: `local base_gist = client:BaseGist(nil)`
...
```lua
local base_gists, err = client:BaseGist():list()
```

#### Example: Create

```lua
local base_gist, err = client:BaseGist():create({
  gist_id = "example_gist_id", -- string
})
```
...
Create an instance: `local billing_usage_report = client:BillingUsageReport(nil)`
...
```lua
local billing_usage_reports, err = client:BillingUsageReport():list()
```


### BillingUsageReportUser

Create an instance: `local billing_usage_report_user = client:BillingUsageReportUser(nil)`
...
```lua
local billing_usage_report_users, err = client:BillingUsageReportUser():list()
```


### Blob

Create an instance: `local blob = client:Blob(nil)`
...
```lua
local blob, err = client:Blob():load({ id = "blob_id", owner = "owner", repo = "repo" })
```


### Block

Create an instance: `local block = client:Block(nil)`
...
```lua
local blocks, err = client:Block():list()
```
...
| `pattern` | `string` |  |
| `protected` | `boolean` |  |
| `protection` | `table` | Branch Protection |
...
```lua
local branch, err = client:Branch():load({ id = "branch_id", owner = "owner", repo = "repo" })
```


### BranchProtection

Create an instance: `local branch_protection = client:BranchProtection(nil)`
...
| `allow_force_pushes` | `table` |  |
| `allow_fork_syncing` | `table` | Whether users can pull changes from upstream when the branch is locked. |
| `block_creations` | `table` |  |
| `enabled` | `boolean` |  |
| `enforce_admins` | `table` | Protected Branch Admin Enforced |
| `id` | `string` |  |
| `lock_branch` | `table` | Whether to set the branch as read-only. |
| `name` | `string` |  |
| `protection_url` | `string` |  |
| `required_conversation_resolution` | `table` |  |
| `required_linear_history` | `table` |  |
| `required_pull_request_reviews` | `table` | Protected Branch Pull Request Review |
| `required_signatures` | `table` |  |
| `required_status_checks` | `table` | Protected Branch Required Status Check |
| `restrictions` | `table` | Branch Restriction Policy |
| `url` | `string` |  |

#### Example: Load

```lua
local branch_protection, err = client:BranchProtection():load({ id = "branch_protection_id", owner = "owner", repo = "repo" })
```


### BranchRestrictionPolicy

Create an instance: `local branch_restriction_policy = client:BranchRestrictionPolicy(nil)`
...
```lua
local branch_restriction_policys, err = client:BranchRestrictionPolicy():list()
```


### BranchShort

Create an instance: `local branch_short = client:BranchShort(nil)`
...
```lua
local branch_shorts, err = client:BranchShort():list()
```


### BranchWithProtection

Create an instance: `local branch_with_protection = client:BranchWithProtection(nil)`
...
```lua
local branch_with_protection, err = client:BranchWithProtection():create({
  id = "example_id", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  new_name = "example_new_name", -- string
})
```
...
| `code_scanning_alerts` | `table` | The code scanning alerts to include in this campaign |
...
| `id` | `string` |  |
| `managers` | `table` | The campaign managers |
...
local campaign, err = client:Campaign():create({
  ...
  contact_link = "example_contact_link", -- string
  created_at = "example_created_at", -- string
  description = "example_description", -- string
  ends_at = "example_ends_at", -- string
  managers = {}, -- table
  number = 1, -- number
  state = "example_state", -- string
  updated_at = "example_updated_at", -- string
...
| --- | --- | --- |
| `after` | `string` |  |
| `app` | `table` | GitHub apps are a new way to extend GitHub. |
| `before` | `string` |  |
...
| `conclusion` | `string` |  |
| `created_at` | `string` |  |
| `deployment` | `table` | A deployment created as the result of an Actions check run from a workflow that references an environment |
| `details_url` | `string` |  |
| `external_id` | `string` |  |
| `head_branch` | `string` |  |
| `head_commit` | `table` | A commit. |
...
| `node_id` | `string` |  |
| `output` | `table` |  |
| `pull_requests` | `table` | Pull requests that are open with a `head_sha` or `head_branch` that matches the check. |
| `repository` | `table` | Minimal Repository |
...
Create an instance: `local check_annotation = client:CheckAnnotation(nil)`
...
```lua
local check_annotations, err = client:CheckAnnotation():list()
```


### CheckAutomatedSecurityFix

Create an instance: `local check_automated_security_fix = client:CheckAutomatedSecurityFix(nil)`
...
```lua
local check_automated_security_fix, err = client:CheckAutomatedSecurityFix():load({ owner = "owner", repo = "repo" })
```


### CheckRun

Create an instance: `local check_run = client:CheckRun(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `actions` | `table` | Displays a button on GitHub that can be clicked to alert your app to do additional tasks. |
| `app` | `table` | GitHub apps are a new way to extend GitHub. |
| `check_suite` | `table` |  |
...
| `name` | `string` | The name of the check. |
| `node_id` | `string` |  |
| `output` | `table` | Check runs can accept a variety of data in the `output` object, including a `title` and `summary` and can optionally provide descriptive details about the run. |
| `pull_requests` | `table` | Pull requests that are open with a `head_sha` or `head_branch` that matches the check. |
...
```lua
local check_run, err = client:CheckRun():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local check_run, err = client:CheckRun():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  app = {}, -- table
  check_suite = {}, -- table
  ...
  conclusion = "example_conclusion", -- string
  deployment = {}, -- table
  details_url = "example_details_url", -- string
...
Create an instance: `local check_suite = client:CheckSuite(nil)`
...
```lua
local check_suite, err = client:CheckSuite():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local check_suite, err = client:CheckSuite():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  after = "example_after", -- string
  app = {}, -- table
  before = "example_before", -- string
  check_runs_url = "example_check_runs_url", -- string
  conclusion = "example_conclusion", -- string
  created_at = "example_created_at", -- string
  head_branch = "example_head_branch", -- string
  head_commit = {}, -- table
  head_sha = "example_head_sha", -- string
  id = 1, -- number
  latest_check_runs_count = 1, -- number
  node_id = "example_node_id", -- string
  pull_requests = {}, -- table
  repository = {}, -- table
  status = "example_status", -- string
...
Create an instance: `local check_suite_preference = client:CheckSuitePreference(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `auto_trigger_checks` | `table` | Enables or disables automatic creation of CheckSuite events upon pushes to the repository. |
| `preferences` | `table` |  |
| `repository` | `table` | Minimal Repository |
...
Create an instance: `local classroom = client:Classroom(nil)`
...
```lua
local classroom, err = client:Classroom():load({ id = 1 })
```

#### Example: List

```lua
local classrooms, err = client:Classroom():list()
```


### ClassroomAcceptedAssignment

Create an instance: `local classroom_accepted_assignment = client:ClassroomAcceptedAssignment(nil)`
...
```lua
local classroom_accepted_assignments, err = client:ClassroomAcceptedAssignment():list()
```


### ClassroomAssignment

Create an instance: `local classroom_assignment = client:ClassroomAssignment(nil)`
...
| `classroom` | `table` | A GitHub Classroom classroom |
...
| `starter_code_repository` | `table` | A GitHub repository view for Classroom |
...
```lua
local classroom_assignment, err = client:ClassroomAssignment():load({ id = 1 })
```


### ClassroomAssignmentGrade

Create an instance: `local classroom_assignment_grade = client:ClassroomAssignmentGrade(nil)`
...
```lua
local classroom_assignment_grades, err = client:ClassroomAssignmentGrade():list()
```
...
```lua
local clones, err = client:Clone():list()
```


### CodeFrequency

Create an instance: `local code_frequency = client:CodeFrequency(nil)`
...
```lua
local code_frequencys, err = client:CodeFrequency():list()
```


### CodeFrequencyStat

Create an instance: `local code_frequency_stat = client:CodeFrequencyStat(nil)`
...
```lua
local code_frequency_stats, err = client:CodeFrequencyStat():list()
```


### CodeOfConduct

Create an instance: `local code_of_conduct = client:CodeOfConduct(nil)`
...
```lua
local code_of_conduct, err = client:CodeOfConduct():load({ key = "key" })
```

#### Example: List

```lua
local code_of_conducts, err = client:CodeOfConduct():list()
```


### CodeScanning

Create an instance: `local code_scanning = client:CodeScanning(nil)`
...
```lua
local code_scanning, err = client:CodeScanning():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  commit_sha = "example_commit_sha", -- string
  ref = "example_ref", -- string
  sarif = "example_sarif", -- string
})
```
...
Create an instance: `local code_scanning_alert = client:CodeScanningAlert(nil)`
...
| `dismissal_approved_by` | `table` | A GitHub user. |
| `dismissed_at` | `string` | The time that the alert was dismissed in ISO 8601 format: `YYYY-MM-DDTHH:MM:SSZ`. |
| `dismissed_by` | `table` | A GitHub user. |
...
```lua
local code_scanning_alert, err = client:CodeScanningAlert():load({ id = 1, owner = "owner", repo = "repo" })
```


### CodeScanningAlertInstance

Create an instance: `local code_scanning_alert_instance = client:CodeScanningAlertInstance(nil)`
...
| `classifications` | `table` | Classifications that have been applied to the file that triggered the alert. |
| `commit_sha` | `string` |  |
...
| `html_url` | `string` |  |
| `location` | `table` | Describe a region within a file for the alert. |
| `message` | `table` |  |
...
```lua
local code_scanning_alert_instances, err = client:CodeScanningAlertInstance():list()
```


### CodeScanningAlertItem

Create an instance: `local code_scanning_alert_item = client:CodeScanningAlertItem(nil)`
...
```lua
local code_scanning_alert_items, err = client:CodeScanningAlertItem():list()
```


### CodeScanningAnalysi

Create an instance: `local code_scanning_analysi = client:CodeScanningAnalysi(nil)`
...
```lua
local code_scanning_analysi, err = client:CodeScanningAnalysi():load({ analysis_id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local code_scanning_analysis, err = client:CodeScanningAnalysi():list()
```


### CodeScanningAnalysisDeletion

Create an instance: `local code_scanning_analysis_deletion = client:CodeScanningAnalysisDeletion(nil)`
...
Create an instance: `local code_scanning_autofix = client:CodeScanningAutofix(nil)`
...
```lua
local code_scanning_autofix, err = client:CodeScanningAutofix():load({ alert_number = 1, owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local code_scanning_autofix, err = client:CodeScanningAutofix():create({
  alert_number = 1, -- number
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  description = "example_description", -- string
  started_at = "example_started_at", -- string
...
Create an instance: `local code_scanning_autofix_commit = client:CodeScanningAutofixCommit(nil)`
...
```lua
local code_scanning_autofix_commit, err = client:CodeScanningAutofixCommit():create({
  alert_id = 1, -- number
  owner = "example_owner", -- string
  repo = "example_repo", -- string
})
...
Create an instance: `local code_scanning_codeql_database = client:CodeScanningCodeqlDatabase(nil)`
...
```lua
local code_scanning_codeql_database, err = client:CodeScanningCodeqlDatabase():load({ language = "language", owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local code_scanning_codeql_databases, err = client:CodeScanningCodeqlDatabase():list()
```


### CodeScanningDefaultSetup

Create an instance: `local code_scanning_default_setup = client:CodeScanningDefaultSetup(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `languages` | `table` | Languages to be analyzed. |
...
```lua
local code_scanning_default_setups, err = client:CodeScanningDefaultSetup():list()
```


### CodeScanningOrganizationAlertItem

Create an instance: `local code_scanning_organization_alert_item = client:CodeScanningOrganizationAlertItem(nil)`
...
| `most_recent_instance` | `table` |  |
| `number` | `number` | The security alert number. |
| `repository` | `table` | A GitHub repository. |
| `rule` | `table` |  |
| `state` | `string` | State of a code scanning alert. |
| `tool` | `table` |  |
| `updated_at` | `string` | The time that the alert was last updated in ISO 8601 format: `YYYY-MM-DDTHH:MM:SSZ`. |
...
```lua
local code_scanning_organization_alert_items, err = client:CodeScanningOrganizationAlertItem():list()
```


### CodeScanningSarifsStatus

Create an instance: `local code_scanning_sarifs_status = client:CodeScanningSarifsStatus(nil)`
...
| `errors` | `table` | Any errors that ocurred during processing of the delivery. |
...
```lua
local code_scanning_sarifs_status, err = client:CodeScanningSarifsStatus():load({ owner = "owner", repo = "repo", sarif_id = "sarif_id" })
```


### CodeScanningVariantAnalysi

Create an instance: `local code_scanning_variant_analysi = client:CodeScanningVariantAnalysi(nil)`
...
| `controller_repo` | `table` | A GitHub repository. |
...
| `repositories` | `table` | List of repository names (in the form `owner/repo-name`) to run the query against. |
| `repository_lists` | `table` | List of repository lists to run the query against. |
| `repository_owners` | `table` | List of organization or user names whose repositories the query should be run against. |
| `scanned_repositories` | `table` |  |
| `skipped_repositories` | `table` | Information about repositories that were skipped from processing. |
| `status` | `string` |  |
...
```lua
local code_scanning_variant_analysi, err = client:CodeScanningVariantAnalysi():load({ codeql_variant_analysis_id = 1, owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local code_scanning_variant_analysi, err = client:CodeScanningVariantAnalysi():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  actor = {}, -- table
  ...
  id = 1, -- number
  language = "example_language", -- string
  query_language = "example_query_language", -- string
  query_pack = "example_query_pack", -- string
  query_pack_url = "example_query_pack_url", -- string
...
Create an instance: `local code_scanning_variant_analysis_repo_task = client:CodeScanningVariantAnalysisRepoTask(nil)`
...
```lua
local code_scanning_variant_analysis_repo_task, err = client:CodeScanningVariantAnalysisRepoTask():load({ codeql_variant_analysis_id = 1, owner = "owner", repo = "repo", repo_name = "repo_name", repo_owner = "repo_owner" })
```


### CodeSecurity

Create an instance: `local code_security = client:CodeSecurity(nil)`
...
| `code_scanning_default_setup_options` | `table` | Feature options for code scanning default setup |
| `code_scanning_delegated_alert_dismissal` | `string` | The enablement status of code scanning delegated alert dismissal |
| `code_scanning_options` | `table` | Feature options for code scanning |
| `created_at` | `string` |  |
...
| `dependency_graph_autosubmit_action_options` | `table` | Feature options for Automatic dependency submission |
...
| `secret_scanning_delegated_bypass_options` | `table` | Feature options for secret scanning delegated bypass |
...
Create an instance: `local code_security_configuration = client:CodeSecurityConfiguration(nil)`
...
| `secret_scanning_push_protection` | `string` | The enablement status of secret scanning push protection |
| `secret_scanning_validity_checks` | `string` | The enablement status of secret scanning validity checks |
| `selected_repository_ids` | `table` | An array of repository IDs to attach the configuration to. |
| `target_type` | `string` | The type of the code security configuration. |
| `updated_at` | `string` |  |
| `url` | `string` | The URL of the configuration |
...
```lua
local code_security_configuration, err = client:CodeSecurityConfiguration():load({ id = 1, enterprise = "enterprise" })
```

#### Example: List

```lua
local code_security_configurations, err = client:CodeSecurityConfiguration():list()
```

#### Example: Create

```lua
local code_security_configuration, err = client:CodeSecurityConfiguration():create({
  enterprise = "example_enterprise", -- string
...
Create an instance: `local code_security_configuration_repository = client:CodeSecurityConfigurationRepository(nil)`
...
```lua
local code_security_configuration_repositorys, err = client:CodeSecurityConfigurationRepository():list()
```


### CodeSecurityDefaultConfiguration

Create an instance: `local code_security_default_configuration = client:CodeSecurityDefaultConfiguration(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `configuration` | `table` | A code security configuration |
...
```lua
local code_security_default_configurations, err = client:CodeSecurityDefaultConfiguration():list()
```


### CodeownersError

Create an instance: `local codeowners_error = client:CodeownersError(nil)`
...
```lua
local codeowners_errors, err = client:CodeownersError():list()
```


### Codespace

Create an instance: `local codespace = client:Codespace(nil)`
...
| `archived` | `boolean` |  |
| `assignees_url` | `string` |  |
| `billable_owner` | `table` | A GitHub user. |
| `blobs_url` | `string` |  |
...
| `git_commits_url` | `string` |  |
| `git_refs_url` | `string` |  |
| `git_status` | `table` | Details about the codespace's git repository. |
| `git_tags_url` | `string` |  |
| `git_url` | `string` |  |
...
| `machine` | `table` | A description of the machine powering a codespace. |
...
| `pushed_at` | `string` |  |
| `recent_folders` | `table` | Recently opened folders inside the codespace. |
...
| `selected_repository_ids` | `table` | An array of repository IDs that can access the organization secret. |
| `selected_usernames` | `table` | The usernames of the organization members whose codespaces be billed to the organization. |
...
```lua
local codespace, err = client:Codespace():load({ id = "codespace_id" })
```

#### Example: List

```lua
local codespaces, err = client:Codespace():list()
```

#### Example: Create

```lua
local codespace, err = client:Codespace():create({
  ...
  git_tags_url = "example_git_tags_url", -- string
  hooks_url = "example_hooks_url", -- string
  idle_timeout_minutes = 1, -- number
  issue_comment_url = "example_issue_comment_url", -- string
  issue_events_url = "example_issue_events_url", -- string
  issues_url = "example_issues_url", -- string
  key = "example_key", -- string
  key_id = "example_key_id", -- string
  keys_url = "example_keys_url", -- string
  labels_url = "example_labels_url", -- string
  languages_url = "example_languages_url", -- string
  last_used_at = "example_last_used_at", -- string
  location = "example_location", -- string
  machine = {}, -- table
  machines_url = "example_machines_url", -- string
  memory_in_bytes = 1, -- number
  merges_url = "example_merges_url", -- string
  milestones_url = "example_milestones_url", -- string
  name = "example_name", -- string
  node_id = "example_node_id", -- string
  notifications_url = "example_notifications_url", -- string
  operating_system = "example_operating_system", -- string
  owner = {}, -- table
  path = "example_path", -- string
  prebuild = true, -- boolean
  prebuild_availability = "example_prebuild_availability", -- string
  pulls_url = "example_pulls_url", -- string
  ...
  stargazers_url = "example_stargazers_url", -- string
  start_url = "example_start_url", -- string
  statuses_url = "example_statuses_url", -- string
  stop_url = "example_stop_url", -- string
  storage_in_bytes = 1, -- number
  subscribers_url = "example_subscribers_url", -- string
  subscription_url = "example_subscription_url", -- string
  tags_url = "example_tags_url", -- string
  teams_url = "example_teams_url", -- string
  ...
  web_url = "example_web_url", -- string
})
```
...
```lua
local collaborators, err = client:Collaborator():list()
```


### CombinedBillingUsage

Create an instance: `local combined_billing_usage = client:CombinedBillingUsage(nil)`
...
```lua
local combined_billing_usage, err = client:CombinedBillingUsage():load({ org_id = "org_id" })
```


### CombinedCommitStatus

Create an instance: `local combined_commit_status = client:CombinedCommitStatus(nil)`
...
```lua
local combined_commit_statuss, err = client:CombinedCommitStatus():list()
```
...
```lua
local commit, err = client:Commit():load({ id = "commit_id", owner = "owner", repo = "repo" })
```
...
```lua
local commit, err = client:Commit():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author = "example_author", -- any
  base = "example_base", -- string
  comments_url = "example_comments_url", -- string
  commit = {}, -- table
  committer = "example_committer", -- any
  head = "example_head", -- string
  html_url = "example_html_url", -- string
  node_id = "example_node_id", -- string
  parents = {}, -- table
...
Create an instance: `local commit_activity = client:CommitActivity(nil)`
...
```lua
local commit_activitys, err = client:CommitActivity():list()
```


### CommitComment

Create an instance: `local commit_comment = client:CommitComment(nil)`
...
```lua
local commit_comment, err = client:CommitComment():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local commit_comments, err = client:CommitComment():list()
```

#### Example: Create

```lua
local commit_comment, err = client:CommitComment():create({
  commit_sha = "example_commit_sha", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author_association = "example_author_association", -- string
  body = "example_body", -- string
  commit_id = "example_commit_id", -- string
  created_at = "example_created_at", -- string
  html_url = "example_html_url", -- string
  id = 1, -- number
  line = 1, -- number
...
Create an instance: `local commit_comparison = client:CommitComparison(nil)`
...
| --- | --- | --- |
| `ahead_by` | `number` |  |
| `base_commit` | `table` | Commit |
| `behind_by` | `number` |  |
| `commits` | `table` |  |
| `diff_url` | `string` |  |
| `files` | `table` |  |
| `html_url` | `string` |  |
| `merge_base_commit` | `table` | Commit |
| `patch_url` | `string` |  |
...
```lua
local commit_comparison, err = client:CommitComparison():load({ basehead = "basehead", owner = "owner", repo = "repo" })
```


### CommunityProfile

Create an instance: `local community_profile = client:CommunityProfile(nil)`
...
| `code_of_conduct_file` | `table` |  |
| `contributing` | `table` |  |
| `issue_template` | `table` |  |
| `license` | `table` | License Simple |
| `pull_request_template` | `table` |  |
| `readme` | `table` |  |

#### Example: Load

```lua
local community_profile, err = client:CommunityProfile():load({ owner = "owner", repo = "repo" })
```


### ContentFile

Create an instance: `local content_file = client:ContentFile(nil)`
...
```lua
local content_file, err = client:ContentFile():load({ owner = "owner", repo = "repo" })
```


### ContentTraffic

Create an instance: `local content_traffic = client:ContentTraffic(nil)`
...
```lua
local content_traffics, err = client:ContentTraffic():list()
```


### Contributor

Create an instance: `local contributor = client:Contributor(nil)`
...
```lua
local contributors, err = client:Contributor():list()
```


### Copilot

Create an instance: `local copilot = client:Copilot(nil)`
...
| `organization` | `table` | A GitHub organization. |
...
| `selected_teams` | `table` | List of team names within the organization to which to grant access to GitHub Copilot. |
| `selected_usernames` | `table` | The usernames of the organization members to be granted access to GitHub Copilot. |
| `updated_at` | `string` | **Closing down notice:** This field is no longer relevant and is closing down. |

#### Example: Load

```lua
local copilot, err = client:Copilot():load({ org_id = "org_id", username = "username" })
```

#### Example: List

```lua
local copilots, err = client:Copilot():list()
```

#### Example: Create

```lua
local copilot, err = client:Copilot():create({
  org_id = "example_org_id", -- string
  assignee = {}, -- table
  created_at = "example_created_at", -- string
  organization = {}, -- table
...
Create an instance: `local copilot_organization_detail = client:CopilotOrganizationDetail(nil)`
...
```lua
local copilot_organization_detail, err = client:CopilotOrganizationDetail():load({ org_id = "org_id" })
```


### CopilotUsageMetricsDay

Create an instance: `local copilot_usage_metrics_day = client:CopilotUsageMetricsDay(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `copilot_dotcom_chat` | `table` | Usage metrics for Copilot Chat in GitHub.com |
| `copilot_dotcom_pull_requests` | `table` | Usage metrics for Copilot for pull requests. |
| `copilot_ide_chat` | `table` | Usage metrics for Copilot Chat in the IDE. |
| `copilot_ide_code_completions` | `table` | Usage metrics for Copilot editor code completions in the IDE. |
...
```lua
local copilot_usage_metrics_days, err = client:CopilotUsageMetricsDay():list()
```
...
| Field | Type | Description |
| --- | --- | --- |
| `credentials` | `table` | A list of credentials to be revoked, up to 1000 per request. |
...
Create an instance: `local custom_property = client:CustomProperty(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `allowed_values` | `table` | An ordered list of the allowed values of the property. |
| `default_value` | `any` | Default value of the property |
| `description` | `string` | Short description of the property |
| `properties` | `table` | The array of custom properties to create or update. |
...
```lua
local custom_property, err = client:CustomProperty():load({ custom_property_name = "custom_property_name", org_id = "org_id" })
```

#### Example: List

```lua
local custom_propertys, err = client:CustomProperty():list()
```


### CustomPropertyValue

Create an instance: `local custom_property_value = client:CustomPropertyValue(nil)`
...
```lua
local custom_property_values, err = client:CustomPropertyValue():list()
```


### Dependabot

Create an instance: `local dependabot = client:Dependabot(nil)`
...
| `releases_url` | `string` |  |
| `repos_url` | `string` |  |
| `repository_ids_to_add` | `table` | List of repository IDs to add. |
| `repository_ids_to_remove` | `table` | List of repository IDs to remove. |
| `role_name` | `string` |  |
| `security_and_analysis` | `table` |  |
...
```lua
local dependabots, err = client:Dependabot():list()
```


### DependabotAlert

Create an instance: `local dependabot_alert = client:DependabotAlert(nil)`
...
| `created_at` | `string` | The time that the alert was created in ISO 8601 format: `YYYY-MM-DDTHH:MM:SSZ`. |
| `dependency` | `table` | Details for the vulnerable dependency. |
| `dismissed_at` | `string` | The time that the alert was dismissed in ISO 8601 format: `YYYY-MM-DDTHH:MM:SSZ`. |
| `dismissed_by` | `table` | A GitHub user. |
...
| `id` | `string` |  |
| `number` | `number` | The security alert number. |
| `security_advisory` | `table` | Details for the GitHub Security Advisory. |
| `security_vulnerability` | `table` | Details pertaining to one vulnerable version range for the advisory. |
...
```lua
local dependabot_alert, err = client:DependabotAlert():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local dependabot_alerts, err = client:DependabotAlert():list()
```


### DependabotAlertWithRepository

Create an instance: `local dependabot_alert_with_repository = client:DependabotAlertWithRepository(nil)`
...
```lua
local dependabot_alert_with_repositorys, err = client:DependabotAlertWithRepository():list()
```


### DependabotPublicKey

Create an instance: `local dependabot_public_key = client:DependabotPublicKey(nil)`
...
```lua
local dependabot_public_key, err = client:DependabotPublicKey():load({ org_id = "org_id" })
```


### DependabotRepositoryAccessDetail

Create an instance: `local dependabot_repository_access_detail = client:DependabotRepositoryAccessDetail(nil)`
...
```lua
local dependabot_repository_access_details, err = client:DependabotRepositoryAccessDetail():list()
```


### DependabotSecret

Create an instance: `local dependabot_secret = client:DependabotSecret(nil)`
...
```lua
local dependabot_secret, err = client:DependabotSecret():load({ id = "dependabot_secret_id", owner = "owner", repo = "repo" })
```


### DependencyGraph

Create an instance: `local dependency_graph = client:DependencyGraph(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `detector` | `table` | A description of the detector used. |
| `job` | `table` |  |
| `manifests` | `table` | A collection of package manifests, which are a collection of related dependencies declared in a file or representing a logical group of dependencies. |
| `metadata` | `table` | User-defined metadata to store domain-specific information limited to 8 keys with scalar values. |
...
```lua
local dependency_graph, err = client:DependencyGraph():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  detector = {}, -- table
  job = {}, -- table
  ref = "example_ref", -- string
  scanned = "example_scanned", -- string
  sha = "example_sha", -- string
  version = 1, -- number
})
```
...
Create an instance: `local dependency_graph_diff = client:DependencyGraphDiff(nil)`
...
```lua
local dependency_graph_diff, err = client:DependencyGraphDiff():load({ basehead = "basehead", owner = "owner", repo = "repo" })
```


### DependencyGraphSpdxSbom

Create an instance: `local dependency_graph_spdx_sbom = client:DependencyGraphSpdxSbom(nil)`
...
```lua
local dependency_graph_spdx_sbom, err = client:DependencyGraphSpdxSbom():load({ owner = "owner", repo = "repo" })
```
...
```lua
local deploy_key, err = client:DeployKey():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local deploy_keys, err = client:DeployKey():list()
```

#### Example: Create

```lua
local deploy_key, err = client:DeployKey():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  created_at = "example_created_at", -- string
  id = 1, -- number
...
| `created_at` | `string` |  |
| `creator` | `table` | A GitHub user. |
| `description` | `string` | Short description of the deployment. |
| `environment` | `string` | Name for the target deployment environment. |
| `environment_ids` | `table` | The list of environment ids to approve or reject |
| `id` | `number` | Unique identifier of the deployment |
| `node_id` | `string` |  |
| `original_environment` | `string` |  |
| `payload` | `any` |  |
| `performed_via_github_app` | `table` | GitHub apps are a new way to extend GitHub. |
...
| `required_contexts` | `table` | The [status](https://docs.github.com/rest/commits/statuses) contexts to verify against commit status checks. |
| `sha` | `string` |  |
...
```lua
local deployment, err = client:Deployment():load({ id = 1, owner = "owner", repo = "repo" })
```
...
Create an instance: `local deployment_branch_policy = client:DeploymentBranchPolicy(nil)`
...
```lua
local deployment_branch_policy, err = client:DeploymentBranchPolicy():load({ id = 1, environment_id = "environment_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local deployment_branch_policy, err = client:DeploymentBranchPolicy():create({
  environment_name = "example_environment_name", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
})
```
...
Create an instance: `local deployment_protection_rule = client:DeploymentProtectionRule(nil)`
...
```lua
local deployment_protection_rule, err = client:DeploymentProtectionRule():load({ id = 1, environment_id = "environment_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local deployment_protection_rule, err = client:DeploymentProtectionRule():create({
  environment_name = "example_environment_name", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  id = 1, -- number
  integration_url = "example_integration_url", -- string
  node_id = "example_node_id", -- string
...
Create an instance: `local deployment_status = client:DeploymentStatus(nil)`
...
```lua
local deployment_status, err = client:DeploymentStatus():load({ id = 1, deployment_id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local deployment_statuss, err = client:DeploymentStatus():list()
```

#### Example: Create

```lua
local deployment_status, err = client:DeploymentStatus():create({
  id = 1, -- number
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  created_at = "example_created_at", -- string
  creator = {}, -- table
  deployment_url = "example_deployment_url", -- string
  description = "example_description", -- string
  node_id = "example_node_id", -- string
  performed_via_github_app = {}, -- table
  repository_url = "example_repository_url", -- string
...
Create an instance: `local diff_entry = client:DiffEntry(nil)`
...
```lua
local diff_entrys, err = client:DiffEntry():list()
```
...
```lua
local email, err = client:Email():create({
})
```
...
Create an instance: `local empty_object = client:EmptyObject(nil)`
...
| `include_claim_keys` | `table` | Array of unique strings. |
| `key_id` | `string` | ID of the key you used to encrypt the secret. |
| `languages` | `table` | CodeQL languages to be analyzed. |
| `name` | `string` | The name of the variable. |
| `query_suite` | `string` | CodeQL query suite to be used. |
| `runner_label` | `string` | Runner label to be used if the runner type is labeled. |
| `runner_type` | `string` | Runner type to be used. |
| `selected_repository_ids` | `table` | An array of repository ids that can access the organization variable. |
...
```lua
local empty_object, err = client:EmptyObject():load({ subject_digest = "subject_digest", username = "username" })
```

#### Example: Create

```lua
local empty_object, err = client:EmptyObject():create({
  org_id = "example_org_id", -- string
  encrypted_value = "example_encrypted_value", -- string
  key_id = "example_key_id", -- string
  name = "example_name", -- string
  use_default = true, -- boolean
...
Create an instance: `local enterprise_team = client:EnterpriseTeam(nil)`
...
```lua
local enterprise_team, err = client:EnterpriseTeam():load({ id = "enterprise_team_id", enterprise = "enterprise" })
```

#### Example: List

```lua
local enterprise_teams, err = client:EnterpriseTeam():list()
```

#### Example: Create

```lua
local enterprise_team, err = client:EnterpriseTeam():create({
  enterprise = "example_enterprise", -- string
  created_at = "example_created_at", -- string
  group_id = "example_group_id", -- string
  html_url = "example_html_url", -- string
  id = 1, -- number
  members_url = "example_members_url", -- string
  name = "example_name", -- string
  slug = "example_slug", -- string
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
...
Create an instance: `local enterprise_team_membership = client:EnterpriseTeamMembership(nil)`
...
| `deployment_branch_policy` | `table` | The type of deployment branch policy for this environment. |
| `html_url` | `string` |  |
...
| `protection_rules` | `table` | Built-in deployment protection rules for the environment. |
| `reviewers` | `table` | The people or teams that may review jobs that reference the environment. |
...
```lua
local environment, err = client:Environment():load({ id = "environment_id", owner = "owner", repo = "repo" })
```


### EnvironmentApproval

Create an instance: `local environment_approval = client:EnvironmentApproval(nil)`
...
| `environments` | `table` | The list of environments that were approved or rejected |
...
```lua
local environment_approvals, err = client:EnvironmentApproval():list()
```
...
```lua
local event, err = client:Event():load({ org = "org", username = "username" })
```
...
Create an instance: `local feed = client:Feed(nil)`
...
| `current_user_organization_urls` | `table` |  |
...
```lua
local feeds, err = client:Feed():list()
```


### FileCommit

Create an instance: `local file_commit = client:FileCommit(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `author` | `table` | The author of the file. |
| `branch` | `string` | The branch name. |
| `commit` | `table` |  |
| `committer` | `table` | The person that committed the file. |
| `content` | `table` | The new file content, using Base64 encoding. |
...
Create an instance: `local follower = client:Follower(nil)`
...
```lua
local followers, err = client:Follower():list()
```


### Following

Create an instance: `local following = client:Following(nil)`
...
```lua
local followings, err = client:Following():list()
```


### FullRepository

Create an instance: `local full_repository = client:FullRepository(nil)`
...
```lua
local full_repository, err = client:FullRepository():load({ owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local full_repository, err = client:FullRepository():create({
  archive_url = "example_archive_url", -- string
  archived = true, -- boolean
  assignees_url = "example_assignees_url", -- string
  blobs_url = "example_blobs_url", -- string
...
```lua
local gist, err = client:Gist():load({ id = "gist_id" })
```
...
Create an instance: `local gist_comment = client:GistComment(nil)`
...
```lua
local gist_comment, err = client:GistComment():load({ id = 1, gist_id = "gist_id" })
```

#### Example: List

```lua
local gist_comments, err = client:GistComment():list()
```

#### Example: Create

```lua
local gist_comment, err = client:GistComment():create({
  id = "example_id", -- string
  author_association = "example_author_association", -- string
  ...
  created_at = "example_created_at", -- string
  events_url = "example_events_url", -- string
  followers_url = "example_followers_url", -- string
  following_url = "example_following_url", -- string
  gists_url = "example_gists_url", -- string
  gravatar_id = "example_gravatar_id", -- string
  html_url = "example_html_url", -- string
  login = "example_login", -- string
  node_id = "example_node_id", -- string
  organizations_url = "example_organizations_url", -- string
  received_events_url = "example_received_events_url", -- string
  repos_url = "example_repos_url", -- string
  site_admin = true, -- boolean
  starred_url = "example_starred_url", -- string
  subscriptions_url = "example_subscriptions_url", -- string
...
Create an instance: `local gist_commit = client:GistCommit(nil)`
...
```lua
local gist_commits, err = client:GistCommit():list()
```


### GistSimple

Create an instance: `local gist_simple = client:GistSimple(nil)`
...
```lua
local gist_simples, err = client:GistSimple():list()
```


### Git

Create an instance: `local git = client:Git(nil)`
...
Create an instance: `local git_commit = client:GitCommit(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `author` | `table` | Identifying information for the git-user |
| `committer` | `table` | Identifying information for the git-user |
| `html_url` | `string` |  |
| `id` | `string` |  |
| `message` | `string` | Message describing the purpose of the commit |
| `node_id` | `string` |  |
| `parents` | `table` | The full SHAs of the commits that were the parents of this commit. |
...
| `tree` | `table` | The SHA of the tree object this commit points to |
| `url` | `string` |  |
| `verification` | `table` |  |
...
```lua
local git_commit, err = client:GitCommit():load({ id = "git_commit_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local git_commit, err = client:GitCommit():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author = {}, -- table
  committer = {}, -- table
...
Create an instance: `local git_ref = client:GitRef(nil)`
...
```lua
local git_ref, err = client:GitRef():load({ id = "git_ref_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local git_ref, err = client:GitRef():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  node_id = "example_node_id", -- string
  object = {}, -- table
...
Create an instance: `local git_tag = client:GitTag(nil)`
...
| `node_id` | `string` |  |
| `object` | `table` | The SHA of the git object this is tagging. |
| `sha` | `string` |  |
| `tag` | `string` | Name of the tag |
| `tagger` | `table` | An object with information about the individual creating the tag. |
...
```lua
local git_tag, err = client:GitTag():load({ id = "git_tag_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local git_tag, err = client:GitTag():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  message = "example_message", -- string
  node_id = "example_node_id", -- string
...
Create an instance: `local git_tree = client:GitTree(nil)`
...
| `id` | `string` |  |
| `sha` | `string` |  |
| `tree` | `table` | Objects specifying a tree structure |
| `truncated` | `boolean` |  |
| `url` | `string` |  |
...
```lua
local git_tree, err = client:GitTree():load({ id = "git_tree_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local git_tree, err = client:GitTree():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  sha = "example_sha", -- string
  tree = {}, -- table
...
Create an instance: `local gitignore = client:Gitignore(nil)`
...
```lua
local gitignores, err = client:Gitignore():list()
```


### GitignoreTemplate

Create an instance: `local gitignore_template = client:GitignoreTemplate(nil)`
...
```lua
local gitignore_template, err = client:GitignoreTemplate():load({ id = "gitignore_template_id" })
```


### GlobalAdvisory

Create an instance: `local global_advisory = client:GlobalAdvisory(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `credits` | `table` | The users who contributed to the advisory. |
| `cve_id` | `string` | The Common Vulnerabilities and Exposures (CVE) ID. |
| `cvss` | `table` |  |
| `cvss_severities` | `table` |  |
| `cwes` | `table` |  |
| `description` | `string` | A detailed description of what the advisory entails. |
| `epss` | `table` | The EPSS scores as calculated by the [Exploit Prediction Scoring System](https://www.first.org/epss). |
...
| `vulnerabilities` | `table` | The products and respective version ranges affected by the advisory. |
| `withdrawn_at` | `string` | The date and time of when the advisory was withdrawn, in ISO 8601 format. |

#### Example: Load

```lua
local global_advisory, err = client:GlobalAdvisory():load({ id = "global_advisory_id" })
```

#### Example: List

```lua
local global_advisorys, err = client:GlobalAdvisory():list()
```


### GpgKey

Create an instance: `local gpg_key = client:GpgKey(nil)`
...
```lua
local gpg_key, err = client:GpgKey():load({ id = 1 })
```

#### Example: List

```lua
local gpg_keys, err = client:GpgKey():list()
```

#### Example: Create

```lua
local gpg_key, err = client:GpgKey():create({
  armored_public_key = "example_armored_public_key", -- string
  can_certify = true, -- boolean
  can_encrypt_comms = true, -- boolean
  can_encrypt_storage = true, -- boolean
  can_sign = true, -- boolean
  created_at = "example_created_at", -- string
  emails = {}, -- table
  expires_at = "example_expires_at", -- string
  id = 1, -- number
  key_id = "example_key_id", -- string
  primary_key_id = 1, -- number
  public_key = "example_public_key", -- string
  raw_key = "example_raw_key", -- string
...
| `add_events` | `table` | Determines a list of events to be added to the list of events that the Hook triggers for. |
| `config` | `table` | Configuration object of the webhook |
| `created_at` | `string` |  |
| `deliveries_url` | `string` |  |
| `events` | `table` | Determines what events the hook is triggered for. |
| `id` | `number` | Unique identifier of the webhook. |
| `last_response` | `table` |  |
| `name` | `string` | The name of a valid service, use 'web' for a webhook. |
| `ping_url` | `string` |  |
| `remove_events` | `table` | Determines a list of events to be removed from the list of events that the Hook triggers for. |
| `test_url` | `string` |  |
| `type` | `string` |  |
| `updated_at` | `string` |  |
| `url` | `string` |  |
...
```lua
local hook, err = client:Hook():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local hooks, err = client:Hook():list()
```
...
local hook, err = client:Hook():create({
  ...
  last_response = {}, -- table
  name = "example_name", -- string
  ping_url = "example_ping_url", -- string
  test_url = "example_test_url", -- string
  type = "example_type", -- string
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
})
...
Create an instance: `local hook_delivery = client:HookDelivery(nil)`
...
```lua
local hook_delivery, err = client:HookDelivery():load({ id = 1 })
```


### HookDeliveryItem

Create an instance: `local hook_delivery_item = client:HookDeliveryItem(nil)`
...
```lua
local hook_delivery_items, err = client:HookDeliveryItem():list()
```


### HostedCompute

Create an instance: `local hosted_compute = client:HostedCompute(nil)`
...
| `network_settings_ids` | `table` | The unique identifier of each network settings in the configuration. |

#### Example: List

```lua
local hosted_computes, err = client:HostedCompute():list()
```


### Hovercard

Create an instance: `local hovercard = client:Hovercard(nil)`
...
```lua
local hovercards, err = client:Hovercard():list()
```
...
```lua
local imports, err = client:Import():list()
```


### Installation

Create an instance: `local installation = client:Installation(nil)`
...
```lua
local installation, err = client:Installation():load({ id = 1 })
```

#### Example: List

```lua
local installations, err = client:Installation():list()
```


### InstallationToken

Create an instance: `local installation_token = client:InstallationToken(nil)`
...
| `id` | `string` |  |
| `permissions` | `table` | The permissions granted to the user access token. |
| `repositories` | `table` | List of repository names that the token should have access to |
| `repository_ids` | `table` | List of repository IDs that the token should have access to |

#### Example: Create

```lua
local installation_token, err = client:InstallationToken():create({
  id = 1, -- number
})
```
...
| Field | Type | Description |
| --- | --- | --- |
| `apps` | `table` | The GitHub Apps that have push access to this branch. |
| `client_id` | `string` |  |
| `created_at` | `string` |  |
| `description` | `string` |  |
| `events` | `table` | The list of events for the GitHub app. |
| `external_url` | `string` |  |
| `html_url` | `string` |  |
...
| `node_id` | `string` |  |
| `owner` | `any` |  |
| `permissions` | `table` | The set of permissions for the GitHub app |
...
```lua
local integration, err = client:Integration():load({ app_slug = "app_slug" })
```
...
local integration, err = client:Integration():create({
  ...
  description = "example_description", -- string
  events = {}, -- table
  external_url = "example_external_url", -- string
  html_url = "example_html_url", -- string
  id = 1, -- number
  name = "example_name", -- string
  node_id = "example_node_id", -- string
...
Create an instance: `local integration_installation = client:IntegrationInstallation(nil)`
...
| `node_id` | `string` |  |
| `requester` | `table` | A GitHub user. |

#### Example: List

```lua
local integration_installations, err = client:IntegrationInstallation():list()
```
...
```lua
local interaction, err = client:Interaction():load()
```


### InteractionLimit

Create an instance: `local interaction_limit = client:InteractionLimit(nil)`
...
| `assignee` | `table` | A GitHub user. |
| `assignees` | `table` | Usernames of people to assign this issue to. |
| `assigner` | `table` | A GitHub user. |
| `author_association` | `string` | How the author is associated with the repository. |
...
| `issue` | `table` | Issues are a great way to keep track of tasks, enhancements, and bugs for your projects. |
...
| `issue_url` | `string` |  |
| `label` | `table` | Issue Event Label |
| `labels` | `table` | Labels to associate with this issue; pass one or more label names to replace the set of labels on this issue; send an empty array to clear all labels from the issue; note that the labels are silently dropped for users without push access t… |
| `labels_url` | `string` |  |
| `lock_reason` | `string` | The reason for locking the issue or pull request conversation. |
| `locked` | `boolean` |  |
| `milestone` | `table` | Issue Event Milestone |
...
| `performed_via_github_app` | `table` | GitHub apps are a new way to extend GitHub. |
| `project_card` | `table` | Issue Event Project Card |
| `pull_request` | `table` |  |
| `reactions` | `table` |  |
| `rename` | `table` | Issue Event Rename |
| `replace_parent` | `boolean` | Option that, when true, instructs the operation to replace the sub-issues current parent issue |
| `repository` | `table` | A repository on GitHub. |
| `repository_url` | `string` |  |
| `requested_reviewer` | `table` | A GitHub user. |
| `requested_team` | `table` | Groups of organization members that gives permissions on specified repositories. |
| `review_requester` | `table` | A GitHub user. |
...
local issue, err = client:Issue():create({
  ...
  comments_url = "example_comments_url", -- string
  commit_id = "example_commit_id", -- string
  commit_url = "example_commit_url", -- string
  created_at = "example_created_at", -- string
  dismissed_review = {}, -- table
  event = "example_event", -- string
  events_url = "example_events_url", -- string
...
Create an instance: `local issue_type = client:IssueType(nil)`
...
```lua
local issue_types, err = client:IssueType():list()
```

#### Example: Create

```lua
local issue_type, err = client:IssueType():create({
  org_id = "example_org_id", -- string
  description = "example_description", -- string
  id = 1, -- number
  name = "example_name", -- string
...
| `labels` | `table` | Labels for the workflow job. |
...
```lua
local job, err = client:Job():load({ id = 1, owner = "owner", repo = "repo" })
```
...
```lua
local key, err = client:Key():load({ id = 1 })
```
...
```lua
local key, err = client:Key():create({
  created_at = "example_created_at", -- string
  id = 1, -- number
  key = "example_key", -- string
  read_only = true, -- boolean
...
```lua
local label, err = client:Label():load({ id = "label_id", owner = "owner", repo = "repo" })
```
...
```lua
local language, err = client:Language():load({ owner = "owner", repo = "repo" })
```


### License

Create an instance: `local license = client:License(nil)`
...
```lua
local license, err = client:License():load({ id = "license_id" })
```

#### Example: List

```lua
local licenses, err = client:License():list()
```


### Markdown

Create an instance: `local markdown = client:Markdown(nil)`
...
```lua
local markdown, err = client:Markdown():create({
...
Create an instance: `local marketplace_listing_plan = client:MarketplaceListingPlan(nil)`
...
```lua
local marketplace_listing_plans, err = client:MarketplaceListingPlan():list()
```


### MarketplacePurchase

Create an instance: `local marketplace_purchase = client:MarketplacePurchase(nil)`
...
| `id` | `number` |  |
| `login` | `string` |  |
| `marketplace_pending_change` | `table` |  |
| `marketplace_purchase` | `table` |  |
...
```lua
local marketplace_purchase, err = client:MarketplacePurchase():load({ account_id = 1 })
```

#### Example: List

```lua
local marketplace_purchases, err = client:MarketplacePurchase():list()
```
...
```lua
local membership, err = client:Membership():load({ id = "membership_id", enterprise = "enterprise", team_id = "team_id" })
```
...
Create an instance: `local merged_upstream = client:MergedUpstream(nil)`
...
```lua
local merged_upstream, err = client:MergedUpstream():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  branch = "example_branch", -- string
})
```
...
```lua
local metas, err = client:Meta():list()
```


### Metaroot

Create an instance: `local metaroot = client:Metaroot(nil)`
...
```lua
local metaroot, err = client:Metaroot():load()
```
...
| `downloads_url` | `string` |  |
| `events_url` | `string` |  |
| `exclude` | `table` | Exclude related items from being returned in the response in order to improve performance of the request. |
...
| `pushed_at` | `string` |  |
| `releases_url` | `string` |  |
| `repositories` | `table` | The repositories included in the migration. |
| `role_name` | `string` |  |
| `security_and_analysis` | `table` |  |
| `size` | `number` | The size of the repository, in kilobytes. |
| `ssh_url` | `string` |  |
...
```lua
local migration, err = client:Migration():load({ id = 1 })
```
...
local migration, err = client:Migration():create({
  ...
  downloads_url = "example_downloads_url", -- string
  events_url = "example_events_url", -- string
  exclude_attachments = true, -- boolean
  exclude_git_data = true, -- boolean
  exclude_metadata = true, -- boolean
  exclude_owner_projects = true, -- boolean
  exclude_releases = true, -- boolean
  fork = true, -- boolean
  forks_url = "example_forks_url", -- string
  full_name = "example_full_name", -- string
  git_commits_url = "example_git_commits_url", -- string
  ...
  labels_url = "example_labels_url", -- string
  languages_url = "example_languages_url", -- string
  lock_repositories = true, -- boolean
  merges_url = "example_merges_url", -- string
  milestones_url = "example_milestones_url", -- string
  name = "example_name", -- string
  node_id = "example_node_id", -- string
  notifications_url = "example_notifications_url", -- string
  org_metadata_only = true, -- boolean
  owner = {}, -- table
  private = true, -- boolean
  pulls_url = "example_pulls_url", -- string
  releases_url = "example_releases_url", -- string
...
```lua
local milestone, err = client:Milestone():load({ id = 1, owner = "owner", repo = "repo" })
```
...
Create an instance: `local minimal_repository = client:MinimalRepository(nil)`
...
```lua
local minimal_repositorys, err = client:MinimalRepository():list()
```


### NetworkConfiguration

Create an instance: `local network_configuration = client:NetworkConfiguration(nil)`
...
```lua
local network_configuration, err = client:NetworkConfiguration():load({ id = "network_configuration_id", org_id = "org_id" })
```

#### Example: Create

```lua
local network_configuration, err = client:NetworkConfiguration():create({
  org_id = "example_org_id", -- string
...
Create an instance: `local network_setting = client:NetworkSetting(nil)`
...
```lua
local network_setting, err = client:NetworkSetting():load({ id = "network_setting_id", org_id = "org_id" })
```


### OidcCustomSub

Create an instance: `local oidc_custom_sub = client:OidcCustomSub(nil)`
...
```lua
local oidc_custom_subs, err = client:OidcCustomSub():list()
```


### OidcCustomSubRepo

Create an instance: `local oidc_custom_sub_repo = client:OidcCustomSubRepo(nil)`
...
```lua
local oidc_custom_sub_repos, err = client:OidcCustomSubRepo():list()
```
...
| `attestations` | `table` |  |
| `attestations_subject_digests` | `table` | Mapping of subject digest to bundles. |
| `avatar_url` | `string` |  |
...
| `enterprise_teams_providing_indirect_membership` | `table` | The slugs of the enterprise teams providing the user with indirect membership in the organization. |
| `events` | `table` |  |
| `events_url` | `string` |  |
| `filled_seats` | `number` |  |
| `has_multiple_single_files` | `boolean` |  |
...
| `organization` | `table` | A GitHub organization. |
| `organization_url` | `string` |  |
| `page_info` | `table` | Information about the current page. |
| `pat_ids` | `table` | The IDs of the fine-grained personal access tokens. |
| `pat_request_ids` | `table` | Unique identifiers of the requests for access via fine-grained personal access token. |
...
| `properties` | `table` | List of custom property names and associated values to apply to the repositories. |
| `public_members_url` | `string` |  |
| `query_suite` | `string` | CodeQL query suite to be used. |
...
| `repository_names` | `table` | The names of repositories that the custom property values will be applied to. |
| `repository_selection` | `string` | Describe whether all repositories have been selected or there's a selection involved |
...
| `subject_digests` | `table` | List of subject digests to fetch attestations for. |
| `suspended_at` | `string` |  |
| `suspended_by` | `table` | A GitHub user. |
| `target_id` | `number` | The ID of the user or organization this token is being scoped to. |
| `target_type` | `string` |  |
...
local org, err = client:Org():create({
  enablement = "example_enablement", -- string
  org = "example_org", -- string
  security_product = "example_security_product", -- string
  access_tokens_url = "example_access_tokens_url", -- string
  account = "example_account", -- any
  ...
  node_id = "example_node_id", -- string
  organization = {}, -- table
  organization_url = "example_organization_url", -- string
  pat_ids = {}, -- table
  permissions = {}, -- table
  private_repos = 1, -- number
  properties = {}, -- table
  public_members_url = "example_public_members_url", -- string
  registry_url = "example_registry_url", -- string
  repos_url = "example_repos_url", -- string
  repositories_url = "example_repositories_url", -- string
...
Create an instance: `local org_hook = client:OrgHook(nil)`
...
| `config` | `table` | Key/value pairs to provide settings for this webhook. |
| `created_at` | `string` |  |
| `deliveries_url` | `string` |  |
| `events` | `table` | Determines what [events](https://docs.github.com/webhooks/event-payloads) the hook is triggered for. |
| `id` | `number` |  |
...
```lua
local org_hook, err = client:OrgHook():load({ id = 1, org_id = "org_id" })
```

#### Example: List

```lua
local org_hooks, err = client:OrgHook():list()
```

#### Example: Create

```lua
local org_hook, err = client:OrgHook():create({
  id = "example_id", -- string
  active = true, -- boolean
  config = {}, -- table
  created_at = "example_created_at", -- string
...
Create an instance: `local org_membership = client:OrgMembership(nil)`
...
```lua
local org_membership, err = client:OrgMembership():load({ id = "org_membership_id", org_id = "org_id" })
```


### OrgPrivateRegistryConfiguration

Create an instance: `local org_private_registry_configuration = client:OrgPrivateRegistryConfiguration(nil)`
...
```lua
local org_private_registry_configuration, err = client:OrgPrivateRegistryConfiguration():load({ org_id = "org_id", secret_name = "secret_name" })
```


### OrgPrivateRegistryConfigurationWithSelectedRepository

Create an instance: `local org_private_registry_configuration_with_selected_repository = client:OrgPrivateRegistryConfigurationWithSelectedRepository(nil)`
...
| `registry_type` | `string` | The registry type. |
| `selected_repository_ids` | `table` | An array of repository IDs that can access the organization private registry. |
...
```lua
local org_private_registry_configuration_with_selected_repository, err = client:OrgPrivateRegistryConfigurationWithSelectedRepository():create({
  id = "example_id", -- string
  encrypted_value = "example_encrypted_value", -- string
  key_id = "example_key_id", -- string
  registry_type = "example_registry_type", -- string
  url = "example_url", -- string
  visibility = "example_visibility", -- string
})
```
...
Create an instance: `local org_repo_custom_property_value = client:OrgRepoCustomPropertyValue(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `properties` | `table` | List of custom property names and associated values |
...
```lua
local org_repo_custom_property_values, err = client:OrgRepoCustomPropertyValue():list()
```
...
Create an instance: `local organization_actions_secret = client:OrganizationActionsSecret(nil)`
...
```lua
local organization_actions_secret, err = client:OrganizationActionsSecret():load({ id = "organization_actions_secret_id", org_id = "org_id" })
```


### OrganizationActionsVariable

Create an instance: `local organization_actions_variable = client:OrganizationActionsVariable(nil)`
...
```lua
local organization_actions_variable, err = client:OrganizationActionsVariable():load({ id = "organization_actions_variable_id", org_id = "org_id" })
```


### OrganizationDependabotSecret

Create an instance: `local organization_dependabot_secret = client:OrganizationDependabotSecret(nil)`
...
```lua
local organization_dependabot_secret, err = client:OrganizationDependabotSecret():load({ id = "organization_dependabot_secret_id", org_id = "org_id" })
```
...
| `inviter` | `table` | A GitHub user. |
| `login` | `string` |  |
| `node_id` | `string` |  |
| `role` | `string` | The role for the new member. |
| `team_count` | `number` |  |
| `team_ids` | `table` | Specify IDs for the teams you want to invite new members to. |
...
local organization_invitation, err = client:OrganizationInvitation():create({
  ...
  created_at = "example_created_at", -- string
  id = 1, -- number
  invitation_teams_url = "example_invitation_teams_url", -- string
  inviter = {}, -- table
  login = "example_login", -- string
  node_id = "example_node_id", -- string
  team_count = 1, -- number
...
Create an instance: `local organization_programmatic_access_grant = client:OrganizationProgrammaticAccessGrant(nil)`
...
| `owner` | `table` | A GitHub user. |
| `permissions` | `table` | Permissions requested, categorized by type of permission. |
...
```lua
local organization_programmatic_access_grants, err = client:OrganizationProgrammaticAccessGrant():list()
```


### OrganizationRole

Create an instance: `local organization_role = client:OrganizationRole(nil)`
...
| `organization` | `table` | A GitHub user. |
| `permissions` | `table` | A list of permissions included in this role. |
| `source` | `string` | Source answers the question, "where did this role come from?" |
| `updated_at` | `string` | The date and time the role was last updated. |
...
```lua
local organization_role, err = client:OrganizationRole():load({ id = 1, org_id = "org_id" })
```


### OrganizationSecretScanningAlert

Create an instance: `local organization_secret_scanning_alert = client:OrganizationSecretScanningAlert(nil)`
...
| `push_protection_bypass_request_reviewer` | `table` | A GitHub user. |
...
| `push_protection_bypassed_by` | `table` | A GitHub user. |
| `repository` | `table` | A GitHub repository. |
...
| `resolved_by` | `table` | A GitHub user. |
...
```lua
local organization_secret_scanning_alerts, err = client:OrganizationSecretScanningAlert():list()
```


### OutsideCollaborator

Create an instance: `local outside_collaborator = client:OutsideCollaborator(nil)`
...
```lua
local outside_collaborators, err = client:OutsideCollaborator():list()
```
...
```lua
local package, err = client:Package():load({ package_name = "package_name", package_type = "package_type" })
```
...
local package, err = client:Package():create({
  package_id = "example_package_id", -- string
  package_name = "example_package_name", -- string
  container = {}, -- table
  created_at = "example_created_at", -- string
  docker = {}, -- table
  github_id = 1, -- number
  html_url = "example_html_url", -- string
  id = "example_id", -- string
  included_gigabytes_bandwidth = 1, -- number
  metadata = {}, -- table
  name = "example_name", -- string
  owner = {}, -- table
  package_html_url = "example_package_html_url", -- string
  package_type = "example_package_type", -- string
  repository = {}, -- table
  total_gigabytes_bandwidth_used = 1, -- number
  total_paid_gigabytes_bandwidth_used = 1, -- number
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
  version_count = 1, -- number
  visibility = "example_visibility", -- string
})
```
...
| `source` | `table` | The source branch and directory used to publish your Pages site. |
...
```lua
local page, err = client:Page():load({ owner = "owner", repo = "repo" })
```
...
local page, err = client:Page():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  cname = "example_cname", -- string
  custom_404 = true, -- boolean
  https_certificate = {}, -- table
  public = true, -- boolean
  source = {}, -- table
  status = "example_status", -- string
...
Create an instance: `local page_build = client:PageBuild(nil)`
...
```lua
local page_build, err = client:PageBuild():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local page_builds, err = client:PageBuild():list()
```


### PageBuildStatus

Create an instance: `local page_build_status = client:PageBuildStatus(nil)`
...
```lua
local page_build_status, err = client:PageBuildStatus():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
})
```
...
Create an instance: `local page_deployment = client:PageDeployment(nil)`
...
```lua
local page_deployment, err = client:PageDeployment():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  oidc_token = "example_oidc_token", -- string
  pages_build_version = "example_pages_build_version", -- string
})
```
...
Create an instance: `local pages_deployment_status = client:PagesDeploymentStatus(nil)`
...
```lua
local pages_deployment_status, err = client:PagesDeploymentStatus():load({ owner = "owner", pages_deployment_id = "pages_deployment_id", repo = "repo" })
```

#### Example: Create

```lua
local pages_deployment_status, err = client:PagesDeploymentStatus():create({
...
Create an instance: `local pages_health_check = client:PagesHealthCheck(nil)`
...
```lua
local pages_health_check, err = client:PagesHealthCheck():load({ owner = "owner", repo = "repo" })
```


### Participation

Create an instance: `local participation = client:Participation(nil)`
...
```lua
local participations, err = client:Participation():list()
```


### PendingDeployment

Create an instance: `local pending_deployment = client:PendingDeployment(nil)`
...
| `environment` | `table` |  |
| `reviewers` | `table` | The people or teams that may approve jobs that reference the environment. |
...
```lua
local pending_deployments, err = client:PendingDeployment():list()
```


### PorterAuthor

Create an instance: `local porter_author = client:PorterAuthor(nil)`
...
```lua
local porter_authors, err = client:PorterAuthor():list()
```


### PorterLargeFile

Create an instance: `local porter_large_file = client:PorterLargeFile(nil)`
...
```lua
local porter_large_files, err = client:PorterLargeFile():list()
```


### PrivateRegistry

Create an instance: `local private_registry = client:PrivateRegistry(nil)`
...
```lua
local private_registry, err = client:PrivateRegistry():load({ org_id = "org_id" })
```

#### Example: List

```lua
local private_registrys, err = client:PrivateRegistry():list()
```


### PrivateUser

Create an instance: `local private_user = client:PrivateUser(nil)`
...
local project, err = client:Project():create({
  avatar_url = "example_avatar_url", -- string
  columns_url = "example_columns_url", -- string
  created_at = "example_created_at", -- string
  creator = {}, -- table
  events_url = "example_events_url", -- string
  followers_url = "example_followers_url", -- string
  ...
  number = 1, -- number
  organizations_url = "example_organizations_url", -- string
  owner_url = "example_owner_url", -- string
  received_events_url = "example_received_events_url", -- string
  repos_url = "example_repos_url", -- string
  site_admin = true, -- boolean
  starred_url = "example_starred_url", -- string
...
Create an instance: `local project_collaborator_permission = client:ProjectCollaboratorPermission(nil)`
...
```lua
local project_collaborator_permission, err = client:ProjectCollaboratorPermission():load({ project_id = 1, username = "username" })
```


### ProjectColumn

Create an instance: `local project_column = client:ProjectColumn(nil)`
...
```lua
local project_column, err = client:ProjectColumn():load({ id = 1 })
```

#### Example: List

```lua
local project_columns, err = client:ProjectColumn():list()
```

#### Example: Create

```lua
local project_column, err = client:ProjectColumn():create({
  id = 1, -- number
  cards_url = "example_cards_url", -- string
  created_at = "example_created_at", -- string
  name = "example_name", -- string
  node_id = "example_node_id", -- string
  project_url = "example_project_url", -- string
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
})
```
...
Create an instance: `local projects_classic = client:ProjectsClassic(nil)`
...
```lua
local projects_classic, err = client:ProjectsClassic():create({
  column_id = 1, -- number
  position = "example_position", -- string
})
```
...
Create an instance: `local projects_v2 = client:ProjectsV2(nil)`
...
| `deleted_by` | `table` | A GitHub user. |
...
| `latest_status_update` | `table` | An status update belonging to a project |
...
```lua
local projects_v2, err = client:ProjectsV2():load({ id = 1 })
```

#### Example: List

```lua
local projects_v2s, err = client:ProjectsV2():list()
```


### ProjectsV2Field

Create an instance: `local projects_v2_field = client:ProjectsV2Field(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `configuration` | `table` | Configuration for iteration fields. |
...
| `options` | `table` | The options available for single select fields. |
...
```lua
local projects_v2_field, err = client:ProjectsV2Field():load({ id = 1, projects_v2_id = 1 })
```

#### Example: List

```lua
local projects_v2_fields, err = client:ProjectsV2Field():list()
```


### ProjectsV2ItemSimple

Create an instance: `local projects_v2_item_simple = client:ProjectsV2ItemSimple(nil)`
...
```lua
local projects_v2_item_simple, err = client:ProjectsV2ItemSimple():create({
...
Create an instance: `local projects_v2_item_with_content = client:ProjectsV2ItemWithContent(nil)`
...
| `content` | `table` | The content of the item, which varies by content type. |
| `content_type` | `string` | The type of content tracked in a project item |
| `created_at` | `string` | The time when the item was created. |
| `creator` | `table` | A GitHub user. |
| `fields` | `table` | The fields and values associated with this item. |
...
```lua
local projects_v2_item_with_content, err = client:ProjectsV2ItemWithContent():load({ item_id = 1, projects_v2_id = 1 })
```

#### Example: List

```lua
local projects_v2_item_with_contents, err = client:ProjectsV2ItemWithContent():list()
```


### ProtectedBranch

Create an instance: `local protected_branch = client:ProtectedBranch(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `allow_deletions` | `table` | Allows deletion of the protected branch by anyone with write access to the repository. |
| `allow_force_pushes` | `table` | Permits force pushes to the protected branch by anyone with write access to the repository. |
| `allow_fork_syncing` | `table` | Whether users can pull changes from upstream when the branch is locked. |
| `block_creations` | `table` | If set to `true`, the `restrictions` branch protection settings which limits who can push will also block pushes which create new branches, unless the push is initiated by a user, team, or app which has the ability to push. |
| `enforce_admins` | `table` | Enforce all configured restrictions for administrators. |
| `lock_branch` | `table` | Whether to set the branch as read-only. |
| `required_conversation_resolution` | `table` | Requires all conversations on code to be resolved before a pull request can be merged into a branch that matches this rule. |
| `required_linear_history` | `table` | Enforces a linear commit Git history, which prevents anyone from pushing merge commits to a branch. |
| `required_pull_request_reviews` | `table` | Require at least one approving review on a pull request, before merging. |
| `required_signatures` | `table` |  |
| `required_status_checks` | `table` | Status Check Policy |
| `restrictions` | `table` | Branch Restriction Policy |
| `url` | `string` |  |
...
Create an instance: `local protected_branch_admin_enforced = client:ProtectedBranchAdminEnforced(nil)`
...
```lua
local protected_branch_admin_enforced, err = client:ProtectedBranchAdminEnforced():load({ branch_id = "branch_id", owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local protected_branch_admin_enforced, err = client:ProtectedBranchAdminEnforced():create({
  branch_id = "example_branch_id", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  enabled = true, -- boolean
...
Create an instance: `local protected_branch_pull_request_review = client:ProtectedBranchPullRequestReview(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `bypass_pull_request_allowances` | `table` | Allow specific users, teams, or apps to bypass pull request requirements. |
...
| `dismissal_restrictions` | `table` | Specify which users, teams, and apps can dismiss pull request reviews. |
...
```lua
local protected_branch_pull_request_review, err = client:ProtectedBranchPullRequestReview():load({ branch_id = "branch_id", owner = "owner", repo = "repo" })
```


### PublicMember

Create an instance: `local public_member = client:PublicMember(nil)`
...
```lua
local public_members, err = client:PublicMember():list()
```
...
| `assignees` | `table` |  |
| `author_association` | `string` | How the author is associated with the repository. |
| `auto_merge` | `table` | The status of auto merging a pull request. |
...
| `merged` | `boolean` |  |
| `merged_at` | `string` |  |
| `merged_by` | `table` | A GitHub user. |
| `message` | `string` |  |
| `milestone` | `table` | A collection of related issues and pull requests. |
| `node_id` | `string` |  |
...
Create an instance: `local pull_request_review = client:PullRequestReview(nil)`
...
| `body_html` | `string` |  |
| `body_text` | `string` |  |
| `comments` | `table` | Use the following table to specify the location, destination, and contents of the draft review comment. |
...
```lua
local pull_request_review, err = client:PullRequestReview():load({ id = 1, owner = "owner", pull_id = 1, repo = "repo" })
```

#### Example: List

```lua
local pull_request_reviews, err = client:PullRequestReview():list()
```

#### Example: Create

```lua
local pull_request_review, err = client:PullRequestReview():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author_association = "example_author_association", -- string
  body = "example_body", -- string
  ...
  message = "example_message", -- string
  node_id = "example_node_id", -- string
  pull_request_url = "example_pull_request_url", -- string
  state = "example_state", -- string
  teams = {}, -- table
  user = {}, -- table
  users = {}, -- table
...
Create an instance: `local pull_request_review_comment = client:PullRequestReviewComment(nil)`
...
```lua
local pull_request_review_comment, err = client:PullRequestReviewComment():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local pull_request_review_comments, err = client:PullRequestReviewComment():list()
```

#### Example: Create

```lua
local pull_request_review_comment, err = client:PullRequestReviewComment():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author_association = "example_author_association", -- string
  body = "example_body", -- string
  commit_id = "example_commit_id", -- string
  created_at = "example_created_at", -- string
  diff_hunk = "example_diff_hunk", -- string
  html_url = "example_html_url", -- string
  id = 1, -- number
  links = {}, -- table
  node_id = "example_node_id", -- string
  original_commit_id = "example_original_commit_id", -- string
  path = "example_path", -- string
  pull_request_review_id = 1, -- number
  pull_request_url = "example_pull_request_url", -- string
  reactions = {}, -- table
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
...
Create an instance: `local pull_request_simple = client:PullRequestSimple(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `reviewers` | `table` | An array of user `login`s that will be requested. |
| `team_reviewers` | `table` | An array of team `slug`s that will be requested. |

#### Example: Create

```lua
local pull_request_simple, err = client:PullRequestSimple():create({
  owner = "example_owner", -- string
  pull_number = 1, -- number
  repo = "example_repo", -- string
})
```
...
Create an instance: `local referrer = client:Referrer(nil)`
...
```lua
local referrers, err = client:Referrer():list()
```
...
```lua
local release, err = client:Release():load({ id = 1, owner = "owner", repo = "repo" })
```
...
local release, err = client:Release():create({
  ...
  repo = "example_repo", -- string
  assets = {}, -- table
  assets_url = "example_assets_url", -- string
  author = {}, -- table
  browser_download_url = "example_browser_download_url", -- string
  content_type = "example_content_type", -- string
  created_at = "example_created_at", -- string
  digest = "example_digest", -- string
  download_count = 1, -- number
  draft = true, -- boolean
  html_url = "example_html_url", -- string
  id = 1, -- number
  ...
  name = "example_name", -- string
  node_id = "example_node_id", -- string
  prerelease = true, -- boolean
  published_at = "example_published_at", -- string
  reactions = {}, -- table
  size = 1, -- number
  state = "example_state", -- string
  tag_name = "example_tag_name", -- string
  tarball_url = "example_tarball_url", -- string
  target_commitish = "example_target_commitish", -- string
  upload_url = "example_upload_url", -- string
  uploader = {}, -- table
  url = "example_url", -- string
  zipball_url = "example_zipball_url", -- string
})
```
...
Create an instance: `local release_asset = client:ReleaseAsset(nil)`
...
```lua
local release_asset, err = client:ReleaseAsset():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local release_assets, err = client:ReleaseAsset():list()
```

#### Example: Create

```lua
local release_asset, err = client:ReleaseAsset():create({
  id = 1, -- number
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  name = "example_name", -- string
...
Create an instance: `local release_notes_content = client:ReleaseNotesContent(nil)`
...
```lua
local release_notes_content, err = client:ReleaseNotesContent():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  body = "example_body", -- string
  name = "example_name", -- string
...
| Field | Type | Description |
| --- | --- | --- |
| `usernames` | `table` | The GitHub user handles to be removed from the team. |

#### Example: Create

```lua
local remove, err = client:Remove():create({
  enterprise = "example_enterprise", -- string
  team_id = "example_team_id", -- string
  usernames = {}, -- table
})
...
| `allow_update_branch` | `boolean` | Whether or not a pull request head branch that is behind its base branch can always be updated even if it is not required to be up to date before merging. |
| `anonymous_access_enabled` | `boolean` | Whether anonymous git access is enabled for this repository |
| `app` | `table` | A GitHub App that is providing a custom deployment protection rule. |
| `archive_url` | `string` |  |
| `archived` | `boolean` | Whether the repository is archived. |
| `assignees_url` | `string` |  |
| `attestations` | `table` |  |
...
| `bundle` | `table` | The attestation's Sigstore Bundle. |
| `client_payload` | `table` | JSON payload with extra information about the webhook event that your action or workflow may use. |
| `clone_url` | `string` |  |
...
| `permissions` | `table` |  |
| `private` | `boolean` | Whether the repository is private or public. |
| `properties` | `table` | A list of custom property names and associated values to apply to the repositories. |
| `protection_rules` | `table` | Built-in deployment protection rules for the environment. |
| `pulls_url` | `string` |  |
| `pushed_at` | `string` |  |
| `releases_url` | `string` |  |
...
| `svn_url` | `string` |  |
| `tags_url` | `string` |  |
| `team_ids` | `table` | ID of the team or teams to add to the repository. |
| `teams_url` | `string` |  |
| `temp_clone_token` | `string` |  |
| `topics` | `table` |  |
| `trees_url` | `string` |  |
...
local repo, err = client:Repo():create({
  ...
  milestones_url = "example_milestones_url", -- string
  mirror_url = "example_mirror_url", -- string
  new_owner = "example_new_owner", -- string
  node_id = "example_node_id", -- string
  notifications_url = "example_notifications_url", -- string
  open_issues = 1, -- number
  open_issues_count = 1, -- number
...
Create an instance: `local repository_advisory = client:RepositoryAdvisory(nil)`
...
| `collaborating_teams` | `table` | A list of teams that collaborate on the advisory. |
| `collaborating_users` | `table` | A list of users that collaborate on the advisory. |
| `created_at` | `string` | The date and time of when the advisory was created, in ISO 8601 format. |
| `credits` | `table` | A list of users receiving credit for their participation in the security advisory. |
| `credits_detailed` | `table` |  |
| `cve_id` | `string` | The Common Vulnerabilities and Exposures (CVE) ID. |
| `cvss` | `table` |  |
| `cvss_severities` | `table` |  |
| `cvss_vector_string` | `string` | The CVSS vector that calculates the severity of the advisory. |
| `cwe_ids` | `table` | A list of only the CWE IDs. |
| `cwes` | `table` |  |
| `description` | `string` | A detailed description of what the advisory entails. |
| `ghsa_id` | `string` | The GitHub Security Advisory ID. |
| `html_url` | `string` | The URL for the advisory. |
...
| `updated_at` | `string` | The date and time of when the advisory was last updated, in ISO 8601 format. |
| `url` | `string` | The API URL for the advisory. |
| `vulnerabilities` | `table` | A product affected by the vulnerability detailed in a repository security advisory. |
| `withdrawn_at` | `string` | The date and time of when the advisory was withdrawn, in ISO 8601 format. |

#### Example: Load

```lua
local repository_advisory, err = client:RepositoryAdvisory():load({ ghsa_id = "ghsa_id", owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local repository_advisorys, err = client:RepositoryAdvisory():list()
```

#### Example: Create

```lua
local repository_advisory, err = client:RepositoryAdvisory():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  author = "example_author", -- any
  closed_at = "example_closed_at", -- string
  ...
  credits = {}, -- table
  credits_detailed = {}, -- table
  cve_id = "example_cve_id", -- string
  cvss = {}, -- table
  cwe_ids = {}, -- table
  cwes = {}, -- table
  description = "example_description", -- string
  ghsa_id = "example_ghsa_id", -- string
  html_url = "example_html_url", -- string
  identifiers = {}, -- table
  ...
  state = "example_state", -- string
  submission = {}, -- table
  summary = "example_summary", -- string
  updated_at = "example_updated_at", -- string
  url = "example_url", -- string
  vulnerabilities = {}, -- table
  withdrawn_at = "example_withdrawn_at", -- string
})
```
...
Create an instance: `local repository_collaborator_permission = client:RepositoryCollaboratorPermission(nil)`
...
```lua
local repository_collaborator_permission, err = client:RepositoryCollaboratorPermission():load({ owner = "owner", repo = "repo", username = "username" })
```


### RepositoryInvitation

Create an instance: `local repository_invitation = client:RepositoryInvitation(nil)`
...
```lua
local repository_invitations, err = client:RepositoryInvitation():list()
```


### RepositoryRuleDetailed

Create an instance: `local repository_rule_detailed = client:RepositoryRuleDetailed(nil)`
...
```lua
local repository_rule_detailed, err = client:RepositoryRuleDetailed():load({ branch = "branch", owner = "owner", repo = "repo" })
```


### RepositoryRuleset

Create an instance: `local repository_ruleset = client:RepositoryRuleset(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `bypass_actors` | `table` | The actors that can bypass the rules in this ruleset |
...
| `node_id` | `string` |  |
| `rules` | `table` | An array of rules within the ruleset. |
...
```lua
local repository_ruleset, err = client:RepositoryRuleset():load({ id = 1 })
```

#### Example: List

```lua
local repository_rulesets, err = client:RepositoryRuleset():list()
```

#### Example: Create

```lua
local repository_ruleset, err = client:RepositoryRuleset():create({
  org_id = "example_org_id", -- string
  enforcement = "example_enforcement", -- string
  id = 1, -- number
  name = "example_name", -- string
  source = "example_source", -- string
})
...
Create an instance: `local repository_subscription = client:RepositorySubscription(nil)`
...
```lua
local repository_subscription, err = client:RepositorySubscription():load({ owner = "owner", repo = "repo" })
```


### ReviewComment

Create an instance: `local review_comment = client:ReviewComment(nil)`
...
```lua
local review_comments, err = client:ReviewComment():list()
```


### RuleSuite

Create an instance: `local rule_suite = client:RuleSuite(nil)`
...
| `rule_evaluations` | `table` | Details on the evaluated rules. |

#### Example: Load

```lua
local rule_suite, err = client:RuleSuite():load({ id = 1 })
```

#### Example: List

```lua
local rule_suites, err = client:RuleSuite():list()
```


### RulesetVersion

Create an instance: `local ruleset_version = client:RulesetVersion(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `actor` | `table` | The actor who updated the ruleset |
| `id` | `string` |  |
| `updated_at` | `string` |  |
...
```lua
local ruleset_versions, err = client:RulesetVersion():list()
```


### RulesetVersionWithState

Create an instance: `local ruleset_version_with_state = client:RulesetVersionWithState(nil)`
...
| --- | --- | --- |
| `actor` | `table` | The actor who updated the ruleset |
| `state` | `table` | The state of the ruleset version |
| `updated_at` | `string` |  |
| `version_id` | `number` | The ID of the previous version of the ruleset |
...
```lua
local ruleset_version_with_state, err = client:RulesetVersionWithState():load({ ruleset_id = 1, version_id = 1 })
```
...
```lua
local runner, err = client:Runner():load({ id = 1 })
```


### RunnerApplication

Create an instance: `local runner_application = client:RunnerApplication(nil)`
...
```lua
local runner_applications, err = client:RunnerApplication():list()
```


### RunnerGroup

Create an instance: `local runner_group = client:RunnerGroup(nil)`
...
| `runners_url` | `string` |  |
| `selected_repositories_url` | `string` | Link to the selected repositories resource for this runner group. |
| `selected_repository_ids` | `table` | List of repository IDs that can access the runner group. |
| `selected_workflows` | `table` | List of workflows the runner group should be allowed to run. |
...
```lua
local runner_group, err = client:RunnerGroup():load({ id = 1, org_id = "org_id" })
```

#### Example: Create

```lua
local runner_group, err = client:RunnerGroup():create({
  org_id = "example_org_id", -- string
  allows_public_repositories = true, -- boolean
  default = true, -- boolean
  id = 1, -- number
...
| `commit` | `table` |  |
| `commits_url` | `string` |  |
| `committer` | `table` | Metaproperties for Git author/committer information. |
| `company` | `string` |  |
| `compare_url` | `string` |  |
| `contents_url` | `string` |  |
| `contributors_url` | `string` |  |
...
Create an instance: `local secret_scanning = client:SecretScanning(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `custom_pattern_settings` | `table` | Pattern settings for custom patterns. |
| `pattern_config_version` | `string` | The updated pattern configuration version. |
| `provider_pattern_settings` | `table` | Pattern settings for provider patterns. |


### SecretScanningAlert

Create an instance: `local secret_scanning_alert = client:SecretScanningAlert(nil)`
...
```lua
local secret_scanning_alert, err = client:SecretScanningAlert():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: List

```lua
local secret_scanning_alerts, err = client:SecretScanningAlert():list()
```


### SecretScanningLocation

Create an instance: `local secret_scanning_location = client:SecretScanningLocation(nil)`
...
```lua
local secret_scanning_locations, err = client:SecretScanningLocation():list()
```


### SecretScanningPatternConfiguration

Create an instance: `local secret_scanning_pattern_configuration = client:SecretScanningPatternConfiguration(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `custom_pattern_overrides` | `table` | Overrides for custom patterns defined by the organization. |
| `pattern_config_version` | `string` | The version of the entity. |
| `provider_pattern_overrides` | `table` | Overrides for partner patterns. |

#### Example: List

```lua
local secret_scanning_pattern_configurations, err = client:SecretScanningPatternConfiguration():list()
```


### SecretScanningPushProtectionBypass

Create an instance: `local secret_scanning_push_protection_bypass = client:SecretScanningPushProtectionBypass(nil)`
...
```lua
local secret_scanning_push_protection_bypass, err = client:SecretScanningPushProtectionBypass():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  placeholder_id = "example_placeholder_id", -- string
})
```
...
Create an instance: `local secret_scanning_scan_history = client:SecretScanningScanHistory(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `backfill_scans` | `table` |  |
| `custom_pattern_backfill_scans` | `table` |  |
| `incremental_scans` | `table` |  |
| `pattern_update_scans` | `table` |  |

#### Example: List

```lua
local secret_scanning_scan_historys, err = client:SecretScanningScanHistory():list()
```


### SecurityAdvisory

Create an instance: `local security_advisory = client:SecurityAdvisory(nil)`
...
```lua
local security_advisory, err = client:SecurityAdvisory():create({
  id = "example_id", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
})
...
Create an instance: `local selected_action = client:SelectedAction(nil)`
...
```lua
local selected_actions, err = client:SelectedAction():list()
```


### SelfHostedRunner

Create an instance: `local self_hosted_runner = client:SelfHostedRunner(nil)`
...
```lua
local self_hosted_runner, err = client:SelfHostedRunner():load({ org_id = "org_id" })
```


### ShortBlob

Create an instance: `local short_blob = client:ShortBlob(nil)`
...
```lua
local short_blob, err = client:ShortBlob():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  content = "example_content", -- string
})
...
Create an instance: `local short_branch = client:ShortBranch(nil)`
...
```lua
local short_branchs, err = client:ShortBranch():list()
```


### SimpleClassroom

Create an instance: `local simple_classroom = client:SimpleClassroom(nil)`


### SimpleClassroomAssignment

Create an instance: `local simple_classroom_assignment = client:SimpleClassroomAssignment(nil)`
...
```lua
local simple_classroom_assignments, err = client:SimpleClassroomAssignment():list()
```


### SocialAccount

Create an instance: `local social_account = client:SocialAccount(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `account_urls` | `table` | Full URLs for the social media profiles to add. |
| `provider` | `string` |  |
| `url` | `string` |  |
...
```lua
local social_accounts, err = client:SocialAccount():list()
```

#### Example: Create

```lua
local social_account, err = client:SocialAccount():create({
  account_urls = {}, -- table
...
Create an instance: `local ssh_signing_key = client:SshSigningKey(nil)`
...
```lua
local ssh_signing_key, err = client:SshSigningKey():load({ id = 1 })
```

#### Example: List

```lua
local ssh_signing_keys, err = client:SshSigningKey():list()
```

#### Example: Create

```lua
local ssh_signing_key, err = client:SshSigningKey():create({
  created_at = "example_created_at", -- string
  id = 1, -- number
  key = "example_key", -- string
  title = "example_title", -- string
...
```lua
local statuss, err = client:Status():list()
```

#### Example: Create

```lua
local status, err = client:Status():create({
  id = "example_id", -- string
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  avatar_url = "example_avatar_url", -- string
...
Create an instance: `local status_check_policy = client:StatusCheckPolicy(nil)`
...
| --- | --- | --- |
| `app_id` | `number` |  |
| `checks` | `table` | The list of status checks to require in order to merge into this branch. |
| `context` | `string` |  |
| `contexts` | `table` | **Closing down notice**: The list of status checks to require in order to merge into this branch. |
| `contexts_url` | `string` |  |
...
```lua
local status_check_policys, err = client:StatusCheckPolicy():list()
```
...
Create an instance: `local tag_protection = client:TagProtection(nil)`
...
```lua
local tag_protections, err = client:TagProtection():list()
```

#### Example: Create

```lua
local tag_protection, err = client:TagProtection():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
...
| `license` | `table` | License Simple |
| `login` | `string` |  |
| `maintainers` | `table` | List GitHub usernames for organization members who will become team maintainers. |
| `master_branch` | `string` |  |
...
| `open_issues` | `number` |  |
| `open_issues_count` | `number` |  |
| `organization` | `table` | Team Organization |
| `organization_permission` | `string` | The organization permission for this project. |
| `organizations_url` | `string` |  |
| `owner` | `table` | A GitHub user. |
| `owner_url` | `string` |  |
| `parent` | `table` | Groups of organization members that gives permissions on specified repositories. |
...
| `received_events_url` | `string` |  |
| `releases_url` | `string` |  |
| `repo_names` | `table` | The full name (e.g., "organization-name/repository-name") of repositories to add the team to. |
| `repos_count` | `number` |  |
| `repos_url` | `string` |  |
| `repositories_url` | `string` |  |
...
local team, err = client:Team():create({
  ...
  blobs_url = "example_blobs_url", -- string
  body = "example_body", -- string
  body_html = "example_body_html", -- string
  body_version = "example_body_version", -- string
  branches_url = "example_branches_url", -- string
  clone_url = "example_clone_url", -- string
  code_of_conduct = {}, -- table
  collaborators_url = "example_collaborators_url", -- string
  columns_url = "example_columns_url", -- string
  comments_count = 1, -- number
  comments_url = "example_comments_url", -- string
  commits_url = "example_commits_url", -- string
  compare_url = "example_compare_url", -- string
  contents_url = "example_contents_url", -- string
  ...
  description = "example_description", -- string
  disabled = true, -- boolean
  discussion_url = "example_discussion_url", -- string
  downloads_url = "example_downloads_url", -- string
  events_url = "example_events_url", -- string
  followers_url = "example_followers_url", -- string
  following_url = "example_following_url", -- string
  ...
  language = "example_language", -- string
  languages_url = "example_languages_url", -- string
  last_edited_at = "example_last_edited_at", -- string
  license = {}, -- table
  login = "example_login", -- string
  members_count = 1, -- number
  members_url = "example_members_url", -- string
  merges_url = "example_merges_url", -- string
  milestones_url = "example_milestones_url", -- string
  mirror_url = "example_mirror_url", -- string
  ...
  owner_url = "example_owner_url", -- string
  parent = {}, -- table
  permission = "example_permission", -- string
  permissions = {}, -- table
  pinned = true, -- boolean
  private = true, -- boolean
  pulls_url = "example_pulls_url", -- string
  ...
  tags_url = "example_tags_url", -- string
  team_count = 1, -- number
  team_url = "example_team_url", -- string
  teams_url = "example_teams_url", -- string
  title = "example_title", -- string
  trees_url = "example_trees_url", -- string
  type = "example_type", -- string
...
Create an instance: `local team_simple = client:TeamSimple(nil)`
...
```lua
local team_simples, err = client:TeamSimple():list()
```
...
```lua
local thread, err = client:Thread():load({ id = 1 })
```
...
Create an instance: `local thread_subscription = client:ThreadSubscription(nil)`
...
```lua
local thread_subscription, err = client:ThreadSubscription():load({ id = 1 })
```
...
| Field | Type | Description |
| --- | --- | --- |
| `names` | `table` | An array of topics to add to the repository. |
...
| `html_url` | `string` |  |
| `id` | `number` |  |
| `inherited_from` | `table` | Team the user has gotten the role through |
...
| `url` | `string` |  |
| `user_view_type` | `string` |  |
| `users` | `table` | The username for users |
...
Create an instance: `local user_marketplace_purchase = client:UserMarketplacePurchase(nil)`
...
| `plan` | `table` | Marketplace Listing Plan |
| `unit_count` | `number` |  |
| `updated_at` | `string` |  |
...
```lua
local user_marketplace_purchases, err = client:UserMarketplacePurchase():list()
```
...
```lua
local views, err = client:View():list()
```


### WebhookConfig

Create an instance: `local webhook_config = client:WebhookConfig(nil)`
...
```lua
local webhook_config, err = client:WebhookConfig():load()
```
...
```lua
local workflow, err = client:Workflow():load({ id = "workflow_id", owner = "owner", repo = "repo" })
```
...
```lua
local workflow_run, err = client:WorkflowRun():load({ id = 1, owner = "owner", repo = "repo" })
```

#### Example: Create

```lua
local workflow_run, err = client:WorkflowRun():create({
  owner = "example_owner", -- string
  repo = "example_repo", -- string
  run_id = 1, -- number
  actor = {}, -- table
...
Create an instance: `local workflow_run_usage = client:WorkflowRunUsage(nil)`
...
```lua
local workflow_run_usage, err = client:WorkflowRunUsage():load({ owner = "owner", repo = "repo", run_id = 1 })
```


### WorkflowUsage

Create an instance: `local workflow_usage = client:WorkflowUsage(nil)`
...
```lua
local workflow_usage, err = client:WorkflowUsage():load({ id = "workflow_usage_id", owner = "owner", repo = "repo" })
```
...
```lua
local customproperty = client:CustomProperty()
customproperty:list()
