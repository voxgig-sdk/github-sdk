...
describe("ClassroomDirect", function()
  it("should direct-list-classroom", function()
    local setup = classroom_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-classroom", setup.live and "live" or "unit")
  ...
  it("should direct-load-classroom", function()
    local setup = classroom_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-classroom", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function classroom_direct_setup(mockres)
