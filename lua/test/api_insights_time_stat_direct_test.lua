-- ApiInsightsTimeStat direct test
...
describe("ApiInsightsTimeStatDirect", function()
  it("should direct-list-api_insights_time_stat", function()
    local setup = api_insights_time_stat_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_insights_time_stat", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"actor01", "actor_type01", "org01"}) do
  ...
  it("should direct-load-api_insights_time_stat", function()
    local setup = api_insights_time_stat_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-api_insights_time_stat", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function api_insights_time_stat_direct_setup(mockres)
