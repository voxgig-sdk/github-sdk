-- MarketplacePurchase direct test
...
describe("MarketplacePurchaseDirect", function()
  it("should direct-list-marketplace_purchase", function()
    local setup = marketplace_purchase_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-marketplace_purchase", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"plan01"}) do
  ...
  it("should direct-load-marketplace_purchase", function()
    local setup = marketplace_purchase_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-marketplace_purchase", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function marketplace_purchase_direct_setup(mockres)
