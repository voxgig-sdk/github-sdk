...
describe("KeyDirect", function()
  it("should direct-list-key", function()
    local setup = key_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-key", setup.live and "live" or "unit")
  ...
  it("should direct-load-key", function()
    local setup = key_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-key", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function key_direct_setup(mockres)
