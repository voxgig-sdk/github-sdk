...
describe("GpgKeyDirect", function()
  it("should direct-list-gpg_key", function()
    local setup = gpg_key_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gpg_key", setup.live and "live" or "unit")
  ...
  it("should direct-load-gpg_key", function()
    local setup = gpg_key_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-gpg_key", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function gpg_key_direct_setup(mockres)
