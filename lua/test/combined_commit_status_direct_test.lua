-- CombinedCommitStatus direct test
...
describe("CombinedCommitStatusDirect", function()
  it("should direct-list-combined_commit_status", function()
    local setup = combined_commit_status_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-combined_commit_status", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"owner01", "ref01", "repo01"}) do
...
function combined_commit_status_direct_setup(mockres)
