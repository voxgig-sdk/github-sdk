...
describe("HookDirect", function()
  it("should direct-list-hook", function()
    local setup = hook_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-hook", setup.live and "live" or "unit")
  ...
  it("should direct-load-hook", function()
    local setup = hook_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-hook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function hook_direct_setup(mockres)
