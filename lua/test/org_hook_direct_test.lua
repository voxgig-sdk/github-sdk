...
describe("OrgHookDirect", function()
  it("should direct-list-org_hook", function()
    local setup = org_hook_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-org_hook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org_hook01"}) do
  ...
  it("should direct-load-org_hook", function()
    local setup = org_hook_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-org_hook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function org_hook_direct_setup(mockres)
