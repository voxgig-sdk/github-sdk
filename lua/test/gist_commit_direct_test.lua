...
describe("GistCommitDirect", function()
  it("should direct-list-gist_commit", function()
    local setup = gist_commit_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gist_commit", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"gist_commit01"}) do
...
function gist_commit_direct_setup(mockres)
