...
describe("CheckAnnotationDirect", function()
  it("should direct-list-check_annotation", function()
    local setup = check_annotation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-check_annotation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"check_run01", "owner01", "repo01"}) do
...
function check_annotation_direct_setup(mockres)
