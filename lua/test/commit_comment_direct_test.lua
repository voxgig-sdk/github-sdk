...
describe("CommitCommentDirect", function()
  it("should direct-list-commit_comment", function()
    local setup = commit_comment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-commit_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"commit_sha01", "owner01", "repo01"}) do
  ...
  it("should direct-load-commit_comment", function()
    local setup = commit_comment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-commit_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function commit_comment_direct_setup(mockres)
