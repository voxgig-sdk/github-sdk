...
describe("CodespaceDirect", function()
  it("should direct-list-codespace", function()
    local setup = codespace_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-codespace", setup.live and "live" or "unit")
  ...
  it("should direct-load-codespace", function()
    local setup = codespace_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-codespace", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function codespace_direct_setup(mockres)
