...
describe("CustomPropertyDirect", function()
  it("should direct-list-custom_property", function()
    local setup = custom_property_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-custom_property", setup.live and "live" or "unit")
  ...
  it("should direct-load-custom_property", function()
    local setup = custom_property_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-custom_property", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function custom_property_direct_setup(mockres)
