...
describe("RuleSuiteDirect", function()
  it("should direct-list-rule_suite", function()
    local setup = rule_suite_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rule_suite", setup.live and "live" or "unit")
  ...
  it("should direct-load-rule_suite", function()
    local setup = rule_suite_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-rule_suite", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function rule_suite_direct_setup(mockres)
