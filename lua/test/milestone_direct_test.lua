...
describe("MilestoneDirect", function()
  it("should direct-list-milestone", function()
    local setup = milestone_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-milestone", setup.live and "live" or "unit")
  ...
  it("should direct-load-milestone", function()
    local setup = milestone_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-milestone", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function milestone_direct_setup(mockres)
