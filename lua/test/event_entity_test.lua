...
describe("EventEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_EVENT_ENTID JSON to run live")
      return
    end
...
function event_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "event01", "event02", "event03", "network01", "network02", "network03", "repo01", "repo02", "repo03", "user01", "user02", "user03", "org01", "org02", "org03", "username01" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("GITHUB_TEST_EVENT_ENTID")
