-- CodeSecurityConfigurationRepository direct test
...
describe("CodeSecurityConfigurationRepositoryDirect", function()
  it("should direct-list-code_security_configuration_repository", function()
    local setup = code_security_configuration_repository_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-code_security_configuration_repository", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"configuration01", "enterprise01"}) do
...
function code_security_configuration_repository_direct_setup(mockres)
