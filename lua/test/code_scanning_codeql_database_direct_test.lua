-- CodeScanningCodeqlDatabase direct test
...
describe("CodeScanningCodeqlDatabaseDirect", function()
  it("should direct-list-code_scanning_codeql_database", function()
    local setup = code_scanning_codeql_database_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-code_scanning_codeql_database", setup.live and "live" or "unit")
  ...
  it("should direct-load-code_scanning_codeql_database", function()
    local setup = code_scanning_codeql_database_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-code_scanning_codeql_database", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function code_scanning_codeql_database_direct_setup(mockres)
