...
describe("AutolinkDirect", function()
  it("should direct-list-autolink", function()
    local setup = autolink_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-autolink", setup.live and "live" or "unit")
  ...
  it("should direct-load-autolink", function()
    local setup = autolink_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-autolink", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function autolink_direct_setup(mockres)
