-- PullRequestReviewComment direct test
...
describe("PullRequestReviewCommentDirect", function()
  it("should direct-list-pull_request_review_comment", function()
    local setup = pull_request_review_comment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-pull_request_review_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"owner01", "pull_number01", "repo01"}) do
  ...
  it("should direct-load-pull_request_review_comment", function()
    local setup = pull_request_review_comment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-pull_request_review_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function pull_request_review_comment_direct_setup(mockres)
