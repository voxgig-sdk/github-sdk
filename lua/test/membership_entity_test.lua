...
describe("MembershipEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_MEMBERSHIP_ENTID JSON to run live")
      return
    end
    ...
    local membership_ref01_markdef_up0_name = "avatar_url"
    local membership_ref01_markdef_up0_value = "Mark01-membership_ref01_" .. tostring(setup.now)
    ...
    assert.are.equal(membership_ref01_resdata_up0[membership_ref01_markdef_up0_name], membership_ref01_markdef_up0_value)
...
function membership_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GITHUB_TEST_MEMBERSHIP_ENTID")
  ...
  if idmap_resolved["team_id"] == nil then
