-- EnvironmentApproval direct test
...
describe("EnvironmentApprovalDirect", function()
  it("should direct-list-environment_approval", function()
    local setup = environment_approval_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-environment_approval", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"owner01", "repo01", "run01"}) do
...
function environment_approval_direct_setup(mockres)
