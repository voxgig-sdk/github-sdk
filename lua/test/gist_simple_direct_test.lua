...
describe("GistSimpleDirect", function()
  it("should direct-list-gist_simple", function()
    local setup = gist_simple_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gist_simple", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"gist_simple01"}) do
...
function gist_simple_direct_setup(mockres)
