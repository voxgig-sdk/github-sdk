...
describe("MembershipDirect", function()
  it("should direct-list-membership", function()
    local setup = membership_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-membership", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"enterprise01", "enterprise_team01"}) do
    ...
    if setup.live then
      ...
      params["enterprise_team"] = "direct01"
    end
  ...
  it("should direct-load-membership", function()
    local setup = membership_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-membership", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function membership_direct_setup(mockres)
