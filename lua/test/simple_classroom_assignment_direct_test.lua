-- SimpleClassroomAssignment direct test
...
describe("SimpleClassroomAssignmentDirect", function()
  it("should direct-list-simple_classroom_assignment", function()
    local setup = simple_classroom_assignment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-simple_classroom_assignment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"classroom01"}) do
...
function simple_classroom_assignment_direct_setup(mockres)
