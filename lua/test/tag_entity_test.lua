...
describe("TagEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_TAG_ENTID JSON to run live")
      return
    end
...
function tag_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "tag01", "tag02", "tag03", "repo01", "repo02", "repo03", "owner01" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("GITHUB_TEST_TAG_ENTID")
