...
describe("DeploymentStatusDirect", function()
  it("should direct-list-deployment_status", function()
    local setup = deployment_status_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-deployment_status", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"deployment_status01", "owner01", "repo01"}) do
  ...
  it("should direct-load-deployment_status", function()
    local setup = deployment_status_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-deployment_status", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function deployment_status_direct_setup(mockres)
