-- ClassroomAcceptedAssignment direct test
...
describe("ClassroomAcceptedAssignmentDirect", function()
  it("should direct-list-classroom_accepted_assignment", function()
    local setup = classroom_accepted_assignment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-classroom_accepted_assignment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"assignment01"}) do
...
function classroom_accepted_assignment_direct_setup(mockres)
