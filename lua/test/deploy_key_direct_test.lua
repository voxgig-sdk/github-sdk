...
describe("DeployKeyDirect", function()
  it("should direct-list-deploy_key", function()
    local setup = deploy_key_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-deploy_key", setup.live and "live" or "unit")
  ...
  it("should direct-load-deploy_key", function()
    local setup = deploy_key_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-deploy_key", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function deploy_key_direct_setup(mockres)
