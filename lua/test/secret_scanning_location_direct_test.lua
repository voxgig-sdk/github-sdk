-- SecretScanningLocation direct test
...
describe("SecretScanningLocationDirect", function()
  it("should direct-list-secret_scanning_location", function()
    local setup = secret_scanning_location_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-secret_scanning_location", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"alert_number01", "owner01", "repo01"}) do
...
function secret_scanning_location_direct_setup(mockres)
