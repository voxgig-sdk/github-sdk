-- BranchRestrictionPolicy direct test
...
describe("BranchRestrictionPolicyDirect", function()
  it("should direct-list-branch_restriction_policy", function()
    local setup = branch_restriction_policy_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-branch_restriction_policy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"branch01", "owner01", "repo01"}) do
...
function branch_restriction_policy_direct_setup(mockres)
