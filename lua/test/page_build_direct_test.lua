...
describe("PageBuildDirect", function()
  it("should direct-list-page_build", function()
    local setup = page_build_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-page_build", setup.live and "live" or "unit")
  ...
  it("should direct-load-page_build", function()
    local setup = page_build_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-page_build", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function page_build_direct_setup(mockres)
