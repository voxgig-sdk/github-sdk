...
describe("HookDeliveryItemDirect", function()
  it("should direct-list-hook_delivery_item", function()
    local setup = hook_delivery_item_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-hook_delivery_item", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"hook_delivery_item01", "owner01", "repo01"}) do
...
function hook_delivery_item_direct_setup(mockres)
