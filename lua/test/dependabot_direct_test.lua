...
describe("DependabotDirect", function()
  it("should direct-list-dependabot", function()
    local setup = dependabot_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-dependabot", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01", "secret_name01"}) do
...
function dependabot_direct_setup(mockres)
