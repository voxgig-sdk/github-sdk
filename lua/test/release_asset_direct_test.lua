...
describe("ReleaseAssetDirect", function()
  it("should direct-list-release_asset", function()
    local setup = release_asset_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-release_asset", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"release_asset01", "owner01", "repo01"}) do
  ...
  it("should direct-load-release_asset", function()
    local setup = release_asset_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-release_asset", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function release_asset_direct_setup(mockres)
