...
describe("ReviewCommentDirect", function()
  it("should direct-list-review_comment", function()
    local setup = review_comment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-review_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"review_comment01", "owner01", "pull01", "repo01"}) do
...
function review_comment_direct_setup(mockres)
