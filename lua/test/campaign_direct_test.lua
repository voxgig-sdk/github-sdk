...
describe("CampaignDirect", function()
  it("should direct-list-campaign", function()
    local setup = campaign_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-campaign", setup.live and "live" or "unit")
  ...
  it("should direct-load-campaign", function()
    local setup = campaign_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-campaign", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function campaign_direct_setup(mockres)
