-- ProjectsV2ItemWithContent direct test
...
describe("ProjectsV2ItemWithContentDirect", function()
  it("should direct-list-projects_v2_item_with_content", function()
    local setup = projects_v2_item_with_content_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-projects_v2_item_with_content", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01", "project_number01"}) do
    ...
    if setup.live then
      ...
      params["project_number"] = "direct01"
    end
  ...
  it("should direct-load-projects_v2_item_with_content", function()
    local setup = projects_v2_item_with_content_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-projects_v2_item_with_content", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function projects_v2_item_with_content_direct_setup(mockres)
