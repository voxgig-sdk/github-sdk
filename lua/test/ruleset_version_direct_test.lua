...
describe("RulesetVersionDirect", function()
  it("should direct-list-ruleset_version", function()
    local setup = ruleset_version_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ruleset_version", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"ruleset_version01", "owner01", "repo01"}) do
...
function ruleset_version_direct_setup(mockres)
