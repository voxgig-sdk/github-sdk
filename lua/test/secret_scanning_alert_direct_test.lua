-- SecretScanningAlert direct test
...
describe("SecretScanningAlertDirect", function()
  it("should direct-list-secret_scanning_alert", function()
    local setup = secret_scanning_alert_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-secret_scanning_alert", setup.live and "live" or "unit")
  ...
  it("should direct-load-secret_scanning_alert", function()
    local setup = secret_scanning_alert_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-secret_scanning_alert", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function secret_scanning_alert_direct_setup(mockres)
