...
describe("CopilotDirect", function()
  it("should direct-list-copilot", function()
    local setup = copilot_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-copilot", setup.live and "live" or "unit")
  ...
  it("should direct-load-copilot", function()
    local setup = copilot_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-copilot", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function copilot_direct_setup(mockres)
