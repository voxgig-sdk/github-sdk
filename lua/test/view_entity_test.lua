...
describe("ViewEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:View(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:View(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_VIEW_ENTID JSON to run live")
      return
    end
    ...
    local view_ref01_match = {
      ["owner"] = setup.idmap["owner01"],
      ["repo"] = setup.idmap["repo01"],
    }

    local view_ref01_list_result, err = view_ref01_ent:list(view_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(view_ref01_list_result)
...
function view_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "view01", "view02", "view03", "repo01", "repo02", "repo03", "owner01" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("GITHUB_TEST_VIEW_ENTID")
