...
func TestPageBuildDirect(t *testing.T) {
	t.Run("direct-list-page_build", func(t *testing.T) {
		setup := page_buildDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-page_build", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-page_build", func(t *testing.T) {
		setup := page_buildDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-page_build", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type page_buildDirectSetupResult struct {
...
func page_buildDirectSetup(mockres any) *page_buildDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_PAGE_BUILD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_PAGE_BUILD_ENTID"]; ok {
		...
		return &page_buildDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &page_buildDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
