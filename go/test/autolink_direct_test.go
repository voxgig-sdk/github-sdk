...
func TestAutolinkDirect(t *testing.T) {
	t.Run("direct-list-autolink", func(t *testing.T) {
		setup := autolinkDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-autolink", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-autolink", func(t *testing.T) {
		setup := autolinkDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-autolink", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{repo}/autolinks/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type autolinkDirectSetupResult struct {
...
func autolinkDirectSetup(mockres any) *autolinkDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_AUTOLINK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_AUTOLINK_ENTID"]; ok {
		...
		return &autolinkDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &autolinkDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
