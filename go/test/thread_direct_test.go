...
func TestThreadDirect(t *testing.T) {
	...
	t.Run("direct-load-thread", func(t *testing.T) {
		setup := threadDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-thread", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func threadDirectSetup(mockres any) *threadDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_THREAD_ENTID"]; ok {
