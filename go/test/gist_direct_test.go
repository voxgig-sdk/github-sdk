...
func TestGistDirect(t *testing.T) {
	...
	t.Run("direct-load-gist", func(t *testing.T) {
		setup := gistDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-gist", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func gistDirectSetup(mockres any) *gistDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GITHUB_TEST_GIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_GIST_ENTID"]; ok {
