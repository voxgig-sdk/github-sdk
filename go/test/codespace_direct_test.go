...
func TestCodespaceDirect(t *testing.T) {
	t.Run("direct-list-codespace", func(t *testing.T) {
		setup := codespaceDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-codespace", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-codespace", func(t *testing.T) {
		setup := codespaceDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-codespace", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type codespaceDirectSetupResult struct {
...
func codespaceDirectSetup(mockres any) *codespaceDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_CODESPACE_ENTID"]; ok {
		...
		return &codespaceDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &codespaceDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
