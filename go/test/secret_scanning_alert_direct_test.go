...
func TestSecretScanningAlertDirect(t *testing.T) {
	t.Run("direct-list-secret_scanning_alert", func(t *testing.T) {
		setup := secret_scanning_alertDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-secret_scanning_alert", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-secret_scanning_alert", func(t *testing.T) {
		setup := secret_scanning_alertDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-secret_scanning_alert", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{repo}/secret-scanning/alerts/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type secret_scanning_alertDirectSetupResult struct {
...
func secret_scanning_alertDirectSetup(mockres any) *secret_scanning_alertDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_SECRET_SCANNING_ALERT_ENTID"]; ok {
		...
		return &secret_scanning_alertDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &secret_scanning_alertDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
