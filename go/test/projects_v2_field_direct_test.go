...
func TestProjectsV2FieldDirect(t *testing.T) {
	t.Run("direct-list-projects_v2_field", func(t *testing.T) {
		setup := projects_v2_fieldDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-projects_v2_field", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "orgs/{org_id}/projectsV2/{project_number}/fields",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-projects_v2_field", func(t *testing.T) {
		setup := projects_v2_fieldDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-projects_v2_field", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "orgs/{org_id}/projectsV2/{projects_v2_id}/fields/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type projects_v2_fieldDirectSetupResult struct {
...
func projects_v2_fieldDirectSetup(mockres any) *projects_v2_fieldDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_PROJECTS_V2_FIELD_ENTID"]; ok {
		...
		return &projects_v2_fieldDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &projects_v2_fieldDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
