...
func TestDeploymentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deploymentRef01Data["owner"] = setup.idmap["owner01"]
		deploymentRef01Data["repo"] = setup.idmap["repo01"]
...
func deploymentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"deployment01", "deployment02", "deployment03", "repo01", "repo02", "repo03", "run01", "run02", "run03", "owner01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_DEPLOYMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_DEPLOYMENT_ENTID"])
