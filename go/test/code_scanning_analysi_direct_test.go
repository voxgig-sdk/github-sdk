...
func TestCodeScanningAnalysiDirect(t *testing.T) {
	t.Run("direct-list-code_scanning_analysi", func(t *testing.T) {
		setup := code_scanning_analysiDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-code_scanning_analysi", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-code_scanning_analysi", func(t *testing.T) {
		setup := code_scanning_analysiDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-code_scanning_analysi", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["analysis_id"] = setup.idmap["analysis01"]
			params["owner"] = setup.idmap["owner01"]
			params["repo"] = setup.idmap["repo01"]
		} else {
			params["analysis_id"] = "direct01"
...
type code_scanning_analysiDirectSetupResult struct {
...
func code_scanning_analysiDirectSetup(mockres any) *code_scanning_analysiDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_CODE_SCANNING_ANALYSI_ENTID"]; ok {
		...
		return &code_scanning_analysiDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &code_scanning_analysiDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
