...
func TestHookDirect(t *testing.T) {
	t.Run("direct-list-hook", func(t *testing.T) {
		setup := hookDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-hook", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-hook", func(t *testing.T) {
		setup := hookDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-hook", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type hookDirectSetupResult struct {
...
func hookDirectSetup(mockres any) *hookDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_HOOK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_HOOK_ENTID"]; ok {
		...
		return &hookDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &hookDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
