...
func TestRuleSuiteDirect(t *testing.T) {
	t.Run("direct-list-rule_suite", func(t *testing.T) {
		setup := rule_suiteDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-rule_suite", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "orgs/{org_id}/rulesets/rule-suites",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-rule_suite", func(t *testing.T) {
		setup := rule_suiteDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-rule_suite", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{repo}/rulesets/rule-suites/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type rule_suiteDirectSetupResult struct {
...
func rule_suiteDirectSetup(mockres any) *rule_suiteDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_RULE_SUITE_ENTID"]; ok {
		...
		return &rule_suiteDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &rule_suiteDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
