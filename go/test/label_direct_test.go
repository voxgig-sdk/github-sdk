...
func TestLabelDirect(t *testing.T) {
	t.Run("direct-list-label", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"milestone_number01", "owner01", "repo01"} {
	...
	t.Run("direct-load-label", func(t *testing.T) {
		...
		if setup.live {
			...
			listParams["milestone_number"] = setup.idmap["milestone_number01"]
...
func labelDirectSetup(mockres any) *labelDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GITHUB_TEST_LABEL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_LABEL_ENTID"]; ok {
