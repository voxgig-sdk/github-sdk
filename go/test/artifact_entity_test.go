...
func TestArtifactEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_ARTIFACT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		artifactRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.artifact")))
		var artifactRef01Data map[string]any
		if len(artifactRef01DataRaw) > 0 {
			artifactRef01Data = core.ToMapAny(artifactRef01DataRaw[0][1])
		}
...
func artifactBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"artifact01", "artifact02", "artifact03", "repo01", "repo02", "repo03", "owner01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_ARTIFACT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_ARTIFACT_ENTID"])
