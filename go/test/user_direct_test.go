...
func TestUserDirect(t *testing.T) {
	t.Run("direct-list-user", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"org01", "organization_role01"} {
	...
	t.Run("direct-load-user", func(t *testing.T) {
		...
		if setup.live {
			...
			listParams["organization_role_id"] = setup.idmap["organization_role01"]
...
func userDirectSetup(mockres any) *userDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GITHUB_TEST_USER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_USER_ENTID"]; ok {
