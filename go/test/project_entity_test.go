...
func TestProjectEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_PROJECT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		projectRef01Data["org_id"] = setup.idmap["org01"]
		projectRef01Data["owner"] = setup.idmap["owner01"]
		projectRef01Data["projects_v2_id"] = setup.idmap["projects_v201"]
		projectRef01Data["repo"] = setup.idmap["repo01"]
		projectRef01Data["username"] = setup.idmap["username01"]
		...
		projectRef01MarkdefUp0Name := "avatar_url"
...
func projectBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"project01", "project02", "project03", "org01", "org02", "org03", "repo01", "repo02", "repo03", "user01", "user02", "user03", "projects_v201", "projects_v202", "projects_v203", "item01", "item02", "item03", "owner01", "username01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_PROJECT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_PROJECT_ENTID"])
