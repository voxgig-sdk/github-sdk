...
func TestMilestoneDirect(t *testing.T) {
	t.Run("direct-list-milestone", func(t *testing.T) {
		setup := milestoneDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-milestone", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-milestone", func(t *testing.T) {
		setup := milestoneDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-milestone", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type milestoneDirectSetupResult struct {
...
func milestoneDirectSetup(mockres any) *milestoneDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_MILESTONE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_MILESTONE_ENTID"]; ok {
		...
		return &milestoneDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &milestoneDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
