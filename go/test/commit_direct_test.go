...
func TestCommitDirect(t *testing.T) {
	...
	t.Run("direct-load-commit", func(t *testing.T) {
		setup := commitDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-commit", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{repo}/commits/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
func commitDirectSetup(mockres any) *commitDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GITHUB_TEST_COMMIT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_COMMIT_ENTID"]; ok {
