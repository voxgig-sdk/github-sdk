...
func TestDeployKeyDirect(t *testing.T) {
	t.Run("direct-list-deploy_key", func(t *testing.T) {
		setup := deploy_keyDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-deploy_key", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-deploy_key", func(t *testing.T) {
		setup := deploy_keyDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-deploy_key", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type deploy_keyDirectSetupResult struct {
...
func deploy_keyDirectSetup(mockres any) *deploy_keyDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_DEPLOY_KEY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_DEPLOY_KEY_ENTID"]; ok {
		...
		return &deploy_keyDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &deploy_keyDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
