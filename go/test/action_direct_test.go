...
func TestActionDirect(t *testing.T) {
	t.Run("direct-list-action", func(t *testing.T) {
		setup := actionDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-action", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"owner01", "repo01", "workflow01"} {
	...
	t.Run("direct-load-action", func(t *testing.T) {
		setup := actionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-action", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["workflow_id"] = setup.idmap["workflow01"]
...
type actionDirectSetupResult struct {
...
func actionDirectSetup(mockres any) *actionDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_ACTION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_ACTION_ENTID"]; ok {
		...
		return &actionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &actionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
