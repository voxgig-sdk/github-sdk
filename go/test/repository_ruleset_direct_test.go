...
func TestRepositoryRulesetDirect(t *testing.T) {
	t.Run("direct-list-repository_ruleset", func(t *testing.T) {
		setup := repository_rulesetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-repository_ruleset", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-repository_ruleset", func(t *testing.T) {
		setup := repository_rulesetDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-repository_ruleset", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type repository_rulesetDirectSetupResult struct {
...
func repository_rulesetDirectSetup(mockres any) *repository_rulesetDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_REPOSITORY_RULESET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_REPOSITORY_RULESET_ENTID"]; ok {
		...
		return &repository_rulesetDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &repository_rulesetDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
