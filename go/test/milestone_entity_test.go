...
func TestMilestoneEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		milestoneRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "milestone"}), "milestone_ref01"))
		milestoneRef01Data["owner"] = setup.idmap["owner01"]
		milestoneRef01Data["repo"] = setup.idmap["repo01"]
		...
		milestoneRef01MarkdefUp0Name := "avatar_url"
...
func milestoneBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"milestone01", "milestone02", "milestone03", "repo01", "repo02", "repo03", "owner01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_MILESTONE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_MILESTONE_ENTID"])
