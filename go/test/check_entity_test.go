...
func TestCheckEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		_, checkRef01ListOk := checkRef01ListResult.([]any)
		if !checkRef01ListOk {
...
func checkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"check01", "check02", "check03", "repo01", "repo02", "repo03", "check_suite01", "check_suite02", "check_suite03", "commit01", "commit02", "commit03", "owner01", "ref01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_CHECK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_CHECK_ENTID"])
