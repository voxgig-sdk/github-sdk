...
func TestCodeScanningCodeqlDatabaseDirect(t *testing.T) {
	t.Run("direct-list-code_scanning_codeql_database", func(t *testing.T) {
		setup := code_scanning_codeql_databaseDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-code_scanning_codeql_database", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-code_scanning_codeql_database", func(t *testing.T) {
		setup := code_scanning_codeql_databaseDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-code_scanning_codeql_database", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type code_scanning_codeql_databaseDirectSetupResult struct {
...
func code_scanning_codeql_databaseDirectSetup(mockres any) *code_scanning_codeql_databaseDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_CODE_SCANNING_CODEQL_DATABASE_ENTID"]; ok {
		...
		return &code_scanning_codeql_databaseDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &code_scanning_codeql_databaseDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
