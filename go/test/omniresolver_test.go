...
import (
	...
	omni "github.com/voxgig-sdk/github-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GithubError); is {
				out := map[string]any{
					"name":    "GithubError",
...
func omniCtx(arg any, client *sdk.GithubSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GithubSDK); is {
			client = live
