...
func TestReleaseAssetDirect(t *testing.T) {
	t.Run("direct-list-release_asset", func(t *testing.T) {
		setup := release_assetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-release_asset", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"release_asset01", "owner01", "repo01"} {
		...
		if setup.live {
			params["id"] = setup.idmap["release_asset01"]
		} else {
			params["id"] = "direct01"
		}
		if setup.live {
	...
	t.Run("direct-load-release_asset", func(t *testing.T) {
		setup := release_assetDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-release_asset", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["id"] = setup.idmap["release_asset01"]
...
type release_assetDirectSetupResult struct {
...
func release_assetDirectSetup(mockres any) *release_assetDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_RELEASE_ASSET_ENTID"]; ok {
		...
		return &release_assetDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &release_assetDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
