...
func TestThreadEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_THREAD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		// LOAD
		threadRef01MatchDt0 := map[string]any{
			"id": threadRef01Data["id"],
		}
		threadRef01DataDt0Loaded, err := threadRef01Ent.Load(threadRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		threadRef01DataDt0LoadResult := core.ToMapAny(entityData(threadRef01DataDt0Loaded))
		if threadRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if threadRef01DataDt0LoadResult["id"] != threadRef01Data["id"] {
...
func threadBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"thread01", "thread02", "thread03", "repo01", "repo02", "repo03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_THREAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUB_TEST_THREAD_ENTID"])
