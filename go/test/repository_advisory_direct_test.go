...
func TestRepositoryAdvisoryDirect(t *testing.T) {
	t.Run("direct-list-repository_advisory", func(t *testing.T) {
		setup := repository_advisoryDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-repository_advisory", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-repository_advisory", func(t *testing.T) {
		setup := repository_advisoryDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-repository_advisory", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["ghsa_id"] = setup.idmap["ghsa01"]
			params["owner"] = setup.idmap["owner01"]
			params["repo"] = setup.idmap["repo01"]
		} else {
			params["ghsa_id"] = "direct01"
...
type repository_advisoryDirectSetupResult struct {
...
func repository_advisoryDirectSetup(mockres any) *repository_advisoryDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITHUB_TEST_REPOSITORY_ADVISORY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_REPOSITORY_ADVISORY_ENTID"]; ok {
		...
		return &repository_advisoryDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &repository_advisoryDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
