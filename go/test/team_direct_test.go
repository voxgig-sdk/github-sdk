...
func TestTeamDirect(t *testing.T) {
	t.Run("direct-list-team", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"discussion_number01", "org01", "team01"} {
	...
	t.Run("direct-load-team", func(t *testing.T) {
		...
		if setup.live {
			...
			listParams["discussion_number"] = setup.idmap["discussion_number01"]
			listParams["org_id"] = setup.idmap["org01"]
			listParams["team_id"] = setup.idmap["team01"]
...
func teamDirectSetup(mockres any) *teamDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GITHUB_TEST_TEAM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_TEAM_ENTID"]; ok {
