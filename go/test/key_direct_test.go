...
func TestKeyDirect(t *testing.T) {
	t.Run("direct-list-key", func(t *testing.T) {
		setup := keyDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-key", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-key", func(t *testing.T) {
		setup := keyDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-key", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type keyDirectSetupResult struct {
...
func keyDirectSetup(mockres any) *keyDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITHUB_TEST_KEY_ENTID"]; ok {
		...
		return &keyDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &keyDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
