...
func TestSubscriberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUB_TEST_SUBSCRIBER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		subscriberRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.subscriber")))
		var subscriberRef01Data map[string]any
		if len(subscriberRef01DataRaw) > 0 {
			subscriberRef01Data = core.ToMapAny(subscriberRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = subscriberRef01Data
		...
		_, subscriberRef01ListOk := subscriberRef01ListResult.([]any)
...
func subscriberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"subscriber01", "subscriber02", "subscriber03", "repo01", "repo02", "repo03", "owner01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITHUB_TEST_SUBSCRIBER_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"GITHUB_TEST_SUBSCRIBER_ENTID": idmap,
		"GITHUB_TEST_LIVE":      "FALSE",
		"GITHUB_TEST_EXPLAIN":   "FALSE",
		"GITHUB_APIKEY":         "",
	})

	idmapResolved := core.ToMapAny(env["GITHUB_TEST_SUBSCRIBER_ENTID"])
