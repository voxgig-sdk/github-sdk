...
func NewHovercardEntity(client *core.GithubSDK, entopts map[string]any) *HovercardEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *HovercardEntity) GetName() string { return e.name }
...
func (e *HovercardEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *HovercardEntity) Deleted() bool {
	return e.deleted
}


func (e *HovercardEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewHovercardEntity(e.client, opts)
}

func (e *HovercardEntity) Data(args ...any) any {
...
func (e *HovercardEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Hovercard; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *HovercardEntity) DataTyped(data ...Hovercard) Hovercard {
	if len(data) > 0 {
		return typedFrom[Hovercard](e.Data(asMap(data[0])))
	}
	return typedFrom[Hovercard](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Hovercard (all fields
// optional at the wire level).
func (e *HovercardEntity) MatchTyped(match ...Hovercard) Hovercard {
	if len(match) > 0 {
		return typedFrom[Hovercard](e.Match(asMap(match[0])))
	}
	return typedFrom[Hovercard](e.Match())
}
...
func (e *HovercardEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *HovercardEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *HovercardEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// HovercardListMatch and returns []Hovercard. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *HovercardEntity) ListTyped(reqmatch HovercardListMatch, ctrl map[string]any) ([]Hovercard, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Hovercard](res), nil
}



func (e *HovercardEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *HovercardEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *HovercardEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *HovercardEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
