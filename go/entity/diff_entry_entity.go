...
func NewDiffEntryEntity(client *core.GithubSDK, entopts map[string]any) *DiffEntryEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *DiffEntryEntity) GetName() string { return e.name }
...
func (e *DiffEntryEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *DiffEntryEntity) Deleted() bool {
	return e.deleted
}


func (e *DiffEntryEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewDiffEntryEntity(e.client, opts)
}

func (e *DiffEntryEntity) Data(args ...any) any {
...
func (e *DiffEntryEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an DiffEntry; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *DiffEntryEntity) DataTyped(data ...DiffEntry) DiffEntry {
	if len(data) > 0 {
		return typedFrom[DiffEntry](e.Data(asMap(data[0])))
	}
	return typedFrom[DiffEntry](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through DiffEntry (all fields
// optional at the wire level).
func (e *DiffEntryEntity) MatchTyped(match ...DiffEntry) DiffEntry {
	if len(match) > 0 {
		return typedFrom[DiffEntry](e.Match(asMap(match[0])))
	}
	return typedFrom[DiffEntry](e.Match())
}
...
func (e *DiffEntryEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *DiffEntryEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *DiffEntryEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// DiffEntryListMatch and returns []DiffEntry. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *DiffEntryEntity) ListTyped(reqmatch DiffEntryListMatch, ctrl map[string]any) ([]DiffEntry, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[DiffEntry](res), nil
}



func (e *DiffEntryEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *DiffEntryEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *DiffEntryEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *DiffEntryEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
