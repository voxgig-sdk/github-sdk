...
type CodeownersErrorEntity struct {
	name    string
...
func NewCodeownersErrorEntity(client *core.GithubSDK, entopts map[string]any) *CodeownersErrorEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
	...
	e := &CodeownersErrorEntity{
		name:    "codeowners_error",
		client:  client,
...
func (e *CodeownersErrorEntity) GetName() string { return e.name }
...
func (e *CodeownersErrorEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *CodeownersErrorEntity) Deleted() bool {
	return e.deleted
}


func (e *CodeownersErrorEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewCodeownersErrorEntity(e.client, opts)
}

func (e *CodeownersErrorEntity) Data(args ...any) any {
...
func (e *CodeownersErrorEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an CodeownersError; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *CodeownersErrorEntity) DataTyped(data ...CodeownersError) CodeownersError {
	if len(data) > 0 {
		return typedFrom[CodeownersError](e.Data(asMap(data[0])))
	}
	return typedFrom[CodeownersError](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through CodeownersError (all fields
// optional at the wire level).
func (e *CodeownersErrorEntity) MatchTyped(match ...CodeownersError) CodeownersError {
	if len(match) > 0 {
		return typedFrom[CodeownersError](e.Match(asMap(match[0])))
	}
	return typedFrom[CodeownersError](e.Match())
}
...
func (e *CodeownersErrorEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *CodeownersErrorEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *CodeownersErrorEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// CodeownersErrorListMatch and returns []CodeownersError. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *CodeownersErrorEntity) ListTyped(reqmatch CodeownersErrorListMatch, ctrl map[string]any) ([]CodeownersError, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[CodeownersError](res), nil
}



func (e *CodeownersErrorEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *CodeownersErrorEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *CodeownersErrorEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *CodeownersErrorEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
