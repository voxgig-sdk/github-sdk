...
class FtCtrl extends GithubControl
{
...
class FtClient
{
    ...
    private GithubUtility $utility;

    public function __construct(GithubUtility $utility, array $options)
    {
    ...
    public function get_utility(): GithubUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GithubContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GithubContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GithubConfig::make_feature($name);
    ...
    private static function buildUrl(GithubSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GithubError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GithubResponse($fetched);
        }
    ...
    private function fail(GithubContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GithubContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
